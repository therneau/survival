...
Step 1 is to accumulate all the terms that appear on the right hand side(s)
into a single ``pseudo formula'' that is used to drive the model.frame
process.  
...
\begin{itemize}
  ...
  \item each element is a list, with one element per term: multiple
    a:b terms are allowed separated by + signs on the left hand side of the ~
