...
multihaz <- function(y, x, position, weight, risk, istrat, ctype, stype, 
    ...
    # We will use cn[,,5]/denom later, avoid any 0/0
    # cn[,,4] is safer since it is integer, but if someone had case weights,
    #  and a subject with weight=0 were the only event, we need cn[,,5]
    noevent <- (cn[,,4]==0 || cn[,,5]==0)
    if (ctype ==1) {
        denom1 <- ifelse(noevent, 1, cn[,,3])   # avoid a later 0/0
        denom2 <- ifelse(noevent, 1, cn[,,3]^2)
    } else {
        denom1 <- ifelse(noevent, 1, cn[,,11])
        denom2 <- ifelse(noevent, 1, cn[,,12])
    }
