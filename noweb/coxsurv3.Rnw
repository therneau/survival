...
an mf3 model frame and an x2 matrix.  We need to expand the latter so as to get 
one matrix for each
...
<<survfit.coxphms-setup3b>>=
cmap <- object$cmap[-1,,drop=FALSE]
x3 <- array(0., dim=c(nrow(x2), max(cmap), ncol(cmap)))
for (i in 1:ncol(cmap)) 
    x3[, cmap[,i], i] <- x2[, cmap[,i]>0]
risk3 <- matrix(x3, ncol= max(cmap)) %*% object$coef
...
if (individual) {
    ...
    if (is.null(cifit$strata)) {
        ...
        cifit$dim <- c(1, nrow(x2), ncol(temp$pstate))
    }
    else {
        ...
        cifit$pstate <- new.p
        cifit$cumhaz <- new.c
        cifit$dim <- c(nx, nrow(x2), ncol(temp$pstate))
    }
    cifit$newdata <- mf2
}
