...
of the covariate are 0 for instance) in which case we will recalculate the
value multiple times unneccarily, since we are using value=0 as a marker for
``not yet computed''.           
...
double coxd2(int d, int n, double *score, double *dmat, double *d1j,
	...
	if (d>1) d2[indx] += score[n-1] * (
	    ...
	    covarj[n-1] * coxd1(d-1, n-1, score, dmat, d1j, covarj, dmax) +
	    covark[n-1] * coxd1(d-1, n-1, score, dmat, d1k, covark, dmax));
	printf(" d=%d, n=%d, d2=%f\n", d, n, d2[indx]);
	}
    return(d2[indx]);
}
...
for (i=0; i<nused;) {
  if (strata[i]==1) { /* first obs of a new strata */
      if (i>0) {
	  strata[j] = maxdeath; /* number of deaths in the strata */
...
for (i=0; i<nused; ) {
	...
	for (j=0; j<nvar; j++) { /* for each covariate */
	    ...
	    if (ndeath > 2) R_CheckUserInterrupt();
	    u[j] -= d1[j];
	    for (k=0; k<= j; k++) {  /* second derivative*/
		printf("ndeath= %d, j=%d, k=%d\n", ndeath, j, k);
