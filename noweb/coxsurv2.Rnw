...
a few years before. (2019: The survfitkm function has now moved back to C.) 
...
(That won't stop some u           

First, compute the baseline survival curves for each strata.  If the strata
was a factor produced output curves in that order, otherwise in sorted order.
...
survfitcoxph.fit <- function(y, x, wt, x2, risk, newrisk, strata, se.fit,
                              stype, ctype, varmat, oldid, cluster,
                              id, y2, strata2,
                              unlist=TRUE) {
