\subsubsection{Single state models}
%
% Second part of coxsurv.Rnw, broken in two to make it easier for me
%   to work with emacs.
Now, we're ready to do the main compuation.   
This part of the routine has had 3 major versions: the
initial code in C, a second version in R, and finally this version in
C again.  The last was due to the addition of robust variance, which
is computationally intensive.
The resulting C code is quite close to that for survfitKM
...
The setup and work is very much a reprise of survfitKM.             

<<survfitcoxph.fit>>=
survfitcoxph.fit <- function(y, x, wt, x2, risk, newrisk, strata, se.fit,
                              survtype, vartype, varmat, id, newid, 
                              y2, strata2,
                              unlist=TRUE) {
    ...
    if (length(xlev) ==1) {# only one group
        ...
        for (i in 1:ngroup) {
            ...
            #  We need to relabel them from 1 to "number of unique clusters in
            #  this curve for the C routine
            if (ncluster > 0) {
                c2 <- cluster[keep]
            ...
            if (ncluster > 0) 
                cfit[[i]] <- .Call(Csurvfitkm, ytemp, weights[keep], sort1 -1L, 
                                   sort2 -1L, type, c2 -1L,
                                   length(c.unique), position, influence)
            else cfit[[i]] <- .Call(Csurvfitkm, ytemp, weights[keep], 
                                    sort1 -1L, sort2 -1L, type,
                                    0L, 0L, position, influence)
        }
    }
...
Now, if \code{id} is missing we can simply apply the \code{expand} function
to each strata.
For the case with \code{id} not missing, we create a single survival
...
\begin{itemize}
  \item Do a separate sum at each death time.  The problem is for very large
    data sets.  For each death time the selection \code{who <- (start<t & stop>=t)}
