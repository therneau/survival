...
dim.survfitms <- function(x) {
    if (is.null(x$strata)) {
        if (is.matrix(x$prev)) c(1L, ncol(x$prev))
        else 1L
    }
    else {
        nr <- length(x$strata)
...
"[.survfitms" <- function(x, ..., drop=TRUE) {
    ...
    if (!is.null(x$influence)) {
        ...
        else if (is.list(x$influence)) x$influence <- x$influence[[1]]
...
The influence array is of the same shape as the \code{prev}, but with
...
the result.  
As with \code{prev} and \code{p0} a major nuisance for all this is
