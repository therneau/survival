...
or integer indices.
The drop argument is important when rows get subscripted such that
...
<<survsum-findrows>>=
    ...
    for (i in 1:nstrat) {
        ...
        cfun <- function(x, indx, init=0) {
            temp <- ifelse(indx==0, init, x[indx])
            diff(c(0, temp))
            }
 
        nevent[[i]] <- cfun(fit$n.event[who], indx1)
        nrisk[[i]]  <- c(0, fit$n.risk[who])[i+1]
  
        if (!is.null(fit$n.censor)) {
    	    n.censor[[i]] <- cfun(fit$n.censor[who], indx1)
    	    }
     }
