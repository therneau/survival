...
summary.survfit <- function(object, times, censored=FALSE, 
    ...
    if (length(indx1)==length(fit$time) && all(indx1 == seq(along=fit$time))) {
        ...
        for (j in c("n.risk", "n.event", "n.censor", "n.enter", "pstate",
        ...
        tfun <- function(x, init=0) {
             if (is.matrix(x)) 
                rbind(rep(init, ncol(x)), x)[1+indx1,,drop=FALSE]
             else c(init, x)[1 + indx1]
        }
        temp$surv <- tfun(temp$surv, 1)
        for (j in c("n.risk", "std.err", "cumhaz", "lower", "upper")) {
...
<<survsum-findrows>>=
    ...
    find2 <- function(x, vec, leftmost.closed=FALSE, lcont=TRUE) {
        if (lcont) findInterval(x, vec, leftmost.closed)
        else length(vec) - findInterval(-x, rev(-vec), leftmost.closed)
    }
    ...
    for (i in 1:nstrat) {
        ...
        ilist2[[i]] <- c(0, who)[1+ find2(ptimes, stime, lcont=FALSE)]
    ...
    cfun <- function(x, init=0) {  #cumulative counts over a time interval
        ...
        if (is.matrix(x)) {
            for (i in 1:nstrat) {
                ...
                j  <- ilist3[[i]]
                tlist[[i]] <- apply(rbind(0,0, x2), 2, function(z) {
                    (diff(cumsum(z)))[1+j]})
            }
            ...
            for (i in 1:nstrat) {
                x2 <- x[stemp==i]
                tlist[[i]] <- diff(cumsum(c(0,0,x2)))[1 + ilist3[[i]]]
            }
            unlist(tlist)
        }
    }
...
summary.survfitms <- function(object, times, censored=FALSE, 
    ...
    if (length(indx1)==length(fit$time) && all(indx1 == seq(along=fit$time))) {
        ...
        temp$time <- temp$time[indx1]/scale
        for (j in c("n.risk", "n.event", "n.censor", "n.enter", "pstate",
                    "pstate", "std.err", "cumhaz", "lower", "upper")) {
            zed <- temp[[j]]
            if (!is.null(zed)) {
                if (is.matrix(zed)) temp[[j]] <- zed[indx1,,drop=FALSE]
                else temp[[j]] <- zed[indx1]
        ...
        temp$pstate <- tfun(temp$pstate, 0)
        # fix up the initial states
        if (any(indx1==0)) {
            if (nstrat==1) temp$sstate[indx1==0,] <- temp$p0
            else {
                ninit <- sapply(ilist1, function(x) sum(x==0))
                zz <- rep(1:nstrat, ninit)
                temp$pstate[indx1==0,] <- temp$p0[zz,]
            }
        }
        for (j in c("n.risk", "std.err", "cumhaz", "lower", "upper")) {
            if (!is.null(temp[[j]])) temp[[j]] <- tfun(temp[[j]])
