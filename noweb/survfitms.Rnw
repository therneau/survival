...
"[.survfitms" <- function(x, ..., drop=FALSE) {
    ...
    # If the first dimension is 1 (there are no strata), we let the
    #  use ignore that dimension
    if (is.null(x$strata) && (ndots+1) == length(dd)) {
        k <- j; j<- i; i <- NULL
        ndots <- ndots +1
    }
    ...
    if (all(dtype !=2)) {  # j indexes the states
        ...
        else newx$newdata <- x$newdata[j,]  #newdata is a data frame
