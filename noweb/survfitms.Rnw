...
"[.survfitms" <- function(x, ..., drop=FALSE) {
    ...
    if (ndots==1 && length(dd2) > 1) {
        ...
        if (length(dd[dd>1]) ==1) {
            ...
            newx$cumhaz <- newx$std.chaz <- newx$influence.c <- NULL
            newx$transitions <- newx$states <- newx$newdata <- NULL

            # what strata and columns do I need?
            itemp <- matrix(1:prod(dd), nrow=dd[1])
            jj <- (cols(itemp))[i]    # columns
            ii <- (rows(itemp))[i]    # this is now the strata id
    ...
    if (length(dd) ==2) {  # j indexes the states
        ...
        for (z in c("pstate", "std.err", "upper", "lower"))
            if (!is.null(x[[z]])) newx[[z]] <- (x[[z]])[irow,j, drop=drop2]
        if (!is.null(x$influence.p)) {
            if (is.list(x$influence.p)) {
                if (length(i)==1) newx$influence.p <- x$influence.p[[i]]
                else newx$influence.p <- lapply(x$influence.p[i],
                                     function(x) x[,,j, drop= drop])
                }
            else newx$influence.p <- x$influence.p[,,j, drop=drop]
        }
        ...
        if (length(j)== nstate && all(j == seq.int(nstate))) {
            ...
            if (!is.null(x$influence.c)) {
                if (is.list(x$influence.c)) {
                    newx$influence.c <- x$influence.c[i]
                    if (length(i)==1 && drop) 
                        newx$influence.c <- x$influence.c[[i]]
                }
                else newx$influence.c <- x$influence.c
            }
        }
        else {
            newx$cumhaz <- newx$std.chaz <- newx$influence.c <- NULL
        ...
        if (!is.null(x$influence.p)) {
            if (is.list(x$influence.p)) {
                if (length(i)==1) 
                    newx$influence.p <- (x$influence.p[[i]])[,,jj, drop=drop]
                else newx$influence.p <- lapply(x$influence.p[i],
                                     function(x) x[,,jj, drop= drop])
                }
            else newx$influence.p <- x$influence.p[,,jj, drop=drop]
        }
