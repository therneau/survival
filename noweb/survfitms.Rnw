...
"[.survfitms" <- function(x, ..., drop=FALSE) {
    ...
    if (ndots==1 && length(dd2) > 1) {
        ...
        if (length(dd[dd>1]) ==1) {
            ...
            for (z in c("pstate", "std.err", "upper", "lower", "influence.p"))
    ...
    if (length(dd) ==2) {  # j points at states
        ...
        for (z in c("pstate", "std.err", 
                    "upper", "lower", "influence.p"))
            if (!is.null(x[[z]])) newx[[z]] <- (x[[z]])[irow,j, drop=drop2]
  
        if (length(j)== nstate && all(j == seq.int(nstate))) {
            # user kept all the states
            newx$states <- x$states
            for (z in c("cumhaz", "std.chaz", "influence.c"))
