...
amount of code.  
Both are included here in order to have common source for the largest
...
summary.survfit <- function(object, times, censored=FALSE, 
    ...
    if (all(indx1 == seq(along=fit$time))) {
        ...
        for (j in c("n.risk", "n.event", "n.censor", "n.enter",
            ...
            zed <- temp[[j]]
            if (!is.null(zed)) {
                if (is.matrix(zed)) temp[[j]] <- zed[j,,drop=FALSE]
                else temp[[j]] <- zed[j]
            }
        }
        ...
        temp <- list(n=object$n, time=newtime/scale, 
        ...
        if (is.matrix(fit$surv)) {
            temp$surv <- fit$surv[indx1,,drop=FALSE]
            if (!is.null(std.err)) temp$std.err <-fit$std.err[indx1,,drop=FALSE]
            if (!is.null(fit$lower)) {
                temp$lower <- fit$lower[indx1,,drop=FALSE]
                temp$upper <- fit$upper[indx1,,drop=FALSE]
            }
        }
        else {
            temp$surv <- fit$surv[indx1]
            if (!is.null(std.err)) temp$std.err <- fit$std.err[indx1]
            if (!is.null(fit$lower)) {
                temp$lower <- fit$lower[indx1]
                temp$upper <- fit$upper[indx1]
            }
        }
        if (!is.null(fit$strata)) 
                temp$strata <- strata
...
summary.survfitms <- function(object, times, censored=FALSE, 
    ...
    if (all(indx1 == seq(along=fit$time))) {
        ...
        if (is.matrix(fit$prev)) {
            temp$prev <- fit$prev[indx1,,drop=FALSE]
            temp$cumhaz <- fit$cumhaz[,,indx1, drop=FALSE]
            if (!is.null(std.err)) temp$std.err <-fit$std.err[indx1,,drop=FALSE]
            if (!is.null(fit$lower)) {
                temp$lower <- fit$lower[indx1,,drop=FALSE]
            ...
            temp$prev <- fit$prev[indx1]
            temp$cumhaz <- fit$cumhaz[indx1]
            if (!is.null(std.err)) temp$std.err <- fit$std.err[indx1]
            if (!is.null(fit$lower)) {
                temp$lower <- fit$lower[indx1]
                temp$upper <- fit$upper[indx1]
