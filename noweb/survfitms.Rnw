...
summary.survfit <- function(object, times, censored=FALSE, 
    ...
    delta <- function(x, indx) {  # sums between chosen times
        if (!is.null(x) && length(indx) >0) {
            fx <- function(x, indx) diff(c(0, cumsum(x[indx])))
    ...
    # select, pasting a value on the front for indices of 0 
    ssub <- function(x, indx, init=0) {  #select an object and index
        if (!is.null(x) && length(indx)>0) {
            if (is.matrix(x)) rbind(init, x)[indx+1,,drop=FALSE]
            else c(init, x)[indx+1]
        }
        else NULL
    }
    # select, pasting a value on the back for longer indices (for n.risk)
    ssub2 <- function(x, indx, end=0) {
        if (!is.null(x) && length(indx)>0) {
            if (is.matrix(x)) rbind(x, end)[indx,,drop=FALSE]
            else c(x, end)[indx]
        }
        else NULL
    }
    ...
    # There are 4 cases: 'times' argument is absent or present,
    #  strata is absent or present.  It is easiest to do them
    #  separately.
    if (missing(times)) {
        # There will be no indices before, after, or in-between observed
        # times, so ssub and ssub2 are identical
        if (!censor) {
            indx1 <- which(rowSums(as.matrix(fit$n.event)) >0)
            for (i in c("time","n.risk", "n.event", "surv", "std.err", 
                        "upper", "lower"))
                fit[[i]] <- ssub[fit[[i]], indx1]
            if (is.null(fit$strata)) {
                for (i in c("n.enter", "n.censor"))
                    if (!is.null(fit[[i]]))
                        fit[[i]] <- delta(fit[[i]], indx1)
                }
                else {
                    for (i in c("n.enter", "n.censor")) {
                        if (!is.null(fit[[i]]))
                            fit[[i]] <- unlist(sapply(1:strat, function(x) 
                               delta(fit$[[i]][sindx==i], indx1[sindx==i])))
                    }
                    fit$strata[] <- as.vector(table(sindx))  #retain names
                }
            }
        }
    }  #if missing(times) and censor=TRUE, fit is ok as it is

    else { # This is the harder case
        <<survfitms-findrows>>
        if (is.null(fit$strata)) {
            ftemp <- findrows(fit, 1:length(fit$time), extend, times/scale)
            fit$time <- ftemp$time
            for (i in c("surv", "upper", "lower"))
                     fit[[i]] <- ssub(fit[[i]], ftemp$indx1, 1)
            fit$std.err <- ssub(fit$std.err, ftemp$indx1, 0)
            fit$n.risk <- ssub2(fit$n.risk,  ftemp$indx2)
            for (i in c("n.event", "n.censor", "n.enter"))
                fit[[i]] <- delta(fit[[i]], ftemp$indx1)
         }
        else {
            # do the curves one at a time, unlist it at the end
            ltemp <- vector("list", nstrat)
            for (j in 1:nstrat) {
                temp <- findrows(fit, which(stemp==j), extend, 
                                       times/scale)
                temp$time <- ftemp$time
                for (i in c("surv", "upper", "lower"))
                     temp[[i]] <- ssub(temp[[i]], ftemp$indx1, 1)
                temp$std.err <- ssub(temp$std.err, ftemp$indx1, 0)
                temp$n.risk <- ssub2(temp$n.risk,  ftemp$indx2)
                for (i in c("n.event", "n.censor", "n.enter"))
                    temp[[i]] <- delta(temp[[i]], ftemp$indx1)
                ltemp[[j]] <- temp
            }
            
            for (i in names(ltemp[[1]])) {
                temp <- lapply(ltemp, function(x) x[[i]])
                if (is.matrix(fit[[i]]))
                    fit[[i]] <- do.call("rbind", temp)
                else fit[[i]] <- unlist(temp)
            }
...
the survival is the value at time 1 (indx1), the number at risk the
number at time 2 (indx2).

<<survfitms-findrows>>=
findrow <- function(fit, who, times, extend) {
    ...
    if (is.null(fit$start.time)) mintime <- min(fit$time[who], 0)
    else                         mintime <- fit$start.time
    ptimes <- times[times >= mintime]

    if (!extend) {
        maxtime <- max(fit$time[who])
        ptimes <- ptimes[ptimes <= maxtime]
    }
        
    j <- find2(ptimes, fit$time[who]) 
    indx1 <- c(0, who)[1+j]
    indx2 <- c(0, who)[1+ find2(ptimes, fit$time[who], left.open=TRUE)]

    list(indx1= indx1, indx2= indx2, time=ptimes)
}
@ 
