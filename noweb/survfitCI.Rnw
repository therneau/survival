...
<<survfitCI>>=
    ...
    if (ncol(Y)==2) {  # 1 transition per subject
        ...
        entry <- rep(min(-1, 2*min(Y[,1])-1), n)  #dummy entry time that is < any event
        for (i in levels(X)) {
            indx <- which(X==i)
            curves[[i]] <- docurve1(entry[indx], Y[indx,1], status[indx], istate[indx],
...
if (se.fit) {
    ...
    if (conf.type=='plain') {
        temp <- zval* std.err
        kfit <- c(kfit, list(lower =pmax(kfit$P-temp, 0), upper=pmin(kfit$P+temp, 1),
    			 conf.type='plain', conf.int=conf.int))
        }

    if (conf.type=='log') {
        #avoid some "log(0)" messages
        xx <- ifelse(kfit$P==1, 1, 1- kfit$P)  
...
class(kfit) <- "survfitms"
kfit
@

The print function is almost identical to that in 
