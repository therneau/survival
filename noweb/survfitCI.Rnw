...
#        warning("Only the infinetesimal jackknife error is supported for CI curves")
    ...
    if (missing(istate) || is.null(istate)) {
        ...
        if (influence) 
            stop("concurrent use of the istate and influence options is not yet supported")
...
docurve2 <- function(entry, etime, status, istate, wt, states, id, se.fit) {
    # entry and etime will be integers, due to a prior normalizetime call
    ...
    if (influence) {
        temp <-  array(fit$influence, 
                       dim=c(length(uid), nstate, length(timeset)),
