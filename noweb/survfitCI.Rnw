...
if (length(curves) ==1) {
    keep <- c("n", "time", "n.risk", "n.event", "n.censor", "pstate",
              "p0", "cumhaz", "influence.pstate", "influence.rmts")
    ...
    if (influence>=4)   kfit$influence.rmst <-
        lapply(curves, function(x) x$influence.rmts)
}                         
...
docurve2 <- function(entry, etime, status, istate, wt, states, id, 
    ...
    if (influence%%2 ==1) {
        temp <-  array(fit$influence.pstate, 
                       dim=c(length(uid), nstate, j),
                       dimnames=list(uid, NULL, NULL))
        out$influence.pstate <- aperm(temp, c(1,3,2))
    }
    if (influence >=4) {
        temp <-  array(fit$influence.rmts, 
                       dim=c(length(uid), nstate, j),
                       dimnames=list(uid, NULL, NULL))
        out$influence.rmts <- aperm(temp, c(1,3,2))
    }
    out
}
@
