...
For this reason I first used the label \emph{current prevalence} estimate, 
...
\begin{align}
  U_i(t) &= \frac{partial p(t)}{\partial w_i} \nonumber \\
         ...
         &= U_i(t-) H(t) +  p(t-) \frac{\partial H(t}{\partial w_i} 
           \label{ci}
\end{align}  
The first term of \mathref{ci} collapses to ordinary matrix multiplication. 
...
Using equation \mathref{H}, the derivative of row $j$
