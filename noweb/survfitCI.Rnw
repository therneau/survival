...
survfitCI <- function(X, Y, weights, id, istate, cluster, 
    ...
    if (missing(istate) || is.null(istate))
        mcheck <- multicheck2(Y, id)  
    else mcheck <- multicheck2(Y, id, istate)
...
if (length(curves) ==1) {
    ...
    kfit$cumhaz <- t(kfit$cumhaz[ckeep,])
    ...
    # rearrange the cumulative hazard to be in time order, which columns
    #  for each transition
    kfit$cumhaz <- do.call(rbind, lapply(curves, function(x)
        t(x$cumhaz[ckeep,])))
    colnames(kfit$cumhaz) <- names(ckeep)
 
    if (influence) kfit$influence.p <- lapply(curves, function(x) x$influence.p)
...
docurve2 <- function(entry, etime, status, istate, wt, states, id, 
    ...
    if (influence) {
        ...
        out$influence.p <- aperm(temp, c(1,3,2))
    }
    out
}
@
