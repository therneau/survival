...
it come from (first event time -0), smallest \code{etime} with status $>0$.
...
If the first event doesn't happen for a while
and subjects are dribbling in this is subject to debate.
Que sera sera.
<<survfitCI-compute>>=
docurve2 <- function(entry, etime, status, istate, wt, states, id, se.fit) {
    ...
    p0 <- tapply(wt[at.zero], factor(istate[at.zero], levels=states), sum) /
          wtsum
    p0 <- ifelse(is.na(p0), 0, p0)  # if not in at.zero, tapply gives NA
    # initial leverage matrix
    if (se.fit) {
        nid <- length(uid)
        i0 <- matrix(0., nid, nstate)
        if (all(p0 <1)) {  #actually have to compute it
            i0[cbind(index[at.zero], istate[at.zero])] <- 1/wtsum
            i0 <- i0 - rep(p0/wtsum, each=nid)
        }
    }
    ...
    n.event <- table(etime, factor(status,c(0,states)))[,-1, drop=FALSE]
    n0 <- sum(at.zero)
    if (se.fit) 
        out <- list(time= timeset, 
             pstate = fit$p, std=fit$std,
             n.risk = fit$nrisk,
             n.event= fit$nevent,
             n.censor=fit$ncensor,
             cumhaz=array(fit$cumhaz, dim=c(nstate,nstate, length(timeset))))
    else out <- list(time= timeset, 
             pstate = fit$p,
             n.risk = fit$nrisk, 
             n.event = fit$nevent, 
             n.censor= fit$ncensor, 
