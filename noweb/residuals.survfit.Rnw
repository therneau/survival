...
residuals.survfit <- function(object, times, 
    ...
    type <- match.arg(casefold(type), c("surv", "pstate", "cumhaz",
                                        "rmst", "rmts"))
    type.int <- c(1,1,3,2,2)[match(type, c("surv", "pstate", "cumhaz",
                                           "rmst", "rmts"))]
...
The derivative form \eqref{Ukm} is based on expanding the recursive definition
of the KM, and \eqref{Ukm2} on using the second definition and the product
...
rsurvpart2 <- function(Y, X, casewt, istate, times, cluster, type, fit) {
    ...
    if (type==2) stop("type auc not yet coded for multi-state")
...
\begin{equation}
  \frac{partial H(d_5)}{\partial w_i} = 
    \right[(0,-1, 1, 0)/r(d_4) H(d_5)H(d_6)H(d_7)H(d_8)H(d_9) \right] -
    \right[C_{2.}(d_4)/r(d_4)H(d_5)H(d_6)H(d_7)H(d_8)H(d_9) \right]
\end{equation}
