...
If there are multiple curves fitrow will be a list, with ff=unlist(fitrow).
The number of curves is normally modest.

One ``watch out'' is that there are 3 separate indices of interest. An
index into the surival curves that points at the event times, an index
to the event number (first, second, third, etc.) and an index to the output
times.  Most intermediate computations give a vector with one element per event
time, and they need the second. 
The output matrix has one row per subject, one col per reporting (output) time. 

For many of the compuations a subject's value at a given time t will be a sum
up to min(t, last fu for the subject).  A primary trick in the code below is
to create two matrices of indices, each matching the output (one row per
subject, one col per output time).
The \code{tindex} array carries, in each column, the index of the last applicable
death for this output time.  This might be 0 -- users sometimes ask for an
output value before the first event.
The \code{yindex} array carries, in each row, the index of the last applicable
death for this subject.  This also might be zero, for a subject censored before
the first event.
 Then \code{pmin(tindex, yindex)} is the last event time for
that subject/output time pair; an index to the event number.
The vector \code{ff} contains the indices of all the event times in the
fit object, e.g., \code{fit\$n.risk[ff]} is the vector of $n$ at the event
times and \code{(/fit\$n.risk[ff])[pmin(tindex, yindex)]} is a common idiom. %$
...
Type = RMTS is the area under $S(t)$ up to a given time. 
The AUC at any time point is $\sum c_k S_k$, where $c_k$ are the widths of
the rectangles.
Let $A(s,t)$ be the area under the curve from  $s$ to $t$, with the convention
that $A=0$ if $s \ge t$.
\begin{align}
  \frac{\partial S(t)}{\partial w_i} &= 
   -\partial{H(t)}{\partial w_i} S(t) \nonumber\\
    &= S(t)\sum_{d_j \le t} dN_i(d_j)/r(d_j) - Y_i(d_j)h(d_j)/r(d_j) 
   \label{rsurv1}\\
  \frac{\partial A(0,t)}{\partial w_i} &= 
     (\sum_{d_j \le t} c_j \frac{\partial S(d_j)}{\partial w_i} \nonumber\\
  &=  \sum_{d_j \le t} -\partial{H(t_j)}\partial{w_i} A(t_j, s \wedge t_{j+1})
    \label{rsurv2} \\
  &=   \sum_{d_j \le t} -\left(dN_i(d_j)/r(d_j) - Y_i(d_j)h(d_j)/r(d_j)\right)
        A(d_j, t)  \label{rsurv3} \\
  &= \sum_{d_j \le t} \left(dN_i(d_j)/r(d_j) - Y_i(d_j)h(d_j)/r(d_j)\right)
        \left[ A(0,d_j) - A(0,t) \right] \nonumber \\
  &= \left[\sum_{d_j \le t} -\left(dN_i(d_j)/r(d_j) - Y_i(d_j)h(d_j)/r(d_j)\right)
          A(0, d_j) \right] - A(0,t)\partial{H(t)}{\partial w_i}
        \label{rsurv4}
\end{align}

The problem with equation \eqref{rsurv2} is that it requires the influence of $H$
at all the death time/subject pairs, which is exactly the computation we want
to avoid.
Substitution of the expansion in \eqref{rsurv1} for $dH$ and rearranging the
terms leads to \eqref{rsurv3}.
Replace the partial in \eqref{rsurv2} with the expansion in \eqref{rsurv1}
and reverse the order if the sums to get \eqref{rsurv3}.

Then break the compuation in into three parts:
\begin{align*}
  1 &=\sum_{d_k \le t} \frac{ dN_i(d_k)}{r(d_k)} A(0,d_k)
  2 &=\sum_{d_k \le t} \frac{Y_i(d_k)h(d_k)}{r(d_k)} A(0, d_k) \\
  3 & A(0,t) \partial H(t)/\partial w_i
\end{align*}

Since there is only one death for a
given subject, pointed to by yindex, the first term is a single value for
each subject/reporting time pair.
The second term requires a running sum lsum2 exactly parallel to lsum.
The third term is the easiest since the partial of $H$ is already available
as the matrix $D$.  We simply need the constant $A(t)$ for each reporting
time.

Not all survival curves start at 0.  However, 
up to the first death time $S$ is a constant value of 1, so the derivatives of
S and the AUC are zero up to that point.  Thus we can use 
$A(s,t) = A(d_1, t) - A(d_1,s)$ in the formula instead.
...
\begin{align}
    S(t) &= S(t-) [1 - h(t)]  \label{recursive}\\
         &= \prod_{d_j \le t} [1- h(d_j)] \label{direct}\\
    U_i(t) &= \frac{\partial S(t)}{\partial w_i}  \nonumber\\
           &= U_i(t-) [1- h(t)] - 
               S(t-)\frac{\partial h(t)}{\partial w_i} \label{sUkm} \\
           &= \sum_j \left(\frac{\partial [1- h(d_j)]}{\partial w_i}
                   \left[\prod_{d_k\le t; k \ne j} (1 - h(d_k)) \right]\right)
                 \label{sUkm2} \\
            &= S(t) \left[-\sum_{d_j\le t} \frac{dN_i(d_j) - Y_i(d_j)h(d_j)}{r(d_j)}
                   \frac{1}{1-h(d_j)} \right] \label{ajfactor}
\end{align}
...
of the KM \eqref{recursive}, and \eqref{sUkm2} on using the direct
definition \eqref{direct} along with the product
rule for derivatives: each term is replaced, one by one, with it's derivative.
The final step is to notice that the right hand product in \eqref{sUkm2}
is $S(t)$ less a single term: multiply by $(1-h_j)/(1-h_j)$ and gather the
numerator term into $S(t)$.
which allows all of the $1-h$ term to be gathered together as $S(t)$.
Our algorithm is based on this last equation \eqref{ajfactor}.
...
This means that the code for AUC is nearly identical to what came before.
