...
Efron approximation in a Cox model or the ctype=2 argument for \code{survfit}
is difficult, i.e., it is not clear \emph{what} should be done, much less how.
Thus the ctype argument is not present.  Currently one \code{stype=1} is
supported.
This makes the code somewhat simpler, but is more than offset by the 
multi-state nature.
For the cumulative hazard, however, the code is very similar to the simple
Nelson-Aalen case, so we start with the same indexing matrices as are used
in the \code{rsurvpart1} routine.
...
\begin{align}
  p(t) &= p(t-) H(t) \\
  \frac{partial p(t)}{\partial w_i} &= \fract{\partial p(t-)}{\partial w_i} H(t)
...
Each individual subject $i$ it has a subject-specific
nstate * nstate $dH$ matrix for that subject, and the matrix product adds to 
that particular row of $U$.
...
For any given observtion the derivative matrix will be 0 for all but the
row $j$ corresponding to the initial state, and that row will be zero if there
are no transitions out of $j$ at this time point.
Otherwise that row will have
$(1- H_{jk}(t))/r_j(t)$ in one position and $-H_{jk}(t)/r_j(t)$ in all others,
the '1' appears at the new state for an observation that transitions and the 
current state for others.

By rearranging terms \eqref{ajresidx} can be re-written as line 1 below,
where $d$ are the unique event times.
This is can also be seen as the product rule for derivatives applied to
$p(t) = p(0) H(d_1) H(d_2) \ldots$.
If we could treat matrices as scalars this can be factored in the form of
the second line.
\begin{align*}
  U_i(t) &= \sum_{d_j \le t} \left( p(d_j-) \frac{\partial H(d_j)}{\partial w_i} 
    \prod_{d_k <d_j \le t} H(d_k) \right)\\
  &= p(t) \left( \sum_j \frac{\partial H(d_j)}{\partial w_i} \frac{1}{H(d_j)} 
  \right)
\end{equation*}
This was used in the KM case to avoid any $O(nd)$ computations 
where $n$ = number of observations and $d$ = number of events.
