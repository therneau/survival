...
<<tmerge-setup>>= 
tmerge.control <- function(id="id", tstart="tstart", tstop="tstop",  
                           defer =0, nadrop=TRUE) {
    if (length(defer) !=1 || !is.numeric(defer) || defer <0)
        stop("defer option must be a non-negative number")
    if (length(id) != 1 || make.names(id) != id)
        stop("id option must be a valid variable name")
    if (!is.null(tstart) && 
        (length(tstart) !=1 || make.names(tstart) != tstart))
        stop("tstart option must be NULL or a valid variable name")
    if (length(tstop) != 1 || make.names(tstop) != tstop)
   ...
   list(id=id, tstart=tstart, tstop=tstop, defer=defer, nadrop=nadrop)
}
...
if (length(tname)==0) { 
    ...
    indx <- match(c(topt$id, topt$tstop, topt$tstart), names(data1), nomatch=0)
...
for (ii in seq(along.with=args)) {
   ...
   if (!is.null(argi$value)) {
       if (length(argi$value) != length(saveid))
            stop("argument", argname[ii], "is not the same length as id")
        if (topt$nodrop) keep <- !(is.na(etime) || is.na(argi$value))
...
if (argclass[ii] %in% c("event", "cumevent")) {
    ...
    if !(argname[[ii]] %in% tevent) {
        tevent <- c(tevent, argname[[i]])
        if (!is.null(argcens)) tcens[[argname[i]]] <- argcens
        else if (is.null(argi$value) || is.numeric(ylev))
            tcens[[argname[i]]] <- 0
        else if (is.logical(ylev)) tcens[[argname[i]]] <- FALSE
        else if (is.factor(ylev)) tcens[[argname[i]]] <- levels(ylev)[1]
