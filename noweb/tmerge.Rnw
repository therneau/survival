...
tmerge <- function(data1, data2, id, ..., tstart, tstop, options) {
    ...
    assign("event", function(time, value=NULL, censor=NULL) {
        ...
        class(x) <-"event"; x},
           envir=new)
    assign("cumevent", function(time, value=NULL, levels=NULL) {
        x <- list(time=time, value=value, censor=censor); 
        class(x) <-"cumevent"; x},
           envir=new)
...
<<tmerge-setup>>= 
tmerge.control <- function(idname="id", tname1="tstart", tname2="tstop",  
                           delay =0, nadrop=TRUE, ...) {
    extras <- list(...)
    if (length(extras) > 0) 
        stop("unrecognized option:", names(extras))
    if (length(idname) != 1 || make.names(idname) != idname)
        stop("idname option must be a valid variable name")
    if (!is.null(tname1) && 
        (length(tname1) !=1 || make.names(tname1) != tname1))
        stop("tstart option must be NULL or a valid variable name")
    if (length(tname2) != 1 || make.names(tname2) != tname2)
    ...
    if (length(nadrop) !=1 || ! is.logical(nadrop))
        stop("nadrop option must be TRUE or FALSE")
   list(idname=idname, tname1=tname1, tname2=tname2, delay=delay, nadrop=nadrop)
}
...
\begin{enumerate}
  \item Adding a time range: tstop comes from data2, optional tstart, and the
    ...
    or that they have the same id string.
...
if (firstcall) {
    ...
    indx <- match(c(topt$idname, topt$tname1, topt$tname2), names(data1), 
                  nomatch=0)
    if (any(indx>0) && FALSE) {  # warning currently turned off. Be chatty?
        overwrite <- c(topt$idname, topt$tname1, topt$tname2)[indx]
        warning("overwriting data1 variables", overwrite)
        }
    ...
    if (missing(tstop)) { # case 2
        ...
        tstop <- args[[1]][[1]]
        if (nrow(data1) != length(id))
            stop("no tstop argument and nrow(data1) != length(id)")
    }
    else { # case 1
        # Do data set 1 and 2 both have the id variable?  This is the
        #  less usual case of separate timeline and baseline data sets (for
        #  which the routine was originally written).
        if (indx[1] > 0) {
            baseid <- data1[[indx[1]]]
            if (all(baseid == id)) newdata <- data1
            else {
                if (any(duplicated(baseid)))
                    stop("data1 and data2 both have the id variable, and data1 has duplicates")
        ...
        else if (nrow(data1) != length(id))
            stop("on the first call data1 and data2 must have the same number of rows or must share an id variable")
        else newdata[topt$idname] <- id   #assume the same order
    }
    ...
    newdata[[topt$idname]] <- id
    newdata[[topt$tname1]] <- tstart
    newdata[[topt$tname2]] <- tstop
...
If the nadrop option is true, we do the same for the variable argument
of the call.
<<tmerge-addvar>>=
saveid <- id
for (ii in seq(along.with=args)) {
    ...
    dstart <- newdata[[topt$tname1]]
    dstop  <- newdata[[topt$tname2]]
    argcen <- argi$censor
   ...
   if (!is.null(argi$value)) {
       if (length(argi$value) != length(saveid))
            stop("argument", argname[ii], "is not the same length as id")
        if (topt$nadrop) keep <- !(is.na(etime) | is.na(argi$value))
...
if (n4 > 0) {
    ...
    newdata[[topt$tname1]] <- dstart
    newdata[[topt$tname2]]  <- dstop
...
<<tmerge-finish>>=
attr(newdata, "tname") <- topt[c("idname", "tname1", "tname2")]
