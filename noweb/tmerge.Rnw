...
\begin{itemize}
  ...
  \item id: id variable in the new data
...
tmerge.control <- function(idname="id", tname1="tstart", tname2="tstop",  
    ...
    if (length(defer) !=1 || !is.numeric(defer) || defer < 0)
        stop("defer option must be a number >= 0")
    if (length(nadrop) !=1 || ! is.logical(nadrop))
        stop("nadrop option must be TRUE or FALSE")
   list(idname=idname, tname1=tname1, tname2=tname2, defer=defer, nadrop=nadrop)
}
...
range of legal times is set.  If \code{tname} is present then it is not
a first call.
There are 3 cases:
\begin{enumerate}
  \item The id and tstop variables are both found in data1, and 
    ...
    to note the variable names in the topt list, check that the data looks
...
if (length(tname)==0) { 
    ...
    indx <- match(c(topt$idname, topt$tname2, topt$tname1), names(data1), nomatch=0)
    if (all(indx[1:2] >0) && missing(tstop)) {
        # case 1 above, just some data checks
        if (indx[3]==0) newdata[[topt$tname1]] <- 0
        if (!is.numeric(newdata[[topt$tname1]]) || 
            !is.numeric(newdata[[topt$tname2]]))
            stop("start and end variables must be numeric")
        if (any(newdata[[topt$tname1]]  >= newdata[[topt$tname2]])) 
        ...
        # If tstart was not supplied, we need to fix up our "0" created above
        baseid <- data1[[topt$idname]]
        ...
        newdata[indx2, topt$tname1] <- tstart
        newdata[indx2, topt$tname2]  <- tstop
    }
}
...
for (ii in seq(along.with=args)) {
   ...
   if (!is.null(argi$value)) {
    ...
    defer <- rep(0., nrow(newdata))  # how much to move each obs
    if (topt$defer >0 && length(tevent) && 
        argclass[ii] %in% c("tdc", "cumtdc")) {
        for (ename in tevent) {
            temp <- newdata[[ename]]
            defer[temp != tcens[[ename]]] <- topt$defer # all uncensored
         }
    }
