...
<<concordance>>=
concordance <- function(fit, ...) 
    UseMethod("concordance")

concordance.formula <- function(fit, data,
    ...
    temp$formula <- if(missing(data)) terms(fit, special)
                    else              terms(fit, special, data=data)
...
print.concordance <- function(x, digits= max(1L, getOption("digits") - 3L), 
    ...
    if (length(x$concordance) > 1) {
        ...
        cat("\n")
             ...
             format(sqrt(x$var[1]), digits=digits), '\n', sep='')
...
<<concordancefit>>=
concordance.fit <- function(y, x, strata, weights, ymin, ymax, timewt,
                            group, influence=0, ranks=FALSE, reverse=FALSE) {
    ...
    docount <- function(y, risk, wts, timeopt= 'n') {
        ...
        if (ncol(y)==2) { 
            ...
            sort.stop  <- order(-y[,2], y[,3]) -1L   #order by endpoint
    ...
    if (influence >=2) {
        if (!missing(group) && length(group) > 0) 
            rval$influence <- rowsum(imat, group)
...
have the model frame by default, but that can be turned off.

<<concordance>>=
concordance.lm <- function(formula, ...) {
    Call <- match.call()
    if (!is.null(formula$y)) Y <- formula$y
    else if (!is.null(formula$model)) Y <- model.response(formula$model)
    temp <- concordance(Y ~ formula$fitted.values, ...)
    temp$call <- Call
    temp
}
concordance.survreg <- function(formula, ...) {
    Call <- match.call()
    if (!is.null(formula$y)) Y <- formula$y
    else if (!is.null(formula$model)) Y <- model.response(formula$model)
    temp <- concordance(Y ~ formula$linear.predictors, ...)
    temp$call <- Call
    temp
}

concordance.coxph <- function(fit, ..., newdata, ymin=NULL, ymax=NULL, 
                               timewt=c("n", "S", "S/G", "n/G", "n/G2"),
                               influence=0, ranks=FALSE, reverse=TRUE, group) {
    Call <- match.call()
    if (missing(group)) group <- NULL
    timewt <- match.arg(timewt)

    # four cases: newdata no/yes * multiple models no/yes
    fits <- list(fit, ...)
    nfit <- length(fits)
    iscox <- sapply(fits, function(x) inherits(x, "coxph"))
    if (!all(iscox)) stop("all fits must be coxph objects")
    
    need.wt <-  any(sapply(fits, function(x) !is.null(x$call$weights)))
    # This will be called to do the actual fit
    # In the simple case of a single model, no new data, no strata, cluster, or
    #  weights, then model.frame won't need to be called.
    dofit <- function(fit, newdata, inf=1) {
        Terms <- terms(fit)
        if (!missing(newdata)) mf <- model.frame(fit, data=newdata)
        else if (need.wt) mf <- model.frame(fit)
        else mf <- NULL
       ...
       if (!is.null(specials$strata)) {
            if (is.null(mf)) mf <- model.frame(fit)
        ...
        if (length(group) ==0) {
            if (!is.null(specials$cluster)) {
                ...
                tempc <- untangle.specials(Terms, 'cluster', 1:10)
                ord <- attr(Terms, 'order')[tempc$terms]
                group <- strata(mf[,tempc$vars], shortlabel=TRUE) 
            }
            else if (!is.null(fit$call$id)) {
                if (is.null(mf)) mf <- model.frame(fit)
                group <- model.extract(mf, "id")
            }
        }
       
        if (!is.null(mf)) {
            Y <- model.response(mf)
            if (is.null(fit$mf)) fit$mf <- mf  #don't make predict recreate it
        }
        else if (!is.null(fit$y)) Y <- fit$y
        else Y <- model.response(model.frame(fit))
        X <- predict(fit, type="lp")

        temp <- concordance.fit(Y, X, strata.keep, weights, ymin, ymax, timewt,
                        group, influence=inf)
        if (need.wt) temp$weights <- weights
        temp
    }

    if (nfit ==1) {
        if (missing(newdata))  rval <- dofit(fit, inf=influence)
        else rval <- dofit(fit, newdata, inf=influence)
    }
    else {
        if (influence==2) inf <-3 else inf <- 1
        if (!missing(newdata)) 
             flist <- lapply(fits, dofit, newdata, inf=inf)
        else flist <- lapply(fits, dofit, inf = inf)

        # all models have to use the exact same n, weights, and grouping
        for (i in 2:nfit) {
            if (!identical(flist[[1]]$n, flist[[i]]$n))
                stop("models must have identical sample size")
            if (!identical(flist[[1]]$weights, flist[[i]]$weights))
                stop("models must use identical case weights")
        ...
        dfbeta <- t(sapply(flist, function(x) x$dfbeta))
            
        mname <- c(deparse(substitute(fit)), deparse(substitute(...)))
        names(concordance) <- mname
        rownames(count) <- mname

        wt <- flist[[1]]$weights
        if (is.null(wt)) vmat <- tcrossprod(dfbeta)
