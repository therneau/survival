...
for (i=0; i<n;) {
	...
	ndeath=0; temp =0; dwt=0;
	   ...
	   for (k=0; k<3; k++) count[k] += wt[j]* wsum[k];
	}
	...
	for (j=i; j< ndeath+i; j++) {
        ...
        for(j=0; j<ndeath; j++) {
            walkup(nwt, twt, x[i], wsum, ntree);
            temp = (wsum[0] - wsum[1])/twt[0];  /* -1 to 1 */
            nevent--;                                                 
            resid[0][nevent] = (temp+1)/2;         /* 0 to 1 */
            resid[1][nevent] = z2/(4 * twt[0]*twt[0] * twt[0]);
            resid[2][nevent] = twt[0];
            dcount++;
	    i++;
	}
    }
