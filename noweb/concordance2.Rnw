...
concordance.fit <- function(y, x, strata, weight, ymin, ymax, timewt,
    ...
    if (missing(strata) || length(strata)==0 || all(strata==strata[1])) {
        ...
        rval <- list(concordance= (d+1)/2, count=fit$count[1:5], n=n,
...
Part of the nuisance above is that for tied.y and tied.xy counts 
I need to compare a
...
for (i=0; i<n;) {
	...
	for (j=i; j<n && time[j]==time[i]; j++) {
	    ndeath++; 
	    imat[4][group[j]] -= dtwt[x[j]];  /* tied on x, prior to additions*/
	}
	...
	for (j=i; j< (i+ndeath); j++) {
	    ...
	    imat[2][group[j]] -= wsum[2];  /* tied.x */
            temp = imat[4][group[j]] + wsum[2] - wt[j]; /*tied on xy with j*/
            imat[4][group[j]] += (wsum[2] - wt[j]); 
	    imat[3][group[j]] += dwt- (temp - wt[j]); /* tied on y, not on x*/
	    count[3] -= temp*wt[j]; 
	    count[4] += temp*wt[j];
        ...
        count[5] += dwt * z2/twt[0];  /* weighted variance in risk set*/
	i += ndeath;
