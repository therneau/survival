...
for (i=0; i<n;) {
	...
	ndeath=0; dwt=0; 
	    ...
	    temp = timewt[utime]/twt[0]; /* adjusted time weight */
	    for (k=0; k<3; k++) {
	...
	/* second pass */
	for (j=0; j< ndeath; j++) {
	    imat[3][group[ii]] += dwt - wt[i];   /* tied on y */
	    /* "initial" estimates for influence */
	    walkup(dnwt, dtwt, x[i], wsum, ntree);
	    imat[0][group[i]] -= wsum[1];
	    imat[1][group[i]] -= wsum[0];
	    imat[2][group[i]] -= wsum[2];
        ...
        count[4] += dwt * z2/twt[0];  /* weighted variance in risk set*/
