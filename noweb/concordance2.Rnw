...
concordance.formula <- function(formula, data,
                                ...
                                influence=0, residuals=FALSE, reverse=FALSE,
                                timefix=TRUE) {
    ...
    if (reverse) {
        ...
        if (is.matrix(fit$influence)) 
            fit$influence <- fit$influence[,c(2,1,3,4,5)]
        if (is.matrix(fit$count)) fit$count <- fit$count[, c(2,1,3,4,5)]
        else fit$count <- fit$count[c(2,1,3,4,5)]
        if (residuals) fit$residuals$rank <- -fit$residuals$rank
    }
...
print.concordance <- function(x, ...) {
    ...
    if (!is.matrix(x$count) || nrow(fit$count < 11)) 
...
concordance.fit <- function(y, x, strata, weight, ymin, ymax, timewt,
    ...
    docount <- function(y, risk, wts, timeopt= 'n') {
        ...
        sfit <- survfit(y~1, weight=wts, se.fit=FALSE)
...
Methods are defined for lm, survfit, and coxph objects.  They are
nearly identical.
I don't call \code{concordance.fit} directly so as to inherit more of
the argument checks.
...
concordance.coxph <- function(formula, reverse=TRUE, ...) {
    Call <- match.call()
    if (!is.null(formula$y)) Y <- formula$y
    else if (!is.null(formula$model)) Y <- model.response(formula$model)
    temp <- concordance(Y ~ formula$linear.predictors, reverse=reverse, ...)
    temp$call <- Call
    temp
}
@ 
