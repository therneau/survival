...
<<concordance>>=
concordance.lm <- function(x, ...) {
    Call <- match.call()
    if (!is.null(x$y)) Y <- x$y
    else if (!is.null(x$model)) Y <- model.response(x$model)
    temp <- concordance(Y ~ x$fitted.values, ...)
    temp$call <- Call
    temp
}
concordance.survreg <- function(x, ...) {
    Call <- match.call()
    if (!is.null(x$y)) Y <- x$y
    else if (!is.null(x$model)) Y <- model.response(x$model)
    temp <- concordance(Y ~ x$linear.predictors, ...)
    temp$call <- Call
    temp
}

concordance.coxph <- function(x, reverse=TRUE, ...) {
    Call <- match.call()
    if (!is.null(x$y)) Y <- x$y
    else if (!is.null(x$model)) Y <- model.response(x$model)
    temp <- concordance(Y ~ x$linear.predictors, reverse=reverse, ...)
    temp$call <- Call
    temp
}
@ 
