...
concordance.fit <- function(y, x, strata, weight, ymin, ymax, timewt,
    ...
    docount <- function(stime, risk, wts, grp, timewt= 'n') {
        ...
        gtemp <- factor(rep(1L, n))
        sfit <- survfitKM(gtemp, stime, se.fit=FALSE)
        ...
        if (timewt %in% c("S/G", "n/G", "n/G2")) {
            ...
            gfit <- survfitKM(gtemp, temp, se.fit=FALSE)
        ...
        grp <- match(group, sort(unique(group))) -1L
...
$-(s_a + s0)$  with between SS contribution of $s_b(s_a + s_0)^2$.
...
SEXP concordance3(SEXP y, SEXP x2, SEXP wt2, SEXP group2) {
    ...
    */
    ...
    resid2 = SET_VECTOR_ELT(rlist, 1, allocMatrix(REALSXP, nevent, 3));
...
for (i=0; i<n;) {
    ...
    walkup(dnwt, dtwt, x[i], wtemp, ntree);
    for (j=0; j<3; j++) imat[j][group[i]] -= wtemp[j];

    if (status[i]==0) { /* censored, simply add them into the tree */
        walkup(nwt, twt, x[j], wtemp, ntree);
	z2 += wt[i]*(wtemp[0]*(wt[i] + 2*(wtemp[1] + wtemp[2])) +
		     wtemp[1]*(wt[i] + 2*(wtemp[0] + wtemp[2])) +
		     (wtemp[0]-wtemp[1])*(wtemp[0]-wtemp[1]));
	...
	for (j=i; j< ndeath+i; j++) {
            walkup(nwt, twt, x[j], wtemp, ntree);
	    z2 += wt[j]*(wtemp[0]*(wt[j] + 2*(wtemp[1] + wtemp[2])) +
			 wtemp[1]*(wt[j] + 2*(wtemp[0] + wtemp[2])) +
			 (wtemp[0]-wtemp[1])*(wtemp[0]-wtemp[1]));
        ...
        for(j=0; j<ndeath; j++) {
            ...
            temp = (wtemp[0] - wtemp[1])/twt[0];  /* -1 to 1 */
            resid[0][dcount] = (temp+1)/2;         /* 0 to 1 */
            resid[1][dcount] = z2/(4 * twt[0]*twt[0] * twt[0]);
            resid[2][dcount] = twt[0];
            dcount++;
	    i++;
	}
    }
...
for (i=0; i<n; i++) {
    walkup(dnwt, dtwt, x[i], wtemp, ntree);
    for (k=0; k<3; k++) imat[k][i] += wtemp[k] - wt[i];
}
@ 
