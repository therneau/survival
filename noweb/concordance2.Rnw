...
concordance.fit <- function(y, x, strata, weight, ymin, ymax, timewt,
    ...
    docount <- function(stime, risk, wts, group, timeopt= 'n') {
        ...
        if (ncol(y) ==2)
            fit <- .Call(Cconcordance3, stime, bindex, wts, rev(timewt), 
                         as.integer(gtemp) -1L, residuals)
        else fit <- .Call(Cconcordance4, stime, bindex, wts, rev(timewt), 
                          sort.start, sort.stop, as.integer(gtemp) -1L, residuals)
        ...
        # The influence matrix counts each pair twice, so the totals are 1/2 the
        #  column sums.
        ...
        if (residuals) {
            if (ncol(y)==2) dtime <- stime[stime[,2]==1, 1]
            else dtime <- stime[stime[,3]==1, 2]
            dimnames(fit$resid) <- list(sort(dtime),
                                    c("rank", "npair", "wt", "var"))
        }
        fit
    }
    ...
    count <- colSums(imat)/2
    npair <- sum(count[1:3])
    somer <- (count[1] - count[2])/npair
    dfbeta <- (imat[,1]- imat[,2])/npair -
            (somer/npair)*rowSums(imat[,1:3])
    var.somer <- sum(dfbeta^2)
    rval <- list(concordance = (somer+1)/2, count=count, n=n,
                 var = c(IJ= var.somer/4, PH= fit$count[6]/(4*npair^2)))
 
    if (influence > 1) {
        if (is.strata) {
            # reorder the influence to be data set order
            # it is currently data set order, within strata
            unsort <- integer(nrow(influence))
            unsort[order(as.numeric(strata))] <- 1:nrow(influence)
            if (influence==1) rval$dfbeta <- dfbeta[unsort]
            else rval$influence <- imat[unsort,]
        } 
        else {
...
<<walkup>>
    ...
    */
    ...
    imat2 = SET_VECTOR_ELT(rlist, 1, allocMatrix(REALSXP, ngroup, 5));
    for (i=0; i<5; i++) {
...
for (i=0; i<n;) {
	...
	ndeath=0; dwt=0; 
        if (twt[0]==0) { /* no one to compare them to */
	    addin(nwt, twt, x[i], wt[i]);
	    i++;
	    utime++;
	    continue;
	}
	else adjtimewt = timewt[utime++] / twt[0];

        /* pass 1 */
	for (j=i; j<n && time[j]==time[i]; j++) {
	    ndeath++; 
	    imat[4][group[j]] -= dnwt[x[j]];  /* tied on x, prior to additions*/
	}

        /* pass 2 */   
        for (j=i; j< (i+ndeath); j++) {
	    count[3] += wt[j] * dwt;  /* update total tied on y */
            dwt += wt[j];   /* count of deaths and sum of wts */

	    /* Count concordant, discordant, etc. */
	    walkup(nwt, twt, x[j], wsum, ntree);
	    for (k=0; k<3; k++) {
		count[k] += wt[j]* wsum[k] * adjtimewt;
		imat[k][group[j]] += wsum[k]*adjtimewt;
	    }

	    /* add to the event tree */
	    addin(dnwt, dtwt, x[j], adjtimewt*wt[j]);  /* weighted deaths */
	...
	for (j=i; j< (i+ndeath); j++) {
	    /* Update influence */
	    walkup(dnwt, dtwt, x[j], wsum, ntree);
	    imat[0][group[j]] -= wsum[1];
	    imat[1][group[j]] -= wsum[0];
	    imat[2][group[j]] -= wsum[2];  /* tied.x */
	    imat[3][group[j]] += (dwt- wt[j])* adjtimewt;
            imat[4][group[j]] += (wsum[2] - wt[j] * adjtimewt); 
            count[4] += wt[j] * (wsum[2] - wt[j] * adjtimewt) /2;
