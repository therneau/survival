...
concordance.fit <- function(y, x, strata, weight, ymin, ymax, timewt,
    ...
    docount <- function(stime, risk, wts, grp, timewt= 'n') {
        ...
        if (ncol(y) ==2) 
            .Call(Cconcordance3, stime, bindex, wts,
                            influence, grp)
        else .Call(Ccondordance4, stime, bindex, wts, sort.start, 
                             sort.stop, influence, grp)
    }
...
This actually made almost no difference in the compute time, but was an
...
\end{align}
A given subject's weight appears twice, once when they are an
event ($w_i \delta_i)$, and the second time as part of the risk set for other's
events.  
The solution is to keep two trees.  
Tree 1 contains all of the subjects at risk, and we traverse it to obtain the 
updates to the concordance.  
...
Deaths cause a separate influence addition when they occur.
...
share a time.  Move from largest to smallest time.
\begin{enumerate}
  \item Move to a new event time
  \item For any new entries, subtract intialize the current grand totals from
    their IJ counts in tree 2.  
    For a grouped jackknife several subjects may share the same indices.
  \item Add those subjects into the main tree.
  \item For all subjects with an an event at this time, make a first pass
    \begin{itemize}
      \item update the values in tree 2 and also update the concordance.
      \item Keep track of the tied time count as well.  Since the predictor
        value plays no role for these we don't need a tree, just a count.  It
        will rezero at the next unique event time.
    \end{itemize}
  \item Remove any observations from the main tree that leave at this event 
    time, updating their IJ counts using tree 2.
  \item At the end of each stratum, remove all remaining subjects from the
    tree, while updating IJ counts.
\end{enumerate}
...
<<concordance1>>=
#include "survS.h"
SEXP concordance3(SEXP y, SEXP x2, SEXP wt2, SEXP influence2, SEXP group2) {
    int i, j, k, index;
    ...
    */
    ...
    SEXP rlist, count2, imat2;
    double *count, *imat[6];
    double *wt;
    int    *x;
    int    *group;
    static const char *outnames[]={"count", "influence", ""};
    ...
    /* assign the influence matrix, and maybe return it */
    if (influence == 2) {
	/* return the whole thing */
	imat2 = SET_VECTOR_ELT(rlist, 1, allocMatrix(REALSXP, ngroup, 4));
	for (i=0; i<4; i++) imat[i] = imat2 + i*ngroup;
    }
    else { 
	imat[0] = allocVector(REALSXP, ngroup, 4);  /* imat is internal only*/
	for (i=1; i<3; i++) imat[i] = imat[0] + i*ngroup;
	
	if (influence==1) {
	    /* return one value per group */
	    imat2 = SET_VECTOR_ELT(rlist, 1, allocVector(REALSXP, ngroup));
	    imatx = REAL(imat2);
	}
    }
...
\begin{itemize}
  \item If it is a death time, we need to process all the deaths tied at
	...
	for all pairs $i,j$ of tied times.
      \item The addition to tied-on-x will be the weight of this 
        observation times
