...
finegray <- function(formula, data, subset, na.action= na.pass,
    ...
    if (!is.null(id)) {
        # get rid of the parenthesis that model.frame puts around the name
        # in an expression like Surv(time, stat) ~ ., id=clinic the id will
        # end up being a duplicate, so toss it.
        temp <- grepl("^\\(.*\\)$", names(mf))
        if (sum(temp) ==1) { # failsafe
            temp2 <- names(mf)[temp]
            temp2 <- substring(temp2, 2, nchar(temp2) -1) 
            if (temp2 %in% names(mf)) mf <- mf[!temp]  #duplicate variable name
            else names(mf)[temp] <- temp2
        }
    }
...
The truncation distribution due to delayed entry is on the reverse time scale.
Geskus assumes that ties are ordered as event < censor < truncation, so on
the reverse scale we don't need to fudge the times.
...
If there is a dely, we still do the computations on the integer scale.
...
1 for the first row of each subject, when the subject is not in time
order in the data set.  The offset of .2 used above is not needed, but due
to the underlying integer scale it doesn't harm anything either.
<<finegray-censor>>= 
if (delay) {
    tstat <- rep(0L, nrow(mf))
    tstat[first] <- 1L
    dsurv <- survfit(Surv(-newtime[,2], -newtime[,1], tstat) ~ istrat, 
                     se.fit =FALSE)
}
@ 
...
with the jumps in dsurv.
The truncation distribution dsurv ($H$) will become 0 at the first entry time; 
it is a left continuous function whereas csurv ($G$) is right continuous.  
However, since the weights are defined as $G(t-)H(t-)$ the details don't
matter.  We can slide $H$ one point to the left and merge them at the
jump points.
...
if (delay) {
    dtime <- rev(-dsurv$time[dsurv$n.event > 0])
    dprob <- c(rev(dsurv$surv[dsurv$n.event > 0]), 1) 
    ctime <- csurv$time[csurv$n.event > 0]
    cprob <- c(1, csurv$surv[csurv$n.event > 0]) 
    temp <- sort(unique(dtime, ctime)) # these will all be integers
    index1 <- findInterval(temp, dtime)
    index2 <- findInterval(temp, ctime)
    ctime <- utime[temp]
    cprob <- dprob[index1+1] * cprob[index2+1]  # G(t)H(t), eq 11 in Geskus
}
else {
