...
finegray <- function(formula, data, subset, na.action= na.pass,
                     endpoint, count="", id="", tstart="fgstart", 
                     end="fgend", status="fgstat", wt="fgwt") {
    Call <- match.call()
    ...
    if (class(formula[[2]]) == "call" && formula[[2]][[1]]== as.name("Surv")){
        ...
        if (nY ==2) {
            if (is.name(temp$time)) end <- as.character(temp$time)
            if (is.null(temp$status){
                if (is.name(temp$time2))  event <- as.character(temp$time2)
            else if (is.name(temp$event)) event <- as.character(temp$event)
        }
        else {
            if (is.name(temp$time)) tstart <- as.character(temp$time)
            if (is.name(temp$time2))end   <-  as.character(temp$time2)
            if (is.name(temp$event))event <-  as.character(temp$event)
            }
    }
     else {
          if (class(formula[[2]]) == "name") {
             was.a.call <- FALSE
             survname <- as.character(formula[[2]])
         }
     }
    <<finegray-censor>>   
    <<finegray-build>>
