...
$H$ is calculated on a reverse time scale, which leads to a left
continuous function which we fix up later.
The only nuisance is the status variable, which is
