...
\begin{align*}
  G(t) &= \prod{s \le t} \left(1 - \frac{c(s)}{r_c(s)} \right ) \\
  H(t) &= \prod{s > t} \left(1 - \frac{e(s)}{r_e(s)} \right ) 
\end{align*}
...
Thus $G$ we use a modified Kapan-Meier where any events at censoring time $t$ are
...
if (delay) {
    ...
    ctime <- utime[temp]
browser()
    cprob <- dprob[index1] * cprob[index2+1]  # G(t)H(t), eq 11 in Geskus
}
else {
...
if (is.null(Gsurv$strata)) cstrat <- rep(1, length(ctime))  
else {
    cstrat <- (rep(1:length(Gsurv$strata), Gsurv$strata))
    cstrat <- cstrat[findInterval(ctime, Gsurv$time)]
}
maxtime <- max(Y[,2])
...
stratfun <- function(i) {
    ...
    ckeep[index] <- TRUE
    expand <- (Y[keep, 3] !=0 & Y[keep,3] != enum & last) #which rows to expand
