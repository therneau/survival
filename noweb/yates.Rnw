...
cmatrix <- function(fit, term, 
                    test =c("default", "linear", "pairwise"),
                    levels) {
    ...
    if (missing(levels)) {
        ...
        if (is.null(temp)) levels <- NULL
        else  levels <- do.call(expand.grid, c(temp, stringsAsFactors=FALSE))
        }
    else {  #user supplied
...
a missing value in beta: for the purposes of prediction we treat
0*NA as zero --- the fact that a coef is missing does not matter if you don't
use it. 
If there are missing coefs then the variance matrix will not have those columns.
...
estfun <- function(cmat, beta, varmat) {
    ...
    if (any(nabeta)) {
        ...
        # does this row of cmat involve an NA?
        hasna <- ifelse(apply(cmat[,nabeta, drop=FALSE] !=0, 1, any), NA, 0)

        estimate <- drop(cmat[,k] %*% beta[k])  # vector of predictions
        evar <- drop(cmat[,k] %*% varmat %*% t(cmat[,k, drop=FALSE]))
        list(estimate = estimate + hasna,
             var = evar + outer(hasna, hasna))
    }
    else {
...
testfun <- function(cmat, beta, varmat, sigma2) {
    ...
    if (any(nabeta)) {
        k <- which(!nabeta)  #columns to keep
        hasna <- apply(cmat[,nabeta, drop=FALSE] !=0, 1, any)
        estimate <- drop(cmat[,k] %*% beta[k])  # vector of predictions
        temp <- qform(cmat[,k] %*% varmat %*% t(cmat[,k,drop=FALSE]), estimate)
        if (any(hasna)) rval <- c(chisq=NA, df=temp$df)
        else rval <- c(chisq=temp$test, df=temp$df)
    }
    else {
