...
cmatrix <- function(fit, term, 
    ...
    if (is.character(term)) term <- formula(paste("~", term))
        ...
        if (all(term == floor(term) & term >0 & term < length(Tatt$term.labels)))
            term <- formula(paste("~", 
                                  paste(collapse(Tatt$term.labels[term], collapse='+'))))
...
yates <- function(fit, test, population=c("none", "data", "factorial", "sas"),
    ...
    if (population =="none" && method != "direct")
        warning(method, " method only applies to population estimates")
    beta <-  coef(fit)
    ...
    # define the cm function before we touch the test argument
    cm <- function(...) cmatrix(fit, ...)
    if (!is.list(test)) test <- list(test)
    
    # check that it is test is legal.  It can be a list whose components can
    #   be lists.  But at the end we have matrices with the right number of
    #   columns
    ncoef <- length(beta)
    legal <- function(x) (is.matrix(x) && ncol(x)==ncoef)
    # this error needs to be more informative
    if (!all(sapply(test, function(x) legal(x) || all(sapply(x, legal)))))
        stop("the test argument is invalid")

    if (population=="none") {
        tfun <- function(x, beta=beta, varmat=vmat, sigma2= sigma^2)
            cfun(x, beta, varmat, sigma2)
        tests <- lapply(test, function(x) {
                        if (is.list(x)) lapply(x, tfun) else tfun(x)
        })
        # cast this into a matrix form
        rget <- function(x, what) {
            if (is.list(x)) lapply(x, rget, what)
            else x[[what]]
        }
        rmat <- cbind(unlist(rget(tests, "estimate")),
                      unlist(rget(tests, "var"))
                      unlist(rget(tests, "ss")))
