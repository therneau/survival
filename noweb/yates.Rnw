...
cmatrix <- function(fit, term, levels, 
    ...
    rval <- list(levels=levels, termname=termname, cmat=cmat)
    class(rval) <- "cmatrix"
    rval
}
@ 
...
#        if (is.null(sigma)) sigma <- 1
    ...
    dotest <- function(x, method, population) {
        if (population=="none") {
            <<yates-build-none>>
...
The first task is to fill in the appropriate columns of an $X$ matrix
with those values.
<<yates-build-none>>=
# x will be a cmatrix object
      
@ 


First is the case of population=``none'', which consists of filling in a zero
for all other coefficients of the $C$ matrix.  This may or may not be a sensible
thing to do.
