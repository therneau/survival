...
<<cmatrix>>=
# Helper routines for the yates function
cmatrix <- function(fit, term, levels, 
    ...
    #  or double check them (categorical).  If not, then for categorical
    #  variables we assume all possible levels.
    if (missing(levels)) {
        if (any(iscat(parts))) {
            temp <- parts[iscat(parts)]
            levels <- do.call("expand.grid", fit$xlevels[temp])
        }
        else levels <- NULL 
    }
    else {  #user supplied
...
type <- match.arg(type)
if (type=="full") {
    if (is.null(levels)) cmat <- matrix(1,1,1)
    else {
        cmat <- diag(nrow(levels))  # Unable to login?
Please enter the email address you registered with and we'll send your password to you.
Email Address
all contrasts with row 1
        cmat[,1] <- -1
        cmat <- cmat[-1]
        }
else {
