...
survfitKM <- function(x, y, weights=rep(1.0,length(x)), 
    ...
    if (missing(start.time)) start.time <- min(c(0, y[,ny-1]))
...
if (influence > 0) {
    if (type==1 | type==2) {
        if (influence==1 || influence ==3) {
            if (length(xlev)==1) {
                rval$influence.s <- cfit$influence1
                row.names(rval$influence.s) <- clname
            } 
            else {
                ...
                rval$influence.s <- temp
            }
        }
        if (influence==2 || influence==3) {
            if (length(xlev)==1) {
                rval$influence.c <- cfit$influence2
                row.names(rval$influence.c) <- clname
            }
            else {
                temp <- vector("list", ngroup)
                for (i in 1:ngroup) {
        ...
        if (influence==1 || influence==3) {
            ...
            if (length(xlev) ==1) 
                rval$influence.s <- -temp * rep(rval$surv, each=nrow(temp))
            else {
                for (i in 1:ngroup)
