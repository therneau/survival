...
<<survfitKM>>= 
survfitKM <- function(x, y, weights=rep(1.0,length(x)), type=1,
                      se.fit=TRUE,
                      conf.int= .95,
    ...
    else if (!is.numeric(type) || type != floor(type) || type < 1 || type >4)
        stop("type argument should be an integer from 1 to 4")
    else type <- as.integer(type)
    stype <- ifelse (type < 3, 1, 2) # survival type 1=direct, 2= exp(chaz)  
    ...
    if (missing(start.time)) {
        ...
        else if (any(y[,ny-1] <0)) start.time <- min(y[,ny-1])
