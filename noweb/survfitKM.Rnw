...
Save the requested influence matrices.
...
If the Fleming-Harrington is being used, then the influence on the 
RMSE has to be calculated here rather than one step up in the R code,
since it needs to be accumulated
...
if (ninftime ==0) { /* save all times */
    ...
    if ((i+1)==n) { 
        /* end of the survival curve, fill in all the rest of the inftimes */
        if (influence & 0x4) {
            for (k=0; k<nid; k++) imat3[k] -= kvec[i]*inf2[k];	
            for (j= itimej +1; j < ninftime; j++) {
                for (k=0; k<nid; k++) imat3[k+nid] = imat3[k];
                k += nid;
            }
        ...
        if (influence &0x4) 
            for (k=0; k<nid; k++) imat3[k] -= kvec[i]* inf1[k];
        while(dtime[i+1] >= intime[itime]) {
            /* we have crossed the next "save it" point */
            if (influence & 0x2)
                for (k=0; k<nid; k++) *imat2++ = inf2[k];
