...
<<survfit.coxph-setup1>>=
if (length(object$coefficients) ==0 || all(is.na(object$coefficients)))
    stop("survfit does not work with coxph models that have no covariates")
Terms  <- terms(object)
