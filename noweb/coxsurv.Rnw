...
if (is.null(mf)) {
    ...
    if (coxms) {
        ...
        if (object$states != mcheck$states)
            stop("failed to rebuild the data set")

        cluster <- model.extract(mf, "cluster")
        xstack <- stacker(object$cmap, as.integer(istate), X, Y, strata,
                          states= object$states)
        X <- xstack$X
        Y <- xstack$Y
        strata <- xstack$strata
...
if (individual) {
    ...
    if (coxms) 
        result <- survfitcoxph.fit2(Y, X, weights, x2, risk, newrisk, strata,
                                se.fit, stype, ctype, varmat, oldid, cluster,
                                id, y2, strata2)
    else result <- survfitcoxph.fit(Y, X, weights, x2, risk, newrisk, strata,
                                se.fit, stype, ctype, varmat, oldid, cluster,
                                id, y2, strata2)
   }
@ 
...
<<survfit.coxph-result>>=
    ...
    result <- survfitcoxph.fit(Y, X, weights, x2, risk, newrisk, strata,
                                se.fit, stype, ctype, varmat, oldid, cluster)
