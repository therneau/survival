...
<<survfit.coxph-setup>>=    
    ...
    else if (is.list(newdata)) newdata <- data.frame(newdata) 

    mf2 <- eval(tcall, parent.frame())
    if (nrow(mf2) != nrow(newdata))
        warning(paste("'newdata' had", nrow(newdata), 
                      "but the variable(s) found have", nrow(mf2), "rows"))
}
@ 
