...
subterms <- function(x, i) {
    ...
    x <- x[i]
    if (!is.null(predvars)) {
        j <- 1 + attr(x, 'response')   # predvars is a call object
                                       # arg for response, one for the function name
        if (is.logical(i)) attr(x, "predvars") <- predvars[c(rep(FALSE,j), i)]
        else if (all(i<0)) attr(x, "predvars")  <- predvars[i-j] 
        else attr(x, "predvars")  <- prevars[i+j]
    }
    x
}
