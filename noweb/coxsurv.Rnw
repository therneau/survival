...
item in the formula, ``tt'' in the call below is the terms object
which represents the formaula and ``i'' is a general subscript.
The predvars component is a call object with one element for each
term in the formula, so y ~ age + ns(height) would lead to a predvars
of length 4, element 1 is the call itself to ``list'', 2 would be y, etc.
The dataClasses attribute is a list with names and can be subscripted in that
way.
<<survfit.coxph-setup>>= 
subterms <- function(tt, i) {
    ...
    if (!is.null(predvars)) {
        if (attr(tt, "response")) {
            if (is.logical(i)) j <- c(TRUE, TRUE, i)
            else if (all(i<0)) j <- i - 2  #shift the index
            else j <- c(1:2, i+2)
        }
        else {
            if (is.logical(i)) j <- c(TRUE, i)
            else if (all(i<0)) j <- i-1
            else  j <- c(1, i+1)
        }
        attr(tt, "predvars") <- predvars[j]
        }
    
    if (!is.null(dataClasses)){
        temp <- dimnames(attr(tt, 'factors'))[[1]]
        attr(tt, "dataClasses") <- dataClasses[temp]
    }
    tt
}
