...
subtraction.  A quick test shows that two \code{lm} models fit this way
have different linear predictors, two \code{coxph} models have different
predictors and log-likelihood, etc.
This is simply a bad model and there is no
``correct'' behavior in creating predicted survival curves.
Strata by covariate interactions work, however, because the coxph code treats
\code{age:strata(sex)} as though it were \code{age*strata(sex)}.
