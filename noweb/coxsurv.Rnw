...
further material.
Always prefer object$y to model.response, since the former will have been
passed through aeqSurv with the options the user specified.
Otherwise we guess that the default timefix=TRUE option of coxph.control
was in force for the coxph fit.
