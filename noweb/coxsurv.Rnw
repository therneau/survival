...
[[strata]] from the original model are not needed. 
...
Well almost that -- I discovered that if xlevels contains any variable
that is no longer in the model then one gets a spurious warning message
out of the [[model.frame]] function.
Hence the long [[if]] statement below.
<<survfit.coxph-setup>>=    
...
One more wrinkle is that in this particular case I do not want to find variables
in the parent environment.  Say a variable ``zed'' is in the formula, not in
mf2, but found in the global environment.  We want to give a not found message
for zed.  The model.frame function ignores the environment argument of eval
so we can't use \texttt{eval(tcall, NULL)}; instead it will use the environment
tacked onto the formula.  So set it to NULL instead.
<<survfit.coxph-setup>>=    
    if (is.vector(newdata, "numeric")) {
        ...
        tcall$data <- newdata
        newdata <- matrix(newdata, nrow=1)  #for the check below
        }
   
    environment(tcall$formula) <- NULL
    mf2 <- eval(tcall)
    }
@ 
