...
if (!is.null(x) && ncol(x) >0) { #not a ~1 or ~offset(x) model
    if (!is.null(i = attr(Terms, "specials")$strata)) 
        stype <- attr(Terms, "factors")[i,] * (1+ attr(Terms, "order")>1)
    else stype <- pmin(attr(Terms, "order"), 2)
    }
...
to do.  
This case is also very rare; the package existed for 15+ years before
...
<<survfit.coxph-result>>=
    ...
    if (missing(newdata)) {
        if (any(attr(Terms, "order") >1))
            stop ("Models with interation terms require newdata")
