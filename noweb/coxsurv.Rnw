...
\begin{description}
  ...
  \item[type] computation type for the survival curve, now replaced by stype and ctype
...
That is, the \code{coxph} computation used for \code{method=``exact''} is
...
The other two rows of labels in \code{temp1} are historical; 
...
the latter will
pick off the \code{xlevels} component if the \code{x} component is missing (which
is the default).
<<survfit.coxph-setup>>=
...
Terms <- object$terms
n <- object$n[1]
if (!has.strata) strats <- NULL
else strats <- as.integer(object$strata)
...
if (is.null(mf)) {
    ...
    if (has.strata) {
        if (is.null(strats)){
            ...
            strats <- as.integer(as.factor(strata))  # the 1, 2, ... form
        }
    }

    if (coxms) {
        # replicate actions found in the coxph-multi-X chunk,
        #  along with recreation of stacker() inputs
        ...
        xstack <- stacker(object$cmap, as.integer(istate), X, Y, strats,
                          states= object$states)
        position <- survflag(Y, oldid)[xstack$rindex]
        X <- xstack$X
        Y <- xstack$Y
        strats <- strats[xstack$rindex]
...
if (!missing(start.time)) {
    ...
    if (length(transition)) transition <- transition[keep]
    n <- nrow(Y)
}
...
if (type!= 'counting') individual <- FALSE
@
...
if (is.null(X) || ncol(X)==0) { # a model with ~1 on the right hand side
    ...
    xcenter <- sum(object$means * beta)  
    if (!is.null(object$frail)) {
       keep <- !grepl("frailty(", dimnames(X)[[2]], fixed=TRUE)
       X <- X[,keep, drop=F]
    }
        
    risk <- c(exp(X%*% beta + offset - (xcenter + mean(offset))))
}
@ 
...
if (individual) {
    ...
    if (!missid) {  #grab the id variable
        newid <- model.extract(mf2, "id")
        if (is.null(newid)) stop("id=NULL is an invalid argument")
        }
    else newid <- rep(1, nrow(mf2))
    ...
    newrisk <- exp(c(x2 %*% beta) + offset2 - (mean(offset) + xcenter))

    if (coxms) 
        result <- survfitcoxph.fit2(ctype, stype, se.fit, varmat, cluster,
                                    object$cmap, object$transitions, object$states,
                                    Y, X, weights, risk, position, strats, oldid,
                                    transition, y2, x2, strata2, id)
    else result <- survfitcoxph.fit(Y, X, weights, x2, risk, newrisk, strats,
                                se.fit, stype, ctype, varmat, oldid, cluster,
                                id, y2, strata2)
   }
@ 

If there is no newdata argument, the
centering means that we need to predict for x2= \code{object\$means}.
This is the second the most common call to the routine.
<<survfit.coxph-result>>=
else {
    if (missing(newdata)) {
    ...
    newrisk <- exp(c(x2 %*% beta) + offset2 - xcenter)
