...
This requires use of [[model.frame]] logic.
...
<<survfit.coxph-setup>>=    
    ...
    if (!is.null(object$xlevels) &&
        ...
        tcall$xlev <- object$xlevels[match(names(object$xlevels),
                                           attr(Terms2, "term.labels"),
                                           nomatch=0)]
...
strata, something that the survfit.coxph.fit subroutine is not set up
to do.  
...
the new data [[mf2]]. 
You can't just paste these together, as I discovered in an evening of failures;
...
<<survfit.coxph-result>>= 
    ...
    for (i in 1:nrow(tframe)) {
        ...
        zed<- survfit.coxph.fit(y, x, wt, x2, risk, newrisk, strata,
                                           se.fit, survtype, vartype, varmat,
                                           id=NULL, unlist=FALSE)
        temp[[i]] <- zed[[i]]
        }
    tfun <- function(x, y) as.vector(unlist(lapply(x,y))) #no names
    ...
    if (nscol >1) {
        ...
        if (se.fit) temp$std.err = tfun(temp, function(x) tx$std.err)
        }
    }
@ 
