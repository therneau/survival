...
survfit.coxph <-
  function(formula, newdata, se.fit=TRUE, conf.int=.95, individual=FALSE,
      ...
      if (inherits(formula, "coxphms")) sd.fit <- FALSE   #still to do
...
if (is.null(mf)) {
    ...
    if (length(oldid)) position <- survflag(Y, oldid)
    ...
    if (coxms) {
        ...
        cifit <- survfitCI(X, Y, weights, oldid, istate, stype=stype,
...
if (individual) {
    ...
    risk2 <- exp(c(x2 %*% beta) + offset2 - (mean(offset) + xcenter))
}
    ...
    if (length(object$means)) {
        ...
        risk2 <- (c(x2 %*% beta) - xcenter)
    }
    else {risk2 <-1;  x2 <- 0}   # a model with only an offset
} else {
    offset2 <- model.offset(mf2)
    if (length(offset2) >0) offset2 <- offset2 - mean(offset)
    else offset2 <- 0
...
if (individual) {
    ...
    else result <- coxsurv.fit1(ctype, stype, se.fit, varmat, cluster, 
                               Y, X, weights, risk, position, strata, oldid,
                               transition, y2, x2, risk2, strata2, id2)
}
else {
