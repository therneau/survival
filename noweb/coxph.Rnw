...
\begin{enumerate}
  ...
  \item Likewise, nested  case-control models can have thousands of strata,
    again leading to intercepts we never needed.
  \item If there are strata by factor interactions in the model, however, 
...
\end{enumerate}
Thus, the usual plan is to remove cluster and strata terms from the ``Terms''
object before calling model.matrix.  
The \code{assign} attribute of the resulting model matrix then needs to be
...
The dropterms contains terms to drop before creation of the X matrix.
