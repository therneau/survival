...
coxph <- function(formula, data, weights, subset, na.action,
    ...
    # Move any cluster() term out of the formula, and make it an id
    #  option instead.  This makes everything easier.
    if (missing(formula)) stop("a formula argument is required")
    
    if (is.list(formula))
        Terms <- if (missing(data)) terms(formula[[1]], specials="cluster") else
                 terms(formula[[1]], specials="cluster")
    else Terms <- if (missing(data)) terms(formula, specials="cluster") else
                 terms(formula, specials="cluster")

    tt <- attr(Terms, 'specials')$cluster
    if (length(tt) > 1) stop("a formula cannot have multiple cluster terms")
    if (length(tt) > 0) { # there is one
        if (!is.null(Call$cluster))
            stop("cannot have both cluster() in a formula and the cluster option")
        # subscripting of formulas is broken at least through R 3.5, if the
        #  formula contains an offset.
        Call$cluster <- attr(Terms, "variables")[[1+tt]][[2]]
        newform <- formula(Terms[attr(Terms, "response")[-tt])
        if (is.list(formula)) formula[[1]] <- newform
        else formula <- newform
    ...
    # grab the cluster, if present.  Using cluster() in a formula is not
    #  encouraged
    ...
    if (missing(robust)) robust <- !is.null(cluster)
        ...
        if (ny==2) cluster <- seq.int(1, nrow(mf))
...
mcheck <- multicheck2(Y, id, istate)
# error messages here
