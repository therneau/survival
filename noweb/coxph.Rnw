...
coxph <- function(formula, data, weights, subset, na.action,
	...
	model=FALSE, x=FALSE, y=!model,  tt, ...) {
...
type <- 'right'  # Y is now right censored, however the data started out
strats <- rep(1:length(counts$nrisk), counts$nrisk) #one
weights <- model.weights(m)
browser()
for (i in ntrans) 
