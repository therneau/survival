...
coxph <- function(formula, data, weights, subset, na.action,
    ...
    if (missing(covariates)) covlist <- NULL
        ...
        if (missing(statedata)) covlist <- parsecovar1(covariates)
            ...
            covlist <- parsecovar1(covariates, names(statedata))
        }
    ...
    if (!multi & !is.null(covlist))
        stop("covariates statement but the response is not multi-state")
    ...
    if (multi) {
        ...
        #  second check of covariates -- build tmap
...
The \code{na.coxph} routine has already located rows that have a missing
response, or missing all the covariates.
