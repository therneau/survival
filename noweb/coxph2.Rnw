...
\begin{itemize}
    \item If there is a covariates statement, we preprocess this to find out 
      any potential extra variables, and create a new formula if necessary.
      Final processing has to wait until we have Y, however.
    \item If there is a covariates statement, missing value processing needs
...
coxph <- function(formula, data, weights, subset, na.action,
	...
	model=FALSE, x=FALSE, y=TRUE,  tt, method=ties, 
        covariates, statedata, istate, cluster, id,  ...) {
    ...
    dformula <- formula   # the default formula for transitions   
    if (missing(covariates)) covlist <- NULL
    else {
        if (missing(statedata)) covlist <- parsecovar1(covariates)
        else covlist <- parsecovar1(covariates, statedata)
        ...
        tlab <- c(attr(terms.formula(formula), "term.labels"), tlab)
        newform <- reformulate(tlab, formula[[2]])
        environment(newform) <- environment(formula)
        formula <- newform
 #       tform$na.action <- na.pass
    ...
    # Grab cluster and/or id to check out multi-state data
    id <- model.extract(mf, "id")
    cluster <- model.extract(mf, "cluster")

    if(length(attr(Terms, "specials")$cluster)) {
        if (length(cluster) >0)
            stop("model has both a cluster option and a cluster term in the formula")
    ...
    if (length(cluster)) robust <- TRUE  # use a robust variance later
    else if (missing(robust)) robust <- FALSE
    ...
    if (multi) {
        ...
        if (length(cluster)==0) cluster <- id # use this as a default

        istate <- model.extract(mf, "istate")
        mcheck <- multicheck(Y, id, istate)
        # error messages here
    ...
    #  second check of covariates -- build tmap and take care of missings
    if (is.null(covlist)) {
        if (multi) {
            # the perhaps not-so-rare case of no covariates statement
            # tmap is particularly simple: separate coefficients for everything
            nterm <- length(attr(Terms, "term.labels"))
            ntran <- sum(transitions[,-1] > 0)
            tmap <- matrix(seq_len(nterm*ntran), nrow=nterm)
            tmap <- rbind(1:ntran, tmap)
            temp <- transitions[,-1, drop=FALSE]
            dd <- dimnames(temp)
            mapid <- cbind((match(dd[[1]], states))[row(temp)],
                           (match(dd[[2]], states))[col(temp)])
        }
    } else {
        if (!multi) stop("covariates statement, but not a multi-state Surv")
        ilev <- if (is.factor(istate)) levels(istate) else unique(istate)
        if (missing(statedata)) statedata <- NULL
        else if (is.null(statedata$state))
            stop("the statedata data set must contain a state variable")
        covlist2 <- parsecovar2(covlist, statedata, dformula= dformula,
                                Terms, transitions)
        # figure out missings
        # first a matrix for each starting state of all the terms used 
        #  for transitions from that state
        tmap <- covlist2$tmap[-1,,drop=FALSE]  # ignore the intercept row
        mapid <- covlist2$mapid[-1,, drop=FALSE]
        terms.used <- matrix(0L, nrow=nrow(transitions), ncol=nrow(tmap))
        for (i in 1:nrow(terms.used)) 
            terms.used[,i] <- any(tmap[,mapid[,1]==i] >0)
 
        mf.map <- match(rownames(attr(delete.response(Terms), 'factors')),
                        names(mf)) # the col of mf for each term
        # initialize the missing using all other terms, e.g. response
        other <- (1:ncol(mf))[-mf.map]  # other model frame columns
        omit <- apply(is.na(mf[,other]), 1, any)
        
        # for each term on the rhs of the formula, who is in an initial
        #  state s.t. one of the transitions from that state uses term
        #  j?, gives data.use vector.  If any of them have missing for term j,
        #  then that subject is out.  The code is essentially na.omit with
        #  "& data.use" tossed in.  
        for (j in seq(along = mf.map)) {
            # which rows of the data use j?
            data.use <- terms.used[match(istate, states), j]
            xx <- is.na(mf[[mf.map[j]]])
            d <- dim(xx)
            if (is.null(d) || length(d) != 2L)
                omit <- omit | (xx & data.use)
            else for (ii in 1L:d[2L]) omit <- omit | (xx[,ii] & data.use)
        }
        if (any(omit)) {
    ...
    if (length(timetrans)) {
        if (multi) stop("tt terms are not supported with multivariate endpoints")
         <<coxph-transform>>
        }
...
Finish the setup.  If someone includes an init statement, make sure
...
<<coxph-multi-X>>=
tmap <- covlist2$tmap
mapid <- covlist2$mapid
cmap <- matrix(0L, 1+ncol(X), ncol(tmap)) # first row is strata
cmap[1,] <- match(tmap[1,], sort(c(0, unique(tmap[1,])))) -1L
xcount <- table(attr(X, "assign"))
mult <- 1+ max(xcount)  #used to keep the coefs in the same oder
j <- 1
for (i in 2:nrow(tmap)) {
    k <- seq_len(xcount[i-1])
    cmap[j+k,] <- tmap[i,]*mult + rep(k, ncol(tmap))
    j <- j + max(k)
}
cmap[-1,] <- match(cmap[-1,], sort(c(0L, unique(cmap[-1,])))) -1L
dimnames(cmap) <- list(c("(Intercept)", colnames(X)), 
                       paste(mapid[,1], mapid[,2], sep=':'))

xstack <- stacker(cmap, as.integer(istate), X, Y, strata=strats,
                  states=states)
X <- xstack$X
Y <- xstack$Y
strats <- xstack$strata
...
<<coxph-compute>>=
    ...
    if( method=="breslow" || method =="efron") {
        if (type== 'right' || type=="mright")  fitter <- get("coxph.fit")
        else                 fitter <- get("agreg.fit")
    }
    else if (method=='exact') {
        if (type== "right" || type=="mright")  fitter <- get("coxexact.fit")
...
@ 
If any of the weights were not 1, save them in the results.
