...
coxph <- function(formula, data, weights, subset, na.action,
    ...
    if (missing(covariates)) covlist <- NULL
        ...
        temp <- unlist(lapply(covlist$rhs, function(x) 
            attr(terms.formula(x$formula), "term.labels")))
        temp <- c(temp, attr(terms.formula(formula), "term.labels"), temp)
        newform <- reformulate(temp, formula[[2]])
    ...
    special <- c("strata", "cluster", "tt")
    temp$formula <- if(missing(data)) terms(formula, special) else
    ...
    if (is.null(covlist)) {
        ...
        omit <- logical(n)  # nrow(mf)
        for (j in seq_len(ncol(mf))) {
            # which rows of the data use j?
            data.use <- terms.used[istate, j]
            x <- is.na(mf[[j]])
            d <- dim(x)
            if (is.null(d) || length(d) != 2L)
                omit <- omit | (x & data.use)
...
cmap[1,] <- match(tmap[1,], sort(unique(tmap[1,])))
...
cmap[-1,] <- match(cmap[-1,], sort(unique(cmap[-1,])))
