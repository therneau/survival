...
The coxph routine returns an Splus style assign component, which is a list with
one element per term. However, for a multistate model, the X matrix that
comes back from coxph.getdata will have more columns, and asgn needs to be
rebuilt.
...
else asgn <- attrassign(cget$x, terms(fit))
