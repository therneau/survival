...
for (i in 1:nrow(resid$used))
    wtmat <- wtmat + outer(resid$used[i,], resid$used[i,], pmin)
vmean <- resid$imat[1:nvar, 1:nvar, drop=FALSE]/wtmat

sresid <- resid$schoen
if (any(istrat!=1)) {
    # sresid will be sorted by time within strata, sort it instead by time
    sresid <- sresid[order(y[event, ny-1]),]
}
if (singledf && any(sapply(asgn, length) > 1)) { # collase multi-column terms
    ...
    vmean <- vmean[keep, keep, drop=FALSE]
}
rownames(sresid) <- signif(rval$time, 4)
