...
if (is.character(transform)) {
    ...
    if (ny==2) {
        ...
        resid <- .Call(Czph1, gtime, y, cget$x, fit$linear.predictors,
                        cget$weights, istrat, fit$method=="efron", ord-1L)
    }
    else {
        ord1 <- order(israt, y[,1])
        ord2 <- order(istrat, y[,2])
        resid <- .Call(Czph2, gtime, y, cget$x, fit$linear.predictors,
                        cget$weights, istrata, fit$method=="efron", 
                        ord1, ord2)
    }
@
...
Some of the survival routines return Splus style assign components,
for future proofing allow both styles.
...
test <- rho <- double(nterm+1)
df   <- rep(1L, nterm+1)
u0 <- rep(0, nvar)
if (is.null(fit$naive.var)) vv <- fit$naive.var
else vv <- fit$var
...
rval <- list(table=tbl, x=sort(gtime[event]))
@ 
