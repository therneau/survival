\subsection{Concordance}
 The concordance statistic has gained popularity as a measure of goodness-of-fit
...
\begin{itemize}
  \item Concordant pairs: for a Cox model this will be pairs where a shorter
    ...
    For logistic regression the larger risk score is paired with the larger
    response.
  \item Discordant pairs: the lower risk score has a shorter survival
  \item Tied pairs: there are three common choices
    \begin{itemize}
      \item Kendall's tau-b: any pair where $r_i=r_j$ or $s_i = s_j$ 
        is considered tied.
      \item Somer's D: pairs with $r_i=r_j$ are tied; 
	those with $s_i=s_j$ are considered incomparable. 
        This is the definition of the AUC in logisitic 
        regression and is the common choice for Cox models as well.
      \item Gamma: All ties are treated as incomparable.
    \end{itemize}
...
We can rewrite this as a correlation like value that goes from -1 to 1
with $2C -1 = (C-D)/(C+D +T)$.  
This latter is a bit clearer to code so is used below.
...
organised into a tree at the beginning and node incremented/decremented as
observations are added and removed.  
...
A second short lemma is that the mean rank $\overline{r}$ of these
values has a simple form.
\begin{equation*}
  \overline{r} = (\sum w_i r_i)/(\sum w_i) = \sum w_i/2
\end{equation*}
Thus when adding a new subject $k$ to the tree the grand mean increases by
    $w_k/2$.
...
\begin{description}
  ...
  \item[sum] scratch space, weights for each node of the tree: 
    ...
    a second three for the subset of deaths.
  \item[count] the returned counts of concordant, discordant, tied on x, 
    tied on time, and the variance
  \item[ijn, ijd] vectors containing the infinitesimal jackknife estimate
