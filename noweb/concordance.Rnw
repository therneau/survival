...
This routine gets the X, Y, and other portions for a call.

<<concordance>>=
cord.getdata <- function(fit, newdata=NULL, group=NULL, need.wt, timefix=TRUE) {
    ...
    if (!is.null(newdata)) {
        mf <- model.frame(fit, data=newdata)
        y <- model.response(mf)
        if (!is.Surv(y)) {
            if (is.numeric(y) && is.vector(y))  y <- Surv(y)
            else stop("left hand side of the formula  must be a numeric vector or a survival object")
        ...
        x <- fit$linear.predictors    # used by most
        if (is.null(x)) x <- fit$fitted.values # used by lm
        if (is.null(x)) {fit$na.action <- NULL; x <- predict(fit)}
        rval <- list(y = y, x= x)
    }
    ...
    if (is.null(group)) {
        if (!is.null(specials$cluster)) {
            ...
            tempc <- untangle.specials(Terms, 'cluster', 1:10)
            ord <- attr(Terms, 'order')[tempc$terms]
            rval$group <- strata(mf[,tempc$vars], shortlabel=TRUE) 
        }
        else if (!is.null(fit$call$id)) {
            if (is.null(mf)) mf <- model.frame(fit)
            rval$group <- model.extract(mf, "id")
        }
    }
    else rval$group <- group
    rval
}
@ 
...
<<concordance>>=
concordance.lm <- function(fit, ..., newdata, ymin, ymax, group,
    ...
    if (any(notok)) {
        ...
        #  has one other than "fit", fname otherwise
        indx <- which(notok)
        id2 <- names(Call)[indx+1]
        temp <- ifelse(id2 %in% c("","fit"), fname, id2)
    ...
    cfun$data <- lapply(fits, cord.getdata, newdata=newdata, group=group,
                        need.wt=need.wt, timefix=timefix)
    rval <- eval(cfun, parent.env())
    rval$call <- Call
    rval
}

concordance.survreg <- function(fit, ..., newdata, ymin, ymax, group,
...
cord.work <- function(data, timewt, ymin, ymax, influence=0, ranks=FALSE, 
    ...
    fcall[[1L]] <- as.name("concordance.fit")
    ...
    if (nfit==1) {
        ...
        rval <- eval(fcall, environment(concordance))
    }
    else {
        ...
        flist <- lapply(data, function(d) {
                         ...
                         eval(temp, environment(concordance))
                     })
