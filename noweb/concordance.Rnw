\subsection{Concordance}
 The concordance statistic is the most popular measure of goodness-of-fit
...
\begin{itemize}
  \item Concordant pairs: for a Cox model this will be pairs where a shorter
    ...
    For all other models conconrance is $r_i>r_j$ and $y_i > y>j$.
  \item Discordant pairs: the lower risk score has a shorter survival.
  \item Tied pairs: there are two common choices
    \begin{itemize}
      \item Kendall's $\tau$-b: any pair where 
        $r_i=r_j$ or $s_i = s_j$ is considered tied.
      \item Somers' $d$: pairs with $r_i=r_j$ are tied; 
...
\begin{enumerate}
\item Sort the data by the response $y$ or survival times.  It then 
  suffices to compare each observation only to those that are larger.
  For survival data is all the subjects in the risk set of each event.
...
2.1, which can be found by 
\begin{enumerate}
  \item intialize the total with the number of right hand children of Smith.
  \item proceed up the tree to the root, and add the (parent-child) total 
    each time that the movement is to the right.
\end{itemize}
...
b2 <- function(n, id=1L) {
    if (n==1) id
        ...
        split <- ceiling(n/2)
        c(b2(split-1, 2L*id), id, b2(n-split, 1L + 2L*id))
    }
}

@ 
Referring again to figure \ref{treefig}, \code{btree(13)} yields the vector
\code{8  4  2 10  5 11  1 12  6  3 14 7 15} meaning that the smallest element
will be in position 8 of the tree, the next smallest in position 4, etc.
The code fills in the bottom row somewhat differently than figure, but 
that does not matter in the final code.
