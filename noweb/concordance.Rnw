...
with $2C -1 = (C-D)/(C+D +T)$.  
This latter is a bit clearer to code.
...
which can be written in terms of the weights.
...
of observations.
I had originally avoided this because it appears to be an $O(nd)$
...
Within the tree we keep the sum of weights for each node and its two 
children, along with a parallel triple that totals only the deaths.   
The tree is updated as each subject is added and later deleted, containing
...
A second question variance estimate is based on the Cox model.  
...
A second short lemma is that the mean rank $\overline{r}$ of the
Cox model has a simple form.
\begin{equation*}
