...
$(x_i, x_j)$ as the two risk scores for each pair and $(y_i, y_j)$ the
...
\begin{itemize}
  \item Concordant pairs: for a Cox model this will be pairs where a shorter
    survival is paired with a larger risk score, e.g. $x_i>x_j$ and $y_i < y_j$
  \item Discordant pairs: the lower risk score has a shorter survival
  \item Tied pairs: pairs with $x_i=x_j$ are tied; 
	those with $y_i=y_j$ are considered
  ...
  \item Incomparable pairs: For survival this always includes pairs where the
    ...
    For instance $y_i$ is censored
    at time 10 and $y_j$ is an event (or censor) at time 20.  Subject $i$ may or
...
In Watson and Therneau (in press) we show that a time weighted
concordace statistic can be written as $N/D$ where $N$
is closely related to the Cox model score statistic.
\begin{align}
  N &= \sum_{i=1}^n w_i \delta_i H(t_i) (s_i(t_i) - .5) \label{eq:n} \\
  D &= \sum_{i=1}^n w_i \delta_i H(t_i) \label{eq:d}
\end{align}
where $H(t)$ is a prespecified function of time, $\delta_i=1$ if
the subject is an event, and
$s_i$ is the scaled rank of the observation at that time point,
among all those still at risk at that time.
Let $Y_i(t)=1$ if subject $i$ is currently at risk at time $t$,
then
\begin{align}
  s_i(t) &= \frac{\sum_j w_j Y_j(t) [I(x_j < x_i) + .5I(x_j=x_i)]}
         {\sum w_j Y_j(t)}        \label{eq:rank1}\\
  s_i(t)-.5   &= \frac{\sum_j w_j Y_j(t)[I(x_j < x_i) - I(x_j > x_i)]}
         {2\sum w_j Y_j(t)} \label{eq:rank2}
\end{align}
The mean value of $s$ at any time point is 1/2.
The second equation shows that $s- 1/2$ can be computed using the number of
observations less than $x_i$ minus the number greater.  
(Sort of like one's rank in an organization: the number under you minus the
number above you.)

Common weight functions are
\begin{itemize}
  \item The weighted number at risk $H(t) = n(t) =\sum w_j Y_j(t)$.  This is the
    usual definition of the concordance.
  \item $H(t) = S(t)$ where $S$ is the overall survival function.
  \item $H(t) = S(t)/G(t)$, where $G$ is the estimated censoring distribution.
    This is the estimate proposed by Schemper.
  \item $H(t) = n(t)/ G^2(t)$, proposed by Uno.
\end{itemize}
Since $n(t) \approx n S(t) G(t)$, there is a close relationship between
the Uno and Schemper weights.

In addition to the concordance the program reports back
...
\item To count the number higher and lower we need to rank the subjects
  in the risk set by their scores $x_i$.  This can be done in $O(\log n)$
...
of observations in that node, the total weight for all branches to the left and
the total for all branches to the right
(not shown on graph).  
...
This takes a little less than $\log_2(n)$ steps on average., 
...
organised into a tree at the beginning and then the weights at each node
are decremented as observations are removed.  
...
node $i$ are $2i$ and $2i +1$.  In C code the indices start at 0, the
...
First is an infinitesimal jackknife. 
Consider the usual concordance where $H = \sum w_i$.
From equation \eqref{eq:n} and \eqref{eq:rank2}, at any event
time the influence of each point is
\begin{align*}
  J_k(t) &= \frac{\partial w_i \delta_i(t) H(t_i) (s_i(t_i) - .5)}
                       {\partial w_k}\\
         &= \frac{\partial (w_i \delta_i(t_i))
                           (\sum_j Y_j(t_i) w_j [I(x_j < x_i) - I(x_j > x_i)]/2)
                                  (H(t_i)/ \sum_j w_j Y_j(t_i))} {\partial w_k}\\
         &=  \frac{(H(t_i)}{ \sum_j w_j Y_j(t_i)} \left\{ \begin{array}{cl} 
                -w_i/2 & x_k < x_i \\
                 0   & x_k = x_i, i\ne k\\
                 w_i/2 & x_k > x_i \\
                 s_i(t_i) - .5 & i = k \end{array} \right. \\
         & \phantom{=} + w_i( s_i(t_i) - .5) 
                       \frac{\partial H(t_i)/ \sum_j w_j Y_j(t_i)}{\partial w_k}
\end{align*}
For the ordinary concordance $H(t) = \sum w_j Y_j(t)$ and the final
results simplifies to a single term.
The total influence for any subject $k$ is $J_k=\sum_t J_k(t)$ and the
estimated variance of the concordance will be $\sum w_k J_k^2$.

Adding each subject's influence to their total at each event time is an
$O(n^2)$ operation, which we want to avoid.  
Looking again at figure \ref{treefig} create auxillary variables
of [[infleft]] and [[infright]] containing the influence for
all observations at each node + all its children below and to the left, or
the node and all children to the right, respectively.
Imagine an event with a risk of 19 in the tree.
Then -$w_i/2$ needs to be added to [[infleft]] for nodes 16 and 8, 
and $w_i/2$ to [[infright]] for nodes 22 and 25.  
Walking the tree to do this addition requires $O(n \log_2n)$ steps.
When an observation leaves the risk set we walk the tree and add
to it's influence: [[infleft]] at each node where it will proceed left and
in the final matching node, [[infright]] at each node where it will proceed 
to the right or the in the matching node.

If we treat $H(t)/\sum w_i(t)$ as a constant at each time point then
it becomes a simple multipier of the influence values added to the tree.
It is not quite right to treat it as a constant, but that's what we do
for the present.

The second variance come from a Cox model.
We recognize equation \eqref{eq:n} as the score statistic from a time
weighted Cox model with the scaled rank $s(t)$ as the covariate.
The Cox variance for each term is 
\begin{equation}
 V(t) = \sum_{i \in R(t)} Y_i(t) w_i (s_i(t) - .5)^2 / 
                   \sum_{i \in R(t)} Y_i(t) w_i 
\end{equation} 
As an update formula for $V$ consider the variance of the
unscaled ranks.
The key formula is below,
true for any set of values $x$ and centering constant $c$.
\begin{equation*} 
  \sum w_i(x_i - \overline{x})^2 = \sum w_i(x_i-c)^2 - 
...
tree of rank lower than $x_k$, the item we are about to add,
...
We have 
\begin{align*}
  \mu_\ell &= \sum_{x_i<x_k} w_i/2 \\
  \mu_u & =  \sum_{x_i<=x_k} w_i + \sum_{x_i \ge x_k} w_i/2
\end{align*}  

For items of lower rank than $x_k$, none of their ranks will change
...
for $x_i < x_k$ below)
\begin{align}
   \sum_{i<k} w_i(x_i - \mu_g)^2 &= \sum_{i<k} w_i(x_i - \mu_\ell)^2 +
      (\sum_{i<k} w_i)(\mu_\ell - \mu_g)^2 \nonumber \\
   \sum_{i<k} w_i(x_i - \mu_n)^2 &= \sum_{i<k} w_i(x_i - \mu_\ell)^2 +
      (\sum_{i<k} w_i)(\mu_\ell - \mu_n)^2 \nonumber \\
   \sum_{i<k} w_i(x_i - \mu_n)^2 -   \sum_{i<k} w_i(x_i - \mu_g)^2 &=
...
For items of larger rank than $x_k$, all of the ranks increase by $w_k$
...
\begin{description}
  ...
  \item[timewt] time weighting, one value for each event time
 ...
 \end{description}
My general rule is that the R object will be called wt2 and the C pointer
to its contents called wt; similarly for other variables.

<<concordance1>>=
#include "survS.h"
SEXP concordance1(SEXP y2, SEXP wt2,  SEXP indx2, SEXP timewt2, SEXP ntree2) {
    int i, j, k, index;
    int child, parent;
    int n, ntree;
    double *time, *status;
    ...
    PROTECT(influence2 = allocVector(REALSXP, n));
    count = REAL(count2);  /* return vector*/
    influence = REAL(influence2);
    
    nwt = (double *) R_alloc(4*ntree, sizeof(double));
    twt= nwt + ntree; 
    leftinf = twt + ntree;
    rightinf= leftinf + ntree;
    for (i=0; i< ntree; i++) {
        nwt[i]= 0.0;  /* weights for all those tied at this risk */
        twt[i]= 0.0;  /* node + kids */
	leftinf[i] = 0.0;
	rightinf[i]= 0.0;
    }
...
@ 
Because of tied death times the computation is done in three stages.
First is to add up the all of the intermediate portions for this death
time, over all of the deaths.  It turns out that the
effect of the tied deaths on each other always adds up to exactly zero, so
we can compute all of the updates and \emph{then} add them into the 
tree of ranks.
The second step is to update the current variance of the ranks, at the
same time updating the tree.
Last we add the intermediate terms into the total statistic.
...
node.
The tree contains the data for everyone whose survival is larger
than the time currently under review, so starts with all weights
equal to zero, we then add subjects to the risk set one by one.  
...
\begin{itemize}
  \item If it is a death time, we need to process all the deaths tied at
	...
	for all pairs $i,j$ of tied times.
      \item The addition to tied-on-risk will be the weight of this 
        observation times
        ...
        (At the moment of calculation only the longer times have been added
        into the tree).
...
for (i=n-1; i>=0; ) {
	...
	rank =0;  /* sum of ranks-mean for deaths at this point */
        for (j=i; j>=0 && status[j]==1 && time[j]==time[i]; j--) {
            ndeath += wt[j];
            index = indx[j];
	    rank += (nwt[index] - twt[0])/2;  
		...
		if (index & 1) {   /* I am the left child */
		    ...
		    rank += wt[j] * (twt[parent] - twt[index]);
		}
		index = parent;
	    }
	}
    ...
    if (ndeath >0) { /* add to the statistic */
    printf("vss=%f, rank=%f, twt=%f, timewt=%f\n", vss, rank, twt[0], *timewt);
       temp = *timewt / twt[0];
	count[4] += temp*temp * ndeath * vss;
	count[5] += *timewt * rank;
	timewt++;
	}
    }
...
survConcordance <- function(formula, data,
    ...
    count <- survConcordance.fit(Y, x, strat, wt, timewt)
    if (is.null(strat)) {
...
<<survConcordance.fit>>=
survConcordance.fit <- function(y, x, strata, weight, timewt) {    
    <<btree>>

    docount <- function(stime, risk, wts, twt) {
        if (attr(stime, 'type') == 'right') {
            ord <- order(stime[,1], -stime[,2])
            ux <- sort(unique(risk))
            n2 <- length(ux)
            index <- btree(n2)[match(risk[ord], ux)] - 1L
             .Call('concordance1', stime[ord,], 
                   as.double(wts[ord]), 
                   as.integer(index), 
                   rev(twt),
    ...
    if (is.na(match(timewt, c("n", "S", "n/G", "S/G", "n/G2"))))
        stop("invalid option for timewt")
    if (attr(y, "type")=="counting" && !(timewt %in% c("n", "S")))
        stop("invalid weighting option for counting process survival")

    # create the weighting vector, start with a call to survfit
    if (missing(strata) || length(strata)==0) {
        stemp <- strata(rep(1, nrow(y)))
        sfit <- survfitKM(stemp, y, casewt=weight, se.fit=FALSE)
        dtimes <- (sfit$n.event>0)
        if (timewt == "n") twt <- sfit$n.risk[dtimes]
        else if (timewt == "S") twt <- sfit$surv[dtimes]
        else {
            gfit <- survfitKM(stemp, Surv(y[,1], 1-y[,2]), 
                              casewt=weight, se.fit=FALSE)
            etime <- sfit$time[dtimes]
            gg <- summary(gfit, time=etime, extend=TRUE)
            if (timewt== "n/G") twt <- sfit$n.risk[dtimes]/gg$surv
            else if (timewt=="S/G") twt <- sfit$surv[dtimes]/gg$surv
            else twt <- sfit$n.risk[dtimes]/gg$surv
        }
    }
    else { #stratified case
        sfit <- survfitKM(strata, y, wt, se.fit=FALSE)
    }
  
    storage.mode(y) <- "double"
    if (missing(strata) || length(strata)==0) {
        count <- docount(y, x, weight, twt)
