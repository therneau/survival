...
concordancefit <- function(y, x, strata, weights, ymin=NULL, ymax=NULL, 
 ...
 survival object, or an orderable factor")
    ...
    # A trick: if A and B are in different strata, we get the same concordance
    #  if we put them in 1 stratum, and make their times not overlap.  This
    #  works if timewt is 'n' or 'I'.
    ...
    if (timewt %in% c("n", "I") && nstrat > 1 && !keepstrata) {
        # mess up the times
        stemp <- as.numeric(as.factor(strata)) -1
        if (ncol(y) ==3) {
            delta <- max(y[,2]) - min(y[,1])
            y[,1] <- y[,1] + stemp*delta
            y[,2] <- y[,2] + stemp*delta
        }
        else {
...
concordance.lm <- function(object, ..., newdata, cluster, ymin, ymax, 
    ...
    cfun$data <- lapply(fits, cord.getdata, newdata=newdata, cluster=cluster,
                        need.wt=need.wt, timefix=timefix, keepstrata=keepstrata)
