...
concordance.coxph <- function(fit, ..., newdata, ymin, ymax, 
    ...
    timewt <- match.arg(timewt)
    # the below is used for labels or errors
    temp <- match.call()[-1]
    mname <- as.character(temp[names(temp) %in% c("fit", "")])
    ...
    if (!all(iscox)) {
        stop(mname[!iscox], "is not a coxph object")
    }
    ...
    dofit <- function(fit, newdata, inf=1) {
       ...
       if (!is.null(specials$strata)) {
        ...
        temp <- concordance.fit(Y, X, strata.keep, weights, ymin, ymax, timewt,
                        group, influence=inf, reverse=reverse, ranks=ranks)
    ...
    if (nfit ==1) {
        ...
        if (ranks) {
            ...
            rdat <- data.frame(fit= rep(mname, sapply(temp, nrow)),
                               do.call(rbind, temp))
            row.names(rdat) <- NULL
            rval$ranks <- rdat
        }
