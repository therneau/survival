...
\item Rearrange the counting so that we do it by death times.
  For each death we count the number of subjects in the risk set whose
...
using 13 risk scores.  For each node the left child and all its descendants
have a smaller value tnan the parent, the right child and all its
descendents have a larger value.
Each node in figure \ref{treefig} is also annotated with
the number of observations in that node and its children (not shown).  
...
organised into a tree at the beginning and weights are changed to zero
...
For integer case weights create an expanded data set with weights all
of 1.  
...
and discordant pairs contributed at that death time (using the
...
Since $\overline{r} = \sum(w_i)/2 the grand mean increases by
    $w_k/2$.
Let $\mu_l$ be the mean rank for all observations currently in the
list of rank lower than $r_k$, the item we are about to add,
$\mu_u$ be the mean for all those above in rank, 
...
update formula on the third line below.
\begin{align*}
   \sum_{i<k} w_i(r_i - \mu_g)^2 &= \sum_{i<k} w_i(r_i - \mu_l)^2 +
      (\sum_{i<k} w_i)(\mu_l - \mu_g)^2 \\
   \sum_{i<k} w_i(r_i - \mu_n)^2 &= \sum_{i<k} w_i(r_i - \mu_l)^2 +
      (\sum_{i<k} w_i)(\mu_l - \mu_n)^2 \\
   \sum_{i<k} w_i(r_i - \mu_n)^2 -   \sum_{i<k} w_i(r_i - \mu_g)^2 &=
    (\sum_{i<k} w_i) [(\mu_l - \mu_n)^2 - (\mu_l - \mu_g)^2]
\end{align*}

For items of larger rank than $r_k$, all of the ranks increase by $w_k$
when we add the new item, thus the sum of squares within the group
is unchanged.  The same derivation as above gives an update of
\begin{equation*}
  \sum_{i>k} w_i(r_i - \mu_n)^2 -   \sum_{i>k} w_i(r_i - \mu_g)^2 =
    (\sum_{i>k} w_i) [(\mu_l+ w_k - \mu_n)^2 - (\mu_l - \mu_g)^2] 
\end{equation*}
...
For updates then we need the four means with respect to the observation
being added.
...
SEXP concordance1(SEXP y, SEXP wt2,  SEXP indx2, SEXP ntree2) {
    ...
    double var, myrank;
    int ndeath;   /* number of tied deaths at this point */
    double temp;
...
for (i=n-1; i>=0; ) {
    ...
    for (; i>j; i--) {
	index = indx[i];
        var += wt[i]* wt[i]* twt[0]/4.0;
	nwt[index] += wt[i];
        twt[index] += wt[i];
printf("var+= %6.2f", var);
        myrank = nwt[index]/2;
        child = 2*index +2;  /* left child */
        if (child < ntree) myrank += twt[child];
	while (index >0) {
	    parent = (index-1)/2;
            twt[parent] += wt[i];
            if (!(index&1)) /* I am a right child */
                myrank += (twt[parent] - twt[index]);
            index=parent;
            }
        temp = myrank - twt[0]/2;
        var += wt[i]* temp* temp;
	}
    printf(" myrank=%4.1f, twt=%4.1f, term2=%6.2f, sum=%6.2f, var=%8.4f\n", 
           myrank, twt[0], wt[i]*temp*temp, var, var/twt[0] );
    count[4] += ndeath * var/twt[0];
    }
@ 
