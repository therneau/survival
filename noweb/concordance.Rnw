...
\begin{itemize}
  ...
  \item Tied pairs: there are three common choices
    \begin{itemize}
      \item Kendall's tau: any pair where $x_i=x_j$ or $y_i = y_j$ is considered
        tied.
      \item AUC: pairs with $x_i=x_j$ are tied; 
...
In Therneau, Schemper and Kremers (in press) we show that a time weighted
concordace statistic can be written as $N/D$ where each is a
sum that is closely related to the Cox model score statistic.
\begin{align}
...
Let $Y_i(t)=1$ is subject $i$ is currently at risk at time $t$,
then
\begin{align*}
  s_i(t) &= \sum w_j Y_j(t) [I(x_j < x_i) + .5I(x_j=x_i)] / \sum w_j Y_j(t) \\
  s_i(t)-.5   &= \sum w_j Y_j(t)[I(x_j < x_i) - I(x_j > x_i)]/ 2\sum w_j Y_j(t)
\end{align*}
The mean value of $s$ at any time point is 1/2.
The second equation shows that $s- 1/2$ can be computed using the number of
observations less than the point minus the number above.  
...
We recognized equation \eqref{eq:n} as the score statistic from a time
...
time, overall all of the deaths.  It turns out that the
