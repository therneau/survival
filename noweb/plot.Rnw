...
I found some problems where plot.survfit, lines.survfit, and points.survfit
...
lines.survfit <- function(x, conf.int,
                          mark.time=TRUE,
                          mark=3, col=1, lty=1, lwd=1,
			  xscale=1, yscale=1, 
...
points.survfit <- function(x, cex=1, xscale=1, yscale=1,
                           xmax, fun, ...) {
    <<plot-transform-ms>>
    <<plot-common-args>>
...
if (is.null(x$upper)) conf.int <- FALSE    
...
# Default is to add confidence bands if there is only 1 curve, otherwise not
ncurve <- nstrat * ncol(ssurv)
if (missing(conf.int)) {
    if (ncurve==1) conf.int <-TRUE
    else conf.int <- FALSE
}
...
Do the setup of the mark.time parameters and for the firstx argument.
...
<<plot-first-arg>>=
if (missing(firsty) & x$type=="mright" || x$type=="mcounting") {
    firsty <- NA
    firstx <- min(stime)
}
    ...
    if (!is.null(x$start.time)) 
        ...
        if (isplot && 
            (logx || (!missing(fun) && is.character(fun) && fun=='cloglog')))
...
Here is the rest of the setup for the plot routine, having to do with
setting up axes.
<<plot-plot-setup1>>= 
if (is.logical(log)) {
    logy <- log
...
These functions do the actual drawing.
...
browser()
    for (j in 1:ncol(ssurv)) {
        ...
        if (conf.int) {
            lines(dostep(xx, slower[who,j]), lty=lty[c2], col=col[c2],
                  lwd=lwd[c2])
            c2 <- c2 +1
            lines(dostep(xx, supper[who,j]), lty=lty[c2], col=col[c2],
                  lwd= lwd[c2])
            c2 <- c2 + 1
        }
    }
