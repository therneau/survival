...
If a log axis is specified for y, then turn any 0 values into NA.
This doesn't change the plot, but it does stop a warning message.

<<plot-firstx>>=
#if (xlog) stime <- ifelse(stime >0, stime, NA)
#if (ylog) ssurv <- ifelse(ssurv >0, ssurv, NA)
