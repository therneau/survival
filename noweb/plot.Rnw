...
stime <- x$time
if (inherits(x, "survfitms")) fun <- "identity"
if (!missing(fun) && is.character(fun) && fun=="cumhaz") {
    ...
    if (!is.null(x$pstate)) {
        ssurv <- as.matrix(x$pstate)
        if (ncol(ssurv)>1 && any(x$states== noplot)) {
            keep <- (x$states != noplot)
            ssurv <- ssurv[,keep, drop=FALSE]
            if (conf.int) {
                supper <- supper[,keep, drop=FALSE]
                supper[,keep, drop=FALSE]
            }
        }
    }
...
if (length(pch)==1 && is.character(pch)) pch <- strsplit(pch, "")[[1]]
if (missing(xlim)) {
    if (missing(firstx)) xmin <- min(time) else xmin <- firstx
    if (missing(xmax))   xmax <- max(time)
    xlim <- c(xmin, xmax)
}
@ 
