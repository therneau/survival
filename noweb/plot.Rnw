...
lines.survfit <- function(x, type='s', 
    ...
    do.clip <- options("survival")
    if (!is.null(xx <- do.clip$plotclip)) clip(xx[1], xx[2], xx[3], xx[4])

    <<plot-functions>>
    <<plot-draw>>
    if (!is.null(xx <- do.clip$plotrestore)) clip(xx[1], xx[2], xx[3], xx[4])
}
...
if (!conf.int) supper <- slower <- NULL
...
if (cumhaz) {  # plot the cumulative hazard instead
    ...
    i <- (x$states != noplot)
    if (!any(i))  i <- seq(along=x$states)  # if all states==noplot, keep all
...
# check consistency
if (!missing(xlim)) {
    if (!missing(xmax)) stop("cannot have both xlim and xmax arguments")
...
temp <- range(stime[is.finite(stime)], na.rm=TRUE)
if (xaxs=='S') {
...
if (ylog) {
    if (!is.null(supper))
        tempy <- range(c(slower[is.finite(slower) & slower>0], 
                         supper[is.finite(supper)]))
    else tempy <-  range(ssurv[is.finite(ssurv)& ssurv>0])
    if (tempy[2]==1) tempy[2] <- .99   # makes for a prettier axis
    if (any(c(ssurv, slower) ==0)) {
            tempy[1] <- tempy[1]*.8
...
# Set limits
if (!missing(xlim)) tempx <- xlim
if (!missing(xmax)) tempx[2] <- xmax
if (!missing(firstx)) tempx[1] <- firstx
if (!missing(ylim)) tempy <- ylim
...
if (resetclip) {
  ...
  options(survival = list(plotclip=c(tempx, tempy), plotreset=par('usr')))
}
@ 
