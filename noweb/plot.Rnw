...
plot.survfit<- function(x, conf.int,  mark.time=FALSE,
			...
			fun, xlab="", ylab="", xaxs='S', 
                        conf.times, conf.cap=.01, conf.offset=.02, ...) {
...
for (i in unique(stemp)) {  #for each strata
    ...
    for (j in 1:ncol(ssurv)) {
        ...
        if (conf.int && !is.null(conf.times)) {
            ...
            x2 <- conf.times + temp.offset
            templow <- approx(x2, c(firsty[i,j], slower[who,j]), conf.times,
                              method='constant', f=1)$y
            temphigh<- approx(x2, c(firsty[i,j], supper[who,j]), conf.times,
                              method='constant', f=1)$y
            ...
            temp.offset <- temp.offset + conf.offset* diff(par("usr")[1:2])
        }
        c1 <- c1 +1
        c2 <- c2 +1
