...
summary.pyears <- function(object, header=TRUE, call=header,
    ...
    if (!is.null(object$data)) {
        ...
        dname <- lapply(tdata, function(x) levels(x)) # dimnames
    ...
    if (totals) {
        object$n <- pytot(object$n, na=TRUE)
...
if (vertical) {
    ...
    plist <- lapply(plist, pformat, digits=digits, nastring, ...) # make it character
    ...
    newmat <- pybox(plist, length(plist[[1]]), digits=digits, nastring,...)
...
if (length(dd) > 2) 
    newmat <- pybox(plist, c(dd[1],dd[2], prod(dd[-(1:2)])), digits=digits, 
                    nastring, ...)
else  newmat <- pybox(plist, dd, digits=digits, nastring, ...)
...
if (length(dd) > 2) {
    ...
    for (i in 1:nrow(outer.label)) {
        paste(":", paste(temp, outer.label[i,], sep="=")) # A table caption
        pyshow(newmat[,,i,], tname, rowname, colname, vline)
    }
}
else {
    newmat <- pybox(plist, dd, digits=digits, nastring, ...)
...
pformat <- function(x, digits, nastring, ...) {
    # This is only called for single index tables, in vertical mode
    # Any matrix will be a confidence interval
    if (is.matrix(x)) 
        ret <- paste(ifelse(is.na(x), nastring,
                            format(x[,1], digits=digits, ...)), "-", 
                     ifelse(is.na(x), nastring, 
                            format(x[,2], digits=digits, ...)))
    else ret <- ifelse(is.na(x), nastring, format(x, digits=digits, ...))
}
@ 
...
<<pyears-charfun>>=
pybox <- function(plist, dd, digits, nastring, ...) {
    ...
    if (any(real)) { # floating point
        ...
        final[,real] <- ifelse(is.na(temp), nastring, format(temp, digits,...))
    }
...
pyshow <- function(dmat, labels, rowname, colname, vline) {
    ...
    colwidth <- c(max(nchar(rowname), nchar(labels[1])),
                  rep(max(nchar(dmat[1,1,])), length(colname)))
...
pytot <- function(x, na=FALSE) {
    ...
    if (length(dd) ==1) {
        ...
        if (na) sum1 <- sum2 <- NA
            ...
            sum1 <- apply(x, indx[-1], sum)    # row sums
            sum2 <- apply(x, indx[-2], sum)    # col sums
            sum3 <- apply(x, indx[-(1:2)], sum) # total sums
            }
