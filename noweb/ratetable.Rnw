...
\title{Ratetables in the Surival Package}
...
This is a short introduction to how the United States ratetables found in the 
...
The log(hazard) for the later ages is extroplated from ages 70-99 using a
smoothing spline. 
...
subdirectory \texttt{noweb/rates} containing the raw data for the rate tables.
...
\begin{description}
  ...
  \item[type] A vector giving the type of each dimension. One of
	...
	allows the routine to be more intellegent in handling the multiple
...
define a rate table.  
Users making thier own table ``mytable''should always follow the exercise with
