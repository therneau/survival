...
Cox model. 
The actual \code{surv} object contains the time points for the first stratum,
...
In a multi-state object, cumulative hazard functions have a different last
dimension; any use of the state subscript will cause them to be dropped.
...
"[.survfit" <- function(x, ... , drop=TRUE) {
    ...
    if (ndots > 2) 
        stop("too many subscripts for survfit object")
    ...
    if (ndots== 1 && length(dtype) > 1) {
        # one subscript given for a two dimensional object
        #  first check that the subscript is legal
        index <- 1:prod(dd)
        itemp <- matrix(1:index, nrow=dd[1])
        ...
        # turn it into a long integer subscript
        indx1 <- split(1:sum(x$strata), 
                       rep(1:length(x$strata), x$strata))
        indx1 <- ulist(indx1[ii])   # rows of the surv object
        indx2 <- rep(jj, x$strata[ii])
        
        # return with each curve as a separate strata
        x$n <- x$n[ii]
        x$time   <- x$time[indx1]
        x$n.risk <- x$n.risk[indx1]
        x$n.event<- x$n.event[indx1]
        x$n.censor<- x$n.censor[indx1]
        if (!is.null(x$n.enter)) x$n.enter <- x$n.enter[indx1]
        k <- cbind(indx1, indx2)
        x$surv <- x$surv[k]
        if (!is.null(x$std.err)) x$std.err <- x$std.err[k]
        if (!is.null(x$upper)) x$upper <- x$upper[k]
        if (!is.null(x$lower)) x$lower <- x$lower[k]
        if (!is.null(x$cumhaz)) x$cumhaz <- x$cumhaz[k]
        if (!is.null(x$std.chaz)) x$std.chaz <- x$std.chaz[k]
        temp <- x$strata[ii]
