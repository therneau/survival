...
dim.survfit <- function(x) {
    if (!is.null(x$dim)) x$dim    # a survfit version 3 object
...
"[.survfit" <- function(x, i, j, k, drop=TRUE) {
    ...
    if (missing(..3)) k<- NULL  else k <- ..3

    dd <- x$dim
    if (is.null(dd)) {
        # we will have either 1 or 2 dimensions
        if (is.matrix(
        if (length(dd) <3 && !is.null(k))  stop("incorrect number of subscripts")
            if (!(is.null(k) && is.null(j)
