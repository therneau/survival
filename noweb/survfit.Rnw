...
Instead use a transformation $y = f(p)$ for which the standar error is
...
log-log & $\log(-\log(p))$ & $1/\left[ p \log(p) \right]$ &
   \exp(-\exp(y)) $  \\
logit & $\log(p/1-p)$ & $1/p + 1/(1-p)$ & $1- 1/\left[1+ \exp(y)\right]$ \\
\end{tabular} \end{center}

Some of the underlying routines compute the standard error of $p$ and some
the standard error of $\log(p)$.  The selow argument is used for the 
...
survfit_confint <- function(p, se, logse=TRUE, conf.type, conf.int,
    ...
    if (conf.type=='plain') {
        ...
        xx <- ifelse(p==0, 1, p)  
        if (logse) se2 <- zval* se * pval
        else       se2 <- zval* se * pval / xx
        temp1 <- ifelse(p==0, NA, exp(log(xx) - se2*scale))
        temp2 <- ifelse(p==0, NA, exp(log(xx) + se2))
        list(lower=pmax(temp1,0), upper= pmin(temp2, 1))
    }
    else if (conf.type=='log-log') {
        avoid <- (p==0 | p==1)
        xx <- ifelse(avoid, NA, p)
        if (logse) se2 <- zval * se/log(xx)
        else       se2 <- zval * s2/(xx * log(xx))
        temp1 <- exp(-exp(log(-log(xx)) + se2*scale))
        temp2 <- exp(-exp(log(-log(xx)) - se2))
        list(lower = pmax(1-temp1, 0), upper = pmin(1-temp2, 1))
    }
    else if (conf.type=='logit') {
        avoid <- (p==0 | p==1) #special cases
        xx <- ifelse(avoid, NA, p)  # avoid log(0) messages
        if (logse) se2 <- zval * se *(1 + p/(1-p))
        else se2 <- zval * se * (1/p + 1/(1-p))
