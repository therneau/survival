...
if (maxiter>1 && ((newlk -loglik[1])/ loglik[1]) < -eps) {
    ...
    ** The last iteration above was worse than one of the earlier ones, by
    **  more than roundoff error.  
    ...
    ** It will happen very rarely that we run out of iterations, and even
    **  less often that the Newton-Raphson is getting worse.
    ** If maxiter=1, we return 1 iteration whether it worked or not. 
    */
    for (i=0; i<nvar; i++) beta[i] = oldbeta[i];
    <<agfit4-addup>>
    }
if (*iter>0) rank = cholesky2(imat, nvar, tol_chol);
