...
Observations that are not part of any risk set can goof up my algorithm
(nrisk will be negative), so we do a first pass to mark them in the
\code{keep} vector.  For most sensible data sets all elements of 
\code{keep} will be positive, but survSplit can create observations that 
are not used.
...
At one point I spent a lot of time worrying about $r_i$ value that are too
...
\begin{center}
  \begin{tabular}{ccccc}
    ...
    \end{tabular} 
\end{center}
The code moves from large times to small, so the first risk set considered has
...
This leads to adding in subjects 1 and 2 before 3 and 4 are removed;
for a coefficient greater than about .5 this leads to a loss of all of
all significant digits.  
The defense is to remove subjects from the risk set as early
as possible, and defer additions as long as possible.
...
log-likelihood trends to an asymptote and there is a race between four
conditions: convergence of the loglik, an overflow of exp for one or
more observations, singularity of the variance matrix, or an invalid
log-likelihood.
The last of these occurs when the denominator becomes $<0$ due to
round off so that log(denom) is undefined.  
...
batches:
\begin{enumerate}
  \item Find the time of the next death (within this stratum). If there are
    none zero the totals and move ahead to the next.
  \item Remove all observations in the stratum with time1 > dtime.
    What survSplit was used to create a data set, this will often remove all.
...
for (istrat=0; istrat < nstrat; istrat++) {
    ...
    meaneta =0;
    while (person < strata[istrat]) {
        /* find the next death time in this stratum */
	for (k=person; k<strata[istrat]; k++) {
	    p = sort2[k];
	    if (event[p] == 1) {
		dtime = tstop[p];
        ...
        if (k == strata[istrat]) {
            /* no more deaths in this stratum, we can move on */
            person = k;
            indx1 = k;
            break;
        }
	...
	** denom2, a2, cmat2, and deaths count only the deaths
	*/
	denom2= 0;
	meanwt =0;
	deaths=0;    
	...
	for (; person<strata[istrat]; person++) {
	    p = sort2[person];
	    if (tstop[p] < dtime) break; /* do not add */
...
<<fixeta>>=
/* 
** If the average eta value is out of hand, fix it.
...
**  hopeless: some observations have effectively 0 weight.  Keeping
**  the mean sensible suffices to keep the max in check.
*/
if (fabs(meaneta/nrisk) > 200) {  
...
/*    printf("fail=%d, loglik=%e, change=%e\n", fail, newlk,
           1-(loglik[1]/newlk)); */
    if (fail ==0 && halving ==0 &&
        fabs(1-(loglik[1]/newlk))<= eps){ /* all done */
	<<agfit4-finish>> 
    }
	...
	**  If this is the last iter, recompute the information matrix 
	**    for that value, otherwise do step halving
	*/
	if (*iter == maxiter-1) {
