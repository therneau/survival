...
We use a cutpoint of $\beta *{\rm std}(x) < 20$, where the standard
deviation is the average std of $x$ within a risk set.
The rationale is that exp(20) is greater than the
current US population, so such a coefficient corresponds to a between
...
for (person=0; person<nused;) {
	...
	** If the average eta value has gotton out of hand, fix it.
        ...
        ** case-cohort papers suggests an offset of -100, so use that
	** to guide our cutoff.
	*/
	if (abs(meaneta/nrisk) > 105) {  
...
[[itemp==1]] is not strictly necessary: if there were deaths=$k$ tied
