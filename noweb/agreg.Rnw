...
*sctest=0;
for (i=0; i<nvar; i++) *sctest +=  u[i]*a[i];
...
for (*iter=1; *iter<= maxiter; (*iter)++) {
    ...
    else fail = isnan(newlk) + isinf(newlk); 

    if (maxiter >1 && (fail >0 || newlk < loglik[1])) {
        /* 
...
if (*iter > maxiter && maxiter > 1) {
    ...
    **  attempted value. We've tossed that old value away, so recompute it.
