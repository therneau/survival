...
The fit routine is called by the coxph function with arguements
\begin{description}
...
agreg.fit <- function(x, y, strata, offset, init, control,
    ...
    agfit <- .Call(y, x, newstrat, weights, offset,
                   sort1, sort2, as.integer(method=="efron"),
                   as.integer(maxiter), 
...
SEXP agfit4(SEXP surv2,      SEXP covar2,
	    SEXP  strata2,   SEXP weigths2,  SEXP offset2,
    ...
    tol_chol = asREAL(tolerance);
    
    /*
    ** scratch space
    **  nvar: mean, a, a2, newbeta, maxbeta
    **  nvar*nvar: cmat, cmat2
    **  n: score, eta
    */
    score = (double *) R_ALLOC(2*n + 5*nvar + 2*nvar*nvar, sizeof(double));
    eta = score +n;
    mean = eta + n;
    a = mean + nvar;
    a2 = a +nvar;
    maxbeta = a2 + nvar;
    newbeta = maxbeta + nvar;
    ...
    */
    ...
    cmat = dmatrix(newbeta+nvar,   nvar, nvar);
    cmat2= dmatrix(newbeta+ 2*nvar*nvar, nvar, nvar);
    ...
    */
    ...
    iter = INTEGER(iter2);
    PROTECT(rlist = mkNamded(VECSXP, outnames));
    nprotect += 8;
    SET_VECTOR_ELT(rlist, 0, beta2);
    SET_VECTOR_ELT(rlist, 1, imat2);
    SET_VECTOR_ELT(rlist, 2, means2);
    SET_VECTOR_ELT(rlist, 3, u2);
    SET_VECTOR_ELT(rlist, 4, loglik2);
    SET_VECTOR_ELT(rlist, 5, sctest2);
    SET_VECTOR_ELT(rlist, 6, flag2);
    SET_VECTOR_ELT(rlist, 7, iter2);
    ...
    */
    ...
    ndeath =0;
    for (i=0; i<nused; i++) ndeath += status[i];
...
a small random amount then each of these threes would be in the first risk set,
...
Because of the exponential $r_i = exp(\eta_i)$ for this step
...
We cut this at 20.  The rationale is that exp(2) is greater than the
current US population, so corresponds to a relative risk larger
than any imaginable.
...
for (person=0; person<nused;) {
	...
	** If the average eta value has gotton out of hand, fix it.
        ...
        ** but don't want to take action very often.  One of the
        ** case-cohort papers uses an offset of -100, so use that
	** as a lower limit guide.
	*/
	if (abs(meaneta/nrisk) > 110) {  
	    meaneta = meaneta/nrisk;
	    for (i=0; i<n; i++) eta[i] -= meaneta;
	    temp = exp(-meaneta);
	    denom *= temp;
	    for (i=0; i<n; i++) {
		a[i] *= temp;
		for (j=0; j<n; j++) cmat[i][j]*= temp;
	    }
	    meaneta =0;
	}
...
for (i=0; i<nvar; i++) 
    maxbeta[i] = 20* sqrt(imat[i][i]/ndeath);
...
for (*iter=1; iter<= maxiter; iter++) {
    ...
    *flag = cholesky2(imat, nvar, *tol_chol);
	...
	if (newlk < loglik[1])   {    /*it is not converging ! */
		...
		else (beta[i] < -maxbeta[i]) beta[i] = -maxbeta[i];
	    }
	}
    }  
