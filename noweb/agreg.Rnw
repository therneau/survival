...
agreg.fit <- function(x, y, strata, offset, init, control,
    ...
    agfit <- .Call(Cagfit4, 
                   y, x, newstrat, weights, offset,
                   init, sort.start, sort.end, 
...
SEXP agfit4(SEXP surv2,      SEXP covar2,    SEXP strata2,
    ...
    int *sort1, *sort2, method, maxiter;
    int *strata;
    ...
    for (i=0; i<nused; i++)
        printf("input %d, start, stop event= %f %f %f\n", 
               i, start[i], stop[i], event[i]);
    /*
    ** scratch space
...
for (person=0; person<nused;) {
    ...
    if (event[p]==0){
        ...
        denom += risk;
printf("person=%d, eta=%f, weights=%f, risk=%f, denom=%f\n",
       p, eta[p], weights[p], risk, denom);
        for (i=0; i<nvar; i++) {
    	a[i] += risk*covar[i][p];
    	for (j=0; j<=i; j++)
	...
	*/
	if (abs(meaneta/nrisk) > 110) {  
	    ...
	    for (i=0; i<nvar; i++) {
		a[i] *= temp;
		for (j=0; j<nvar; j++) cmat[i][j]*= temp;
	    }
	    meaneta =0;
	}
        ...
        */
        ...
        deaths=0;
	printf("k=%d, strata=%d\n", person, strata[istrat]);
        for (k=person; k<strata[istrat]; k++) {
	    p = sort1[k];
	    if (stop[p] < time) break;
	    risk = exp(eta[p]) * weights[p];
	    denom += risk;
printf(" at the death, p= %d, denom=%f\n", p, denom);
	    for (i=0; i<nvar; i++) {
		a[i] += risk*covar[i][p];
		for (j=0; j<=i; j++)
		    cmat[i][j] += risk*covar[i][p]*covar[j][p];
...
printf("denom=%f, d2=%f, newlk=%f, eta=%f\n", denom, d2, newlk, eta[p]);
		for (i=0; i<nvar; i++) {
