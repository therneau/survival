...
agreg.fit <- function(x, y, strata, offset, init, control,
    ...
    # Find rows to be ignored
    event <- y[,3] > 0
    dtime <- sort(unique(y[event, 2]))
    indx1 <- findInterval(y[,1], dtime, left.open=TRUE)
    indx2 <- findInterval(y[,2], dtime) 
    indx3 <- findInterval(y[,2], dtime, left.open=TRUE)
    # indx2 != indx3 for any obs that ends on a death time, they are by
    #  definition good
    # indx1 != indx2 for any obs that spans an event time
    ignore <- (indx2 == indx3 & indx1 == indx2)
    nused  <- sum(!ignore)
