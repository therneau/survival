...
batches: an outer one that goes to the next death time and inner ones that
walk through rows at that time.  Censored additions to the risk set
can happen one at a time.

It is only necessary to remove subjects from the risk sets at each
unique death time.
When a user has a tt() term this can generate data with disjoint time sets,
i.e., all the subjects current at risk are removed a select times and new
ones added.  The code below tries to be efficient for this by first checking,
if everyone was removed before doing the subtractions, and doing this before
adding new subjects to the risk set.  This latter is why we sort events
first within a tied time, so that it is triggered before adding anyone
censored at exactly that time.
<<agfit4-addup>>=
for (i=0; i<nvar; i++) {
    u[i] =0;
    a[i] =0;
...
**    'person': if person=27 that means that 27 subjects have tstop >=time,
...
**    of the risk set.  (tstop > start for each subject guarrantees that
...
if (start[sort1[indx2]] >= time) {
    ...
    */
    for (k=indx2; k<strata[istrat]; k++) {
	p1 = sort1[k];
	if (start[p1] < time) break;
	nrisk--;
    }
...
The only recourse is to rebuild all the totals from scratch.
<<overflow-fix>>=
flag[2]++;  /*a count, for debugging/profiling */
denom=0;
for (i=0; i<nvar; i++) {
    a[i] =0;
    for (j=0; j<=i; j++) cmat[i][j] =0;
    }
if (istrat==0) ii =0; else ii= strata[istrat-1]; /*start of current strata*/
for (k=ii; k<strata[istrat]; k++) {
    p1 = sort2[k];
    if (tstop[p1] <= time) break;  /* further subjects are not yet at risk */
    if (start[p1] < time) {  
	risk = weights[p1] *eta[p1];
	denom += risk;
	for (i=0; i<nvar; i++) {
	    a[i] += risk * covar[i][p1];
	    for (j=0; j<=i; j++) cmat[i][j] += risk * covar[i][p1]*covar[i][p1];
	}
    }
}
@ 
