...
That is, every time we hit a new (unique) death time, and only then,
update the totals.  First remove any
old observations and then add any new ones.
...
while (person < nused) {
    ...
    if (k == nused) person =k;  /* no more deaths */
    else {
...
I've seen this issue in about 1 data set per decade.
<<fixeta>>=
/* 
