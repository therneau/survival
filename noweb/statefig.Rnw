...
of angles $\phi$.
The angles to the centers of the boxes are $\atan(A-C)$ and $\atan(B-C)$,
...
$y$ value.  It returns the angle, within the range, such that the
endpoint of the arc has horizontal coordinate $x$, or NA if none such
exists.  The input angles $a_1$ and $a_2$ lie between $-\pi$ and
$2\pi$. This is done to make sure we have an acute rather than obtuse
angle, e.g., if $a_1=3$ and $a_2=-3$ (170 and -170 degrees) we don't want
an angle of 0 to lie within, so $a_2$ is changed to $2\pi -3$ or 190 degrees.
<<statefig-fun>>=
statefigx <- function(x, C, r, a1, a2) {
    amax <- max(a1, a2)
    amin <- min(a1, a2)
    temp <-(x - C[1])/r
    if (abs(temp) >1) return(NA)  # no intersection of the arc and x
    phi <- acos(temp)  # this will be from 0 to pi
    if (phi < amax && phi > amin) return(phi)
    if (-phi < amax && -phi > amin) return(-phi)
    return(NA)
}
statefigy <-  function(y, C, r, a1, a2) {
    ...
    amin <- min(a1, a2)
    temp <-(y - C[2])/r
    if (abs(temp) >1) return(NA)  # no intersection of the arc and y
    phi <- asin(temp)  # will be from -pi/2 to pi/2
    if (phi < amax && phi > amin) return(phi)
    if ((pi-phi) < amax && (pi-phi) > amin) return(pi-phi)
    NA
}
@ 
