...
\begin{itemize}
  \item for type=`expected' residuals we need the orignal survival y.  This
    ...
    call.
  \item for any call with either newdata, standard errors,  strata, 
    or type='terms'
     the original $X$ matrix, weights, strata, and offset. 
     When checking for the existence of a saved $X$ matrix we can't 
     use [[object$x]]
...
and the second that there is nothing to remove from the terms strucutre.
...
include them in their new dataset, have to build the call up
the way it is done in coxph itself.  We also may not need y.
<<pcoxph-getdata>>=
if (!missing(newdata)) {
