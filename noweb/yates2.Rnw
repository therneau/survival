...
<<yates>>=
yates_xmat <- function(Terms, Tatt, contr, population, mframe, fit, iscat) {
...
if (length(keep) > 1) { # more than 1 term in the model
    ...
    test <- testfun(Cmat[assign2==keep,, drop=FALSE], beta2, vmat, sigma^2)
    test <- matrix(test, nrow=1, 
                   dimnames=list(contr$termname, names(test)))
}
@ 
...
yates_setup.coxph <- function(fit, predict = c("lp", "risk", "expected",
    ...
    if (type=="lp" || type == "linear") NULL  
         ...
         summary <- function(surv, var) {
             ...
             baseline$conf.lower <- exp(-(chaz + zstat*std))
             baseline$conf.upper <- exp(-(chaz - zstat*std))
             colnames(bsurv) <- row.names(surv)
             baseline$survival <- bsurv
