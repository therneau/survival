...
Let $C$ be our final upper triangular contrast matrix,
$Z$ the design matrix from the
balanced design, and $i$ and $j$ each be vectors, the rows corresponding to
two categorical terms with the first term nested in the second.
Because of how terms are ordered in $X$, $i < j$.
We want the two terms to be orthagonal.  Let $V = (Z'Z)^{-1}$ be the variance
matrix for the balanced design, which will always be full rank.
$Z$ ignores any continuous variables.
Orthagonal means that the block diagonal matrix
\begin{equation*}
  H =\left( \begin{array} C_{ii} & C_{ij} \\ 0 & C_{jj} \end{array} \right) 
     \left( \begin{array} V_{ii} & V_{ij} \\ V_{ij}' & V_{jj} \end{array} \right) 
     \left( \begin{array} C_{ii}' & 0 \\ C{ij' & C_{jj}' \end{array} \right) 
\end{equation*}
has zeros in the off diagonal blocks. 
That is, the covariance of $C_{i}\beta$ and $C_j\beta$ is zero: they are
independent contrasts.
Multiplying this out
\begin{equation*}
  \left(C_{ii}V_{ij} + C_{ij}V_{jj}\right) C_{jj} = 0
\end{equation*}


\begin{itemize}
  \item Let $C = (X'X)^-X'X$, the dependency matrix.  These are the coefficients
    of a regression of X on itself.  $C$ will be upper triangular.
  \item Let $Z'Z$ be the matrix for a balanced population, categorical variables
    only.
  \item If term $i$ is nested in term $j$, and both are categorical, then
    replace $C_{ij}'$ ($i$ and $j$ are vectors) with the residuals from a
    regression of $C_{ij}'$ on $C_{j}'$.
  \item $C$ is the contrast matrix for each categorical term.
\end{itemize}
...
diag(C) is 1, if coefficient $j$ is missing then the $j$th column of $C$ is zero,
and 
