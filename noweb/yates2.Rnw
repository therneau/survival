...
yates <- function(fit, term, population=c("data", "none", "factorial", "sas"),
    ...
    if (is.null(fit$assign)) { # glm models don't save assign
        ...
        fit$assign <- xassign  # the cmatrix routine needs this
    }
...
#        if (is.null(sigma)) sigma <- 1
    ...
    else if (type=="linear" || (type=="link" && inherits(fit, "glm")) ||
                               (type=="lp"   && inherits(fit, "coxph")) ||
             (inherits(fit, "survreg") && type %in% c("link", "lp"))) {
         <<yates-linear>>
    }
    else {
        <<yates-nonlinear>>
...
\code{coxph(Surv(time, status) \textasciitilde ns(age,4) + factor(ph.ecog), lung)}.
...
If data=user and x1 was generated from \code{fit\$xlevels}, then we might have
...
creates a global tests for all the terms at once.
...
If $Z$ is full rank we are in the Yate's situation and the cholesky 
...
\begin{align*}
  C_{ii} &= I \\
  C_{jj} &= I \\
  C_{ij} &= -(Z'Z)^-_{ij} \left[(Z'Z)^-{jj} \right]^{-1}, \, j>i
\end{align*}
$C$ starts as an identity matrix and we fill in selected off diagonal
blocks.

Using the formula for the inverse of a symmetric partitioned matrix
\begin{equation*}
  \left( \begin{array}{cc} Q & R \\ R' & S \end{array} \right)^{-1} =
  \left( \begin{array}{cc} \bigcirc & -Q^{-1}R (S - R'Q^{-1}R)^{-1} \\
                           \bigcirc &  (S - R'Q^{-1}R)^{-1} \end{array} \right)
\end{equation*}
this gives 
\begin{equation}
...
Cmat <- diag(ncol(Z))
if (any(iscat)) { # some nut will call this on a model with no factors
    # which terms are factors?
    fterm <- apply(Tatt$factors[!iscat,, drop=FALSE], 2, function(x)!any(x>0))
    for (iterm in which(fterm)) {
        for (j in (iterm:length(fterm))) { # all terms to the right
            ...
            if (j> iterm && fterm[j] && all(Tatt$factors[irow, j] >0)) {
                ...
                Cmat[i1, i2] <- solve(ztz[i1, i1], ztz[i1, i2])
            }
        }
    }
}               
