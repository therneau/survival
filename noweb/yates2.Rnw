    ...
    if (is.character(population)) {
        population <- match.arg(tolower(population[1]),
                                c("data", "none", "factorial", "sas",
                                  "emprical", "yates"))
        if (population=="emprirical") population <- "data"
    ...
    if (type=="linear") {
        dotest <- function(contr, method, population, type) {
            ...
            if (is.list(contr$cmat)) 
                tests <- t(sapply(cmat, function(x)
...
if (is.null(attr(pdata, "terms"))) {
    ...
    # transform the x1 variables, if necessary
    ptrans <- !sapply(Tatt$predvars, is.name)[-1]
    if (any(ptrans[x1indx])) {
        x1name <- names(x1indx)[x1indx]
        for (j in x1name(ptrans & x1indx)){ #transform these
            x1data[[j]] <- eval(Tatt$predvars[[j+1]],x1data)
        } 
    }  
    for (i in 1:nrow(x1data)) {
        j <- rep(i, nrow(pdata))
