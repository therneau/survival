...
yates <- function(fit, term, population=c("data", "none", "factorial", "sas"),
                  ...
                  predict, method=c("direct", "sgtt"), nsim=1000, ...) {
    Call <- match.call()
    ...
    if (missing(predict)) mfun <- yates_setup(fit)
    else if (is.character(predict)) mfun <- yates_setup(fit, predict)
    else {
        # verify the predict is legal
        stop("function prediction is still in progress")
    }
   ...
   # we will need the original model frame ans X matrix
    ...
    # Remove missing coefficients and columns
    if (any(nabeta)) {
...
if (inherits(fit, "coxph")) {
    ...
    offset <- -sum(fit$means * fit$coef)  # recenter the predictions too
    }
else offset <- 0
...
if (inherits(fit, "coxph")) {
    ...
    eta <- xall %*% beta -sum(fit$means*fit$coef)
}
else eta <- xall %*% beta
...
if (inherits(fit, 'coxph')) offset <- bmat %*% fit$means
...
if (ncol(mppv) > 1 && any(estimable)) {
    mvar2 <- matrix(NA, nrow=n1, ncol=ncol(mppv))
    for (i in (1:n1)[estimable]) 
        mvar2[i,] <- apply(sims[,index==i,], 2, var)
    result$mvar2 <- mvar2
}
...
yates_setup.glm <- function(fit, type = c("link", "response", "terms", 
                                          "linear"), ...) {
    type <- match.arg(type)
...
yates_setup.coxph <- function(fit, type= c("lp", "risk", "expected",
                                     "terms", "survival", "linear"), ...) {
    type <- match.arg(type)
    if (type=="lp" || type == "linear") NULL  
    else if (type=="risk") function(x) exp(x)
     else if (type == "survival")
        stop("survival curves not yet supported")
    else stop("type expected not yet supported")
}
