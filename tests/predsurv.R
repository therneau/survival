...
if (FALSE) { # currently an issue with summary.survfit
csum1 <- summary(curve2, time= 1:8 * 60)  # problem with summary
indx <- seq(1, by=8, length=8)  
aeq(p1$fit, csum1$cumhaz[indx]) 
aeq(p1$se.fit , diag(csum1$std.chaz))
aeq(p2$fit, diag(csum1$surv))
aeq(p2$se.fit, diag(csum1$std.err))
