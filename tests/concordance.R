...
cfit <- coxph(Surv(time, status) ~ tt(y), tdata, tt=grank, method='breslow',
              iter=0)
aeq((1/cfit$var)/(2* sum(fit$stats[1:3])), fit$std^2)
...
aeq((1/cfit2$var)/(2* sum(fit2$stats[1:3])), fit2$std^2)
...
aeq((1/cfit3$var)/(2* sum(fit3$stats[1:3])), fit3$std^2)
...
cfit5 <- coxph(Surv(time, status) ~ tt(y), tdata, weight=wt, 
               iter=0, method='breslow', tt=grank2, x=T)
cfit6 <- coxph(Surv(time, status) ~ tt(y), tdata[rep(1:11,tdata$wt),], 
               iter=0, method='breslow', tt=grank)
aeq((1/cfit6$var)/(2* sum(fit6$stats[1:3])), fit6$std^2)
...
cfit8 <- coxph(Surv(time1, time2, status) ~ tt(y), tdata2, weight=wt, 
               iter=0, method='breslow', tt=grank2, x=T)
aeq((1/cfit8$var)/(2* sum(fit8$stats[1:3])), fit8$std^2)
