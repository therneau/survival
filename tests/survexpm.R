#
# test the speed of survexpm, Pade vs Prentice method
#
library(survival)
nstate <- 10
set.seed(1953)
...
setup <- survexpmsetup(rmat) #will return a 0, the matrix is in normal form
setup ==0

indx <- sample(1:10, 10, replace=FALSE)
rmat2 <- rmat[indx, indx]
setup2 <- survexpmsetup(rmat2)
...
if (FALSE) {  # only do this offline. It won't give the same values on two
    n <- 1e5  #  machines, so would trigger a difference in R CMD check
...
# For n=10 the cholesky approach just barely beats Pade, for nstate =6
#  Pade won.  I was surprised.  Expm is fastest, due to sparsity? 
#
...
test1 <- expderiv(rmat, 1, dr, 0)
test2 <- expderiv(rmat, 1, dr, -1)
all.equal(test1, test2)

if (FALSE) {
    n <- 2e4
    t3 <- system.time({for (i in 1:n) z <- expderiv(rmat, 1, dr, 0)})
    t4 <- system.time({for (i in 1:n) z <- expderiv(rmat, 1, dr, -1)})
}
