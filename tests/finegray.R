...
fdata <- data.frame(time  =c(1,2,3,4,4,5,5,5,5,6,8,9,10,12),
                    status=c(1,2,0,1,0,2,0,0,1,0,2,0,1 ,0),
                    x     =c(5,4,3,1,2,1,1,2,2,4,6,1,2, 0),
                    id = 1:14)
fstat <- factor(fdata$status, 0:2, c("cen", "type1", "type2"))
test1 <- finegray(Surv(time, fstat) ~x, fdata)
test2 <- finegray(Surv(time, fstat) ~x, fdata, event=2)
...
csurv <- list(time=c(0, 3, 4, 5, 6, 9),
              p = cumprod(c(1, 11/12, 9/10, 5/7, 4/5, 2/3)))
#
