...
The code below currently could be made faster for the case that a user
had save x, y, etc in the model object --- then model.frame does not
actually need to be called.  This is very rare, except for the case of
the cph function from the rms package, and is on my ``someday'' list.
