...
\begin{description}
  ...
  \item[censor] if FALSE, remove any times that have no events.  This is for 
...
if (!missing(id) && !is.null(id)) individual <- TRUE
else if (missing(id) && individual) id <- rep(0,n)
else id <- NULL
...
\begin{itemize}
  \item For predictions with time-dependent covariates the user will have 
    ...
    the covariate trace over time of a single individual; or if [[id]] is 
    present we have the covariate trace for multiple subjejects.
  ...
  \item If newdata is not present we assume that this is the ordinary case, and
    ...
    not a good idea statistically since many users view this as an
...
When grabbing [newdata] we want to keep the model.frame processing, in
particular the handling of missing values.
...
Once we have newdata in the correct form, pull data from it. 
...
Extract the data items for individual survival.
<<survfit.coxph-setup>>=
if (individual) {
    if (missing(newdata)) 
...
<<survfit.coxph-compute>>=
    ...
    onecurve <- function(slist, x2, y2, strata2, ustrata) {
    ...
    if (all(id ==id[1])) {
        result <- onecurve(survlist, x2, y2, strata2, ustrata)
        varh <- result$varh
        result$varh <-NULL
        }
    else {
        ...
        for (i in unique(id)) {
            indx <- which(id==i)
            onelist[[i]] <- onecurve(survlist, x2[indx,], y2[indx], 
                                     strata2[indx], ustrata)
            }
        result <- list(n=unlist(lapply(onelist, function(x) x$n)),
                       ...
                       n.event=unlist(lapply(onelist, function(x) x$nevent)),
       ...
       result$ varh <- unlist(lapply(onelise, function(x) x$varh))
        }
    }
@ 
...
if (!censor) {
    ...
    if (!is.null(result$strata)) {
        temp <- rep(names(result$strata), each=result$strata)
