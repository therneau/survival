...
if (!missing(newdata)) {
    ...
    if (is.vector(newdata, "numeric")) {
        ...
        if (length(newdata)==length(object$coefficients)) {
            ...
            x2 <- newdata[names(object$coefficients)] -xcenter #reorder
            mf2 <- NULL
            }
...
@ 
To build the mf2 model frame, first remove any [[cluster]]
terms from the model. 
If individual=FALSE, also remove the response and any strata.
<<build-mf2>>=
