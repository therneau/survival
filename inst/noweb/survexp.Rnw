...
formula could contain a mapping between the variables in the data set and
...
very long ratetable clause; at about 40 variable it causes one of the R internal
routines to fail due to a long expression.
A third reason, perhaps the most pressing in reality, is that I've always 
felt that the prior code was clunky.
...
if(length(rate) == 1) {
    if (!missing(rmap)) 
        stop("The rmap argument supercedes a ratetable() call in the formula")
...
else if (!missing(rmap)) rcall <- substitute(rmap)
else rcall <- NULL
browser()
...
if (is.ratetable(ratetable)) {
    ...
    se.fit <- FALSE
    rtemp <- match.ratetable(eval(ratecall, m))
    R <- rtemp$R
    }
else if (inherits(ratetable, 'coxph')) {
    israte <- FALSE
