...
3 variables [[attr(survexp.us, 'dimid']]
\begin{itemize}
  ...
  \item year = date of the start of follow-up
    \end{itemize}
The formula may contain a mapping between the variables in the data set and
...
If there is no ratetable clause in the formula we assume that all the variables
match in name and definition.  To make later processing easier a ratetable 
clause is added, in this case it would be [[ratetable(age=age, sex=sex, year=year]].
<<survexp-setup>>=
...
Cox model.  We call the model.matrix.coxph function to avoid the
steps found there (remove cluster statements, etc); the result of
[[m[rate]]] is of class ratetable2, which needs to be turned into a 
data frame.  We also need to use the [[mf]] argument of the function, otherwise
...
<<survexp-compute>>=
ovars <- names(m)[-c(rate, attr(Terms, 'response'))]
...
if (is.ratetable(ratetable)) {
    ...
    R <- model.matrix.coxph(ratetable, mf=data.frame(unclass(m[,rate])))
    if (no.Y) {
