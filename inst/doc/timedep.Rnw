...
<<>>=
load('d3.rda')
age2 <- tcut(d3$agechf*365.25, 0:110* 365.25, labels=0:109)
rowid <- 1:nrow(d3)
pfit <- pyears(Surv(startday, stopday, hospevt) ~ age2 + rowid,
               data=d3, data.frame=TRUE, scale=1)
print(pfit$offtable)
pdata <- pfit$data
print(pdata[1:6,])
@
...
temp <- d3$startday[index] + lagtime  #start of each new interval
data2 <- data.frame(d3[index,], 
                    time1= temp,
                    time2= temp + pdata$pyears,
                    event= pdata$event,
                    age2=  1+ as.numeric(pdata$age2) )

afit1 <- coxph(Surv(startday, stopday, hospevt) ~ sex + pspline(agechf), d3)
afit2 <- coxph(Surv(time1, time2, event) ~ sex + pspline(age2), data2)
@ 
...
<<>>=
afit2b <- coxph(Surv(startday, stopday, hospevt) ~ sex + tt(agechf),
                data=d3, x=T,
                tt=function(x, t, ...) pspline(x + t/365.25))
#plot(psplineinverse(afit2b$x), exp(terms(afit2b)[,2]),
#     xlab="Current age", ylab="Excess risk", log='y')
@ 
...
should be a list of functions with the requisite number of items.

Because of the data manipulation the termplot() function cannot 
reconstruct the data; we have to replace it with an equivalent 
call.  (This is still not quite working.)
...
coxph call.  (Doing so allowed me to depreciate the survobrien function).
