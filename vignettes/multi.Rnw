...
a matrix with one row for each time and one column for
...
group that was created by the variables on the right hand side of the formula.
...
Both the Kaplan-Meier estimate and the 
cumulative incidence esitmate (for competing risks) are a special case of
the AJ estimate. 
It is more general that that, however; a given subject can have multiple 
transitions from state to state, including transitions to a state was 
visited earlier. 
In this latter case the data structure is similar to that for time varying
...
for each subject, and is the name of a variable in the data set.
...
first event time has no impact and could be removed without changing the
estimates.
...
Here is the code for figure \ref{badfit}.  The use of at logical (true/false)
...
time $t$ is $\sum_i J_{ik}^2(t)$ where $J_{ik}(t)$ is the inflence
...
differ by over 3 standard errors, which is statistically
significant.
...
oldpar <- par(mar=c(5.1, 4.1, 1,.1))
plot(sfit4, col=rep(1:4,2), lwd=2, xmax=48, xaxt='n',
