...
time by state matrix of results and allows for calcuation of the
...
The first and more important is to ask whether 2.2 months is an important gain
...
on this backdrop an extra 2 of those 30 months in a complete response may
only be a small advantage from a patient's point of view.
...
\Sexpr{round(temp$table[3,4],1)} and \Sexpr{round(temp$table[4,4],1)},
...
The difference is suggestive but not statistically significant.
...
axis(1, xtime, xtime)
text(c(40, 40, 40, 40), c(.58, .19, .30, .06),
...
crsurv <- survfit(Surv(tstart, tstop, crstat) ~ trt,
                  data= data2, id=id, influence=TRUE)
curveA <- crsurv[1,]
dim(curveA$pstate)  # P matrix
dim(curveA$influence)
table(data1$trt)
curveA$p0
@

For treatment arm A there are \Sexpr{table(data1$trt)[1]} subjects and
\Sexpr{dim(curve$pstate)[1]} time points in the CR curve.
...
\code{curveA$inflence[1,,]} is the influence of subject 1 on
\code{curveA$pstate}.
The influence has one extra row, however; the first row is the influence
of the subject in $p_0$, the initial state probabilities.
...
for each subject; this does not hold if not all subjects start in the same
state.
...
This next function draws a variation is the illness-death model,
which has an initial state,  
