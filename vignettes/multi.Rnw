...
# These functions are used in the document, but discussed only at the end
...
that from a CALGB trial in subjects with FLT3 mutated acute myeloid
leukemia.
In this randomized comparison of two conditioning regimens the
cannonical path for a subject is initial therapy $\rightarrow$
...
this introduduction will use only the treatment variable.
...
i.e., no one has two events of the same type, which correct for this
...
we see that they spread further apart than the dotted ``ever in response''
curves. 
...
The pointwise variance for $P_k(t)$ = probaility of state $k$ at
...
The less imporant answer is to test whether the apparent gain is sufficiently
...
layout(matrix(c(1,1,2,0),2,2), widths=2:1)
...
stem cell transplant and those whose last state is a complete response
...
the following derived function.  It accepts a vecor of state names,
...
\section{Conclusion}
With a data set such as this we can fit multiple multi-state models.
