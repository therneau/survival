...
<<fig=TRUE>>=
plot(tfit, col=1:3, lwd=2, ylim="Probability in state")
@ 
...
cumulative incidence esitmate (for competing risks) are each a special case of
the AJ estimate. 
...
\begin{enumerate}
  \item The key problem with the naive estimate is that subjects who die can 
    ...
    is an estimate of ``fraction who would
