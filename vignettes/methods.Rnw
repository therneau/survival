...
Say that there is a shared proportional baseline hazard. 
Consider the simple model shown in figure \ref{label:simplems},
a data set with covarariates of (time1,time, state), cstate, x1, and x2.
...
check <- survcheck(Surv(day1, day2, event) ~ 1, ndata, id=id, istate=cstate)
...
nfit$smap
dim(nfit$phZ)
nfit$phZ[1, 7:10, 7:10]
@ 

A base model would have 10 hazards and 20 coefficients. The above
restricts this to 7 hazards and 14 coefficients. 
...
Equation eqref{nogamma} has no $\gamma_1=0$ term, because \code{state} is
a factor variable and we are using the default R coding for factors.
Think of the right hand side as $Z\gamma + X\beta$, a covariate/coefficient block
for shared proportional hazards and another for covariates.

The \code{stacker} routine creates a working $X$ matrix with a block of rows for
each of the 7 baseline hazards and a column for each of the 14 coefficients,
...
see the Baseline row in smap above.
...
constant within every subshare value.  Such covariates can be shifted from
...
The two are returned as a list \code{shared} with two elements: the first is
a vector of 0/1/2 values, identifying each row of cmap as a variable that is
ordinary, time-dependent, or ``move to Z''. 
The second is a vector of $exp(Z\gamma)$ values, one per hazard.

In creating a the cumulative hazard and probability in state estimates, the
survfit.coxphms routine needs, for each curve
\begin{enumerate}
 ...
 \item Form $\eta=X\beta$, ignoring those coefficients that were moved to $Z$.
   This can be done by zeroing those elements of the coefficient vector.
 \item Sums of events and risk, for each subshare are computed by the coxsurv1
   or coxsurv2 routine, within the multihaz.
 \item Compute the shared hazards as sum(event)/sum(risk * exp(Zgamma)), and the
   individual hazards as exp(Zgamma) * shared.
   
   routines, within the multi
   way
   $Z\gamma$ value.
 \item Generate shared baseline hazards as the     
for each row of cmap that identifies the variable as ordinary (0),
time-dependent (1) or time-dependent and constant (2) 
The phZ component contains the $Z$ covariate values for each relevant row of 
cmap, for each substratum of a shared baseline.

When creating a survival curve, ... to fill in.

For a more complex example consider the pbc data with two time-dependent
covariates of bilirubin group and ascites. We are using timeline style
data below.
