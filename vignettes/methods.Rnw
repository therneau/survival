...
In this vignette are discussions of design issues, algorithms, and detailed
...
\chapter{Cox model}
\chapter{Specials}
R formulas allow for special terms. When parsing the coxph formula using the
standard \code{terms} function, the optional argument \code{specials= 'strata'}
will cause the returned object to specially mark any strata terms.
We then deal with this properly in the fit; a stratum is not just another
covariate.

The advent of hundreds of libraries, more users have been using double colon
notation ensure that they actually invoke the function that they think they
are invoking, e.g., 
<<echo=TRUE, eval=FALSE>>=
survival::coxph(survival::Surv(time, status) ~ 
                    age + sex + survival::strata(inst), data=lung)
@
Saying library(tidyverse), alone, attaches 60 (19 packages + 41 name spaces).
A problem with this is that the terms function searches for and recognizes 
the character string ``strata'' and so will not detect ``survival::strata''
as special.  
At least one package assigns the result of \code{strata::(group)} to a
temporary variable and then uses that variable in their formula, i.e.; it would 
not suffice to add survival::strata to my list of specials.

In version 3.7 of the package I made a valiant effort to combat this by 
essentially re-writing the user's formula.  It worked pretty well, but I've
now thrown in the towel and ensured that all of my special functions return
a recognized class and key off of that.  The functions that were special
were Surv, strata, cluster, pspline, frailty, ridge and tt.
\begin{itemize}
  \item The pspline, frailty, and ridge functions (and frailty subfunctions
    like frailty.t) already had the class coxph.penalty. It suffices to recognize
    that rather use terms.
  \item The Surv function also has a class. We never use it on the right hand
    side, however, so have never used specials to find it.
  \item The tt() function did not have to change, since I never created a dummy
    function in the package to correspond to it, it only exists ``on the fly''
    within the package and will always be recognized first.
  \item An update is to do the same with cluster(). Using this within a 
    formula is on its way to deprecation in any case.
  \item That leaves strata.  It appears in lots and lots of places, and all
    will need to be changed.
\end{itemize}

\section{Missing values}
\index{multistate missing}
When the formula is a list, then the possiblility exists that different
transitions will have different covariates.
If a particular row of the data is missing variable x3 say, but x3 is
only used in transitions for which that row is not at risk, then there is
no need to remove said observation.
Assume for instance that one of the states was surgical recection of a
cancer, then pathology results will only be available after that time point.

This leads to a simple dichotomy: if the model formula is simple then call
the model.frame matrix in the usual way. If not, use na.pass to retain all
observations until a proper determination can be made.  To make that 
determination we need the current state for each data row, which is either
obtained or validated by survcheck.

The model.frame and the tmap object suffice for missings, we don't need to
create the X matrix.  (Someone might have saved X and not the model frame,
though, in which case we can use cmap and X).

Steps for a multi-formula model
\begin{enumerate}
  \item Get the raw model frame, no missings removed
  \item Extract Y, istate if present, weights if present
  \item Use survcheck2 to get the      
\end{enumerate}
...
\section{Shared hazards}
The vignette on shared coefficients and shared hazards in the survivalVignettes
package gives important background on this problem. 
...
As a first example look at nafld data, the state space is shown in 
figure \ref{nafld1}.  The code below sets up the data and fits a 
first model.
<<ndata>>=
ndata <- nafld1
...
A base model would have 10 hazards and 20 coefficients. The above
restricts this to 7 hazards and 17 coefficients. 
Here is what was fit, using 0:4 as the state labels, $beta_{m1}$ as the
first male coefficient, $X_i$= (male, nafld) as the data vector for each.
\begin{align}
  \lambda_{01}(t;x) &= \lambda_{01}(t)e^{x_{i1}\beta_{m1} + x_{i2}\beta_{n1} \\
  \lambda_{02}(t;x) &= \lambda_{02}(t)e^{x_{i1}\beta_{m1} + x_{i2}\beta_{n1} \\
  \lambda_{03}(t;x) &= \lambda_{03}(t)e^{x_{i1}\beta_{m1} + x_{i2}\beta_{n1} \\
  \lambda_{12}(t;x) &= \lambda_{12}(t)e^{x_{i1}\beta_{m2} + x_{i2}\beta_{n2} \\
  \lambda_{13}(t;x) &= \lambda_{13}(t)e^{x_{i1}\beta_{m2} + x_{i2}\beta_{n2} \\
  \lambda_{23}(t;x) &= \lambda_{23}(t)e^{x_{i1}\beta_{m3} + x_{i2}\beta_{n3} \\
  \lambda_{04}(t;x) &= \lambda_{04}(t)e^{x_{i1}\beta_{m4} + x_{i2}\beta_{n4}
    \label{nogamma}\\
  \lambda_{14}(t;x) &= \lambda_{04}(t)e^{\gamma_2 + x_{i1}\beta_{m4} + x_{i2}\beta_{n5} \\
  \lambda_{24}(t;x) &= \lambda_{04}(t)e^{\gamma_3 + x_{i1}\beta_{m4} + x_{i2}\beta_{n6} \\
  \lambda_{34}(t;x) &= \lambda_{04}(t)e^{\gamma_4 + x_{i1}\beta_{m4} + x_{i2}\beta_{n7} \\
Equation eqref{nogamma} has no $\gamma_1=0$ term since we have \code{(2:4):5}
rather than \code{(1:4):5}.
Think of the right hand side as $Z\gamma + X\beta$, a covariate/coefficien
block for shared proportional hazards (Z= notzero) and another for covariates
(X= nafld, male).

The \code{stacker} routine creates a working $X$ matrix with a block of rows for
each of the 7 baseline hazards and a column for each of the 17 coefficients,
along with $Y$ and strata vectors.
Block 1 contains all those rows at risk for a 1:2 transition, which is all those
rows with initial state of 1. Likewise block 2 for 2:3 transitons, etc.,
per the Baseline row in smap above.
For the shared 1:5, 2:5, 3:5, 4:5 the working X matrix will have first the
rows with an initial state of 1, then initial state of 2, then 3, then 4,
all marked with the same value of \code{share} but different values for
\code{subshare}.
The subshare for these 4 will have values of 7, 8, 9, and 10, i.e., their
position in the smap vector.

The coxph routine uses this information to reason out two things. The first
is what covariates in this working matrix are time dependent, and which of those
are constant within every subshare value.  Such covariates can be shifted from
the $X\beta$ to the $X\gamma$ portion of our model.
The second is a multiplier for every transition, based on these constant values.
This information is returned as a list \code{shared} with two elements: 
the first is a vector of 0/1/2 values, identifying each row of cmap as a 
variable that is ordinary, time-dependent, or ``moved to Z''. 
The second is a vector of $exp(Z\gamma)$ values, one per hazard. The $Z$ 
values themselves do not need to be saved.

In creating the cumulative hazard and probability in state estimates, the
survfit.coxphms function, for each curve
\begin{enumerate}
 \item Recenter each column of the $X$ matrix to match the target $x$ of the
   curve, then call stacker to recreate the working X matrix.
 \item Form $\eta=X\beta$, ignoring those coefficients that were moved to $Z$.
   This is done by zeroing those elements of the coefficient vector.
 \item Sums of events and risk, for each subshare are computed by the coxsurv1
   or coxsurv2 routine, within the multihaz function.
 \item Compute the shared hazards as sum(event)/sum(risk * exp(Zgamma)), where
   the sum is over the subhazards that are bundled.  Then the
   individual hazards as exp(Zgamma) * shared.
\end{enumerate}

The example that has caused the most thinking is from the dementia data set,
which is included in the mshmm package. We need to treat amyloid
level as a state, because it is time dependent.  There are 4 levels of amyloid,
along with outcomes of non-demented (ND), dementia, and death.
This led to the state space below:
...
We have left out arrows for the 8 transitions to death to make the
figure less busy.
The solid lines are transitions we expect, i.e., amyloid accumulates,
without jumps, and does not change state at the exact same instant as
cognition. The dashed lines are observed transitions that don't fit this
model. But, the transition-based likelihood needs to account for \emph{all}
observed transitions.  
The two solid arcs from missing amyloid to moderate and high
are consequences of the fact that the amyloid ligand for PET (Pittsburgh
compound B) was not available until several years after the study started, so
many imaged subjects start out with a series of non-imaged visits. 
Due to LVCF, no one reverts to the missing state.

If we declare that all transtions from ND to dementia have a shared baseline
hazard based on amyloid group: \code{D(1):D(2) ~ agrp +1 / common}, this
implies that all possible transitions from ND to dementia have the same
hazard. That is, all 16 of them.  
The code really has no choice but to believe the user's declaration.
The result in the stacker routine will be 16 subshare sets for that transition,
with every at risk observation (current state of ND) appearing in 4 of
them.
The partial likelihood for each observed ND:dementia transition will
have all of these, so 4 copies of each.  We can factor this out, giving
a constant term of -log(4) time the number of events in the LPL, thus
the PL will maximize with exactly the same coefficients as before.
Still, the ``wrong'' PL bothered me, and a lot of the excursions documented
in stacker.R (comment block with Dec2024) are based on this, and an attempt
to correct it.

We need all 4 copies to get a proper cumulative hazard, however, since
the solution needs to obey sum(expected) = sum(observed), and the left hand
side of this equation will have all 16 hazards. 
 The real problem is that this is
the correct solution to the wrong estimate.
Say for instance that the ND:dementia rate is 1/20 person-years but the 
amyloid low:mod rate was 1/200.  The above forces both of them to be about
11/200.
What we actually want is an additive model.  


<<pbc10>>=
death <- with(subset(pbcseq, !duplicated(id)),
              data.frame(id=id, day=futime, death= 1*(status==2)))
temp <- subset(pbcseq,,c(id, day, age, sex, albumin, bili, edema, protime))
temp$death <- 0
temp$age <- round(temp$age, 1)  # nicer printout
p10 <- merge(temp, death, all=TRUE)


# bili, bili*edema, and bili*edema*albumin,  all + death
p10$bgrp <- cut(p10$bili, c(0,1,4, 100), c("0-1", "1-4", "4+"))
p10$agrp  <- cut(-p10$albumin, c(-100, -3.5, -3, 0),
                   c("normal", "3-3.5", "<3"))
lastrow <- !duplicated(p10$id, fromLast=TRUE)
p10$bili3 <- with(p10, factor(ifelse(lastrow, 4*death, 1+ 2*edema), 0:4,
                    c("none", levels(bgrp), 'death')))
p10$e3 <- with(p10, factor(ifelse(lastrow, 4*death, egrp+1), 0:4,
                    c("none", "e0", "e.5", "e1", 'death')))
check1 <- survcheck(Surv2(day, bili3) ~ 1, p10, id=id)
check1$transitions 

pfit0 <- coxph(Surv2(day, death) ~ age + bgrp + edema, p10, id=id)
pfit1 <- coxph(list(Surv2(day, e3) ~1, 
                    0:"death" ~ age + bgrp + edema + 1/common), p10, id=id)
pfit1
pfit1$phZ[1,,]

rbind(simple= pfit0$loglik, multi= pfit1$loglik)
@

In the above the transitions to death are proportional, but with respect to
a continuous variable $Z$ rather than a factor.  
A proper survival curve is not possible, however, due to the presence of
bilirubin, a time dependent covariate that has not been converted to a state.
Make a 10 state model with joint bilirubin and edema states.

<<pbc10b>>= 
# make a 3x3 + death outcome, for 2 TD variables
name9 <- outer(c("b0", "b1", "b2"), c("e0", "e1", "e2"), paste, sep='.')
temp9 <- with(p10, as.numeric(bgrp) + 6*edema)
p10$be <- with(p10, factor(ifelse(lastrow, 10*death, temp9), 0:10, 
                 c("none", name9, "death")))
check2 <- survcheck(Surv2(day, be) ~ 1, p10, id=id)
check2$transitions[,-11] # leave off censored, for width

alias <- data.frame(state=c(name9, "death"), B=c(rep(1:3, 3), 0),
                    E= c(rep(1:3, each=3), 0))
pmat <- matrix(0, 10, 10, dimnames=list(alias$state, alias$state))
for (i in 1:3) {for (j in 0:1) pmat[i+3*j, i+ 3 + 3*j] <- 1}
for (i in 1:2) {for (j in 0:2) pmat[i+3*j, i+ 1+ 3*j] <- 1}
pmat[1:9,10] <- 1
pmat[c(2,5),10] <- .7
statefig(rbind(3,3,3,1), pmat, col=rep(c("black", "gray"), c(12,9)))
@ 

We would like to think of the progression process in terms of the figure above.
However, where we have 12 arrows for progression of bilirubin and edema,
24 if we allow reversions, the transtions matrix shows 46.
One of the arrows to death (7:10, b0.e2) is also not present. 
Fit a model with shared hazards.

<<pbc10c>>=
pfit2 <- coxph(list(Surv2(day, be) ~1,
                    0:"death" ~ age + bgrp + edema + 1/ common),
               id=id, data= p10)

dim(pfit2$cmap)
cpos <- colSums(pfit2$cmap) >0
pfit2$cmap[,cpos]

dim(pfit2$phZ)
pfit2$phZ[, cpos,3:5]

sfit <- survfit(pfit2, newdata=list(age=50), p0=rep(1:0, c(1,9)))
@ 

The shared baseline has caused the 7:10 transition to be included, even though
it has no events; these subjects now contribute to the denominator of the
shared hazard. The fixed covariates $Z$ differ for each of the 9 components
of the shared hazard.




It proceeds column by column through
smap and cmap, with one block of rows for each unique baseline in smap, and
coefficients determined by cmap.
\begin{itemize}
  \item Block 1 for the 0mc:1mc transition (1:2 in cmap/smap) 
    includes everyone at risk for said
transtion, i.e., all data rows with a starting state of 0mc. Within the block
the rows are in the same order as the input data frame. 
The first two columns of X are copies of the male and nafld columns, the 
remaining columns are for coefficients not relevant to this transition and
are set to 0. The working Y has a status of 1 for a 0mc:1mc transtion and 0
otherwise, and the strata will be 1.
  \item Blocks 2 and 4 are for the the 0mc:2mc and 0mc:3mc transitions
    (1:3 and 1:4 in smap). The each have the same $X$ as block
    1, strata=2 or 3, but the 'status' portion of Y now is 1 for an 
    observed 0mc:2mc (block 2) or 0m:3mc (block 4) transition, respectively.
  \item Blocks 3 and 5 are for the 1mc:2mc and 1mc:3mc transitions, each
    contains all data rows for which the initial state is 1mc. 
    Columns 3 and 4 of X will contain copies of the
    male and nafld variables, and all others are 0. 
    Y has a 0/1 status for observed 1mc:2mc transitions in block 3,
    1mc:3mc in block 5.
   \item Block 6 has rows at risk for the 2mc:3mc (3:4 in cmap) transition,
     which is all rows with an initial state of 2mc.
   \item Block 7 has all rows at risk for a 1:5, then rows at risk for 2:5, 
    rows at risk for 3:5, and finally rows at risk for the 4:5 transition,
    i.e. everyone in the data. Columns 7 contains a copy of nafld, but only
    for those rows that start in the 0mc state, column 12--14 nafld for the 1mc
    rows, 2mc, and 3mc rows, respectively. Columns 8-11 contain copies of
    the male covariate then the 3 dummy variables corresponding to cstate.
    The status portion of Y is 1 for any death.
\end{itemize}
This set of Y,X, strata is then used to fit a single coxph model for all the
coefficients at once. 

In the fit, there is no distinction between the $\gamma$ coefficients 
of our equations,
which are associated with proportional baseline hazards, and the $\beta$
coefficients associated with per-subject covariates.
This distinction is recovered at the end of the fit, by identifying 
components of $XB$ that are constant within each sub-hazard of a shared
hazard. Such terms are effectively independent of subject, and we assign
them to the $X\gamma$ portion.  This is important later, when computing
absolute risks.
If any are identified, a phZ matrix is included that duplicates relevant
rows of cmap, and contains relevant value from the $Z$ matrix.
By definition, a time-fixed covariate is not eligible, and (IMHO) it
makes no sense to have a TD covariate which is sometime bundled with the
hazard and sometimes not, as I have no blueprint to create a proper curve
in such a case.

In the above example, the nafld and male variable are time-fixed so not
considered. The 3 cstate variables appear in the 1:5, 2:5, 3:5, and 4:5
strata. All are 0 in the 1:5 group, are (1,0,0) for all rows in the 2:5 group,
(0,1,0) for all in the 3:4 and (0,0,1) and 4:5 data rows.  The code is a pair
of nested loops over row of cmap and col of smap.

The shared vignette now fits the model in 2 other ways, leading to similar fits
<<nfit2>>=
ndata$notzero <- 1*(ndata$cstate != "0mc")
...
A second example is the pbc data set, where a bit more complexity arises.
I will have 10 states: 3 bilirubin catetories * 3 edema + death.
...
temp$death <- 0
temp$age <- round(temp$age, 1)  # nicer printout
pbc10 <- merge(temp, death, all=TRUE)

# bili, bili*edema, and bili*edema*albumin,  all + death
pbc10$bgrp <- cut(pbc10$bili, c(0,1,4, 100), c("0-1", "1-4", "4+"))
pbc10$egrp <- 2*pbc10$edema   # 0, .5, 1 to 0, 1, 2
pbc10$agrp  <- cut(-pbc10$albumin, c(-100, -3.5, -3, 0),
                   c("normal", "3-3.5", "<3"))
# no one died on the day of a visit
lastrow <- !duplicated(pbc10$id, fromLast=TRUE)
pbc10$bili3 <- with(pbc10, factor(ifelse(lastrow, 4*death, as.numeric(bgrp)), 
                                  0:4, c("none", levels(bgrp), 'death')))
name9 <- outer(c("b0", "b1", "b2"), c("e0", "e1", "e2"), paste, sep='.')
temp9 <- with(pbc10, as.numeric(bgrp) + 3*egrp)
pbc10$be <- with(pbc10, factor(ifelse(lastrow, 10*death, temp9),0:10, 
                 c("none", name9, "death")))

check1 <- survcheck(Surv2(day, bili3) ~ 1, pbc10, id=id)
check2 <- survcheck(Surv2(day, be) ~ 1, pbc10, id=id)
table(check2$transitions[1:9, 1:9] >0)

# td model, and states model
pfit1 <- coxph(list(Surv2(day, be) ~1,
                    0:'death' ~ age + bgrp + edema +1 / common),
               pbc10, id=id)

ckeep <- colSums(pfit1$cmap) >0
table(ckeep)
pfit1$cmap[,ckeep]
pfit1$phZ[,ckeep]
@ 

The fit imposes no model for the 46 observed transtions between pairs
of the  9 bilrubin*edema states, those 46 have only zeros in the cmap table
since there are no associated coefficients.
(I have thought about not storing these unused columns, since typing
\code{pfit1\$cmap} leads to so much output, but computation is
a bit easier if cmap and smap match and it is easy for a user to create an
index like my ckeep above.)

In the above model there are 9 shared hazards but only 3 $\gamma$ coefficients,
as we have an additive structure $Z\gamma$.  Two coefficients for the 3
level factor \code{bgrp} and one coefficient for edema. 
If we had said \code{/ common + shared} there would have been 8 coefficients,
treating 9 starting states as a factor.
