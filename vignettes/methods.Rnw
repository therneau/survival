...
Equation eqref{nogamma} implicitly has a $\gamma_1=0$ term, because 
 \code{cstate} is a factor variable and we are using the default R coding
for factors.
Think of the right hand side as $Z\gamma + X\beta$, covariate/coefficient block
