...
\begin{align*} 
  \Shat(t; z) &= \prod_{s\le t} \left[1- lhat(t;z)] \\
              &\ne \left(\prod_{s\le t} \left[1- lhat(t;c)]\right)^{e^{(z-c)\beta}}
\end{align*}
...
\begin{align*}
  \sum  w_i Y_i(t) r(x_i;c) &= \sum dN_i(t) w_i \frac{r(x_i;c)}{1- z(t)^{r(x_i;c)}}
      \label{KPhaz1}\\
      z(t) &= left(1- w_d\frac{r(x_d;c)}{\sum  w_i Y_i(t) r(x_i;c)\right)^
...
running mean
\begin{equation}
  \tmu(t;z) &= \int_0^t (z- \xbar(s))\dLambda(s; z) \\
               \e^{(z-c)\beta} \int_0^t (z- \xbar(s))\dLambda(s; c) \label{tmu}
\end{equation}
...
\Sexpr{rowSums((X- rep(z, each=nrow(X)))* D)}. 
