...
\chapter{Cox model}
\section{Survival curves from a coxph model}
\index{cox:survival}
...
\end{align*}
In the above $j$, $k$ are states, $i$ is an observation, and the
$\square$ stands in for the fact that each row of $A$ must sum to zero
(it saves writing the negative sum of the other elements).
...
variance of the cumulative hazard estimate has been derived by (ref) as
\begin{align*}
  v(t;z) &= d'{\cal I}^-1 d +
...
For multistate we will use the IJ estimate of the hazard.

\begin{align}
  \frac{\partial \sum_i w_i Y_i(t) r_i(t;z)}{\partial w_m} &=
  Y_m(t)r_m(t;z) + \sum_i w_iY_i(t) r_i(t;z) (x_i- z)' 
     \frac{\partial \beta}{\partial w_m}  \label{ajij1} \\
     &= Y_m(t)r_m(t;z) + \sum_i w_iY_i(t) r_i(t;z) D_m(x_i- z) \label{ajij2} \\
 \frac{ \partial\lambda_{jk}(t;z)}{\partial w_m} &= Y_{mj}(t)
    \frac{dN_{mjk}(t) -  \lambda_{jk}(t;z) - D_m(\xbar(t)-z)} 
         {\sum_i w_i r_i(z) Y_{ij}(t)}      \label{ajij4}
\end{align}
...
of coefficients.  
The term $\xbar(t)-z)$ is a $p$ by 1 column vector.
Recognize that the term to the right of $D_m$ is a $p$ by 1
vector that is independent of $m$ so need only be computed once at each
event time.
In equation \eqref{ajij4} the first two terms are essentially the 
leverage for the
ordinary cumulative hazard, the third the impact of $w_m$ as mediated through
its effect on the model coefficients.
We are taking advantage of the fact that any row of the data will be at
risk in exactly one state for the (time1, time2) interval of that row to
simplify notation.  The matrix product $D(\xbar(t)-z)$ can be done for all
rows at once.
...
Let $A(t; x)$ be the hazards matrix at time $t$ for covariate values $x$
\begin{align*}
  A &= \left( \begin{array}{cccc}
       \square & \lhat_{12}(t;x) & \lhat_{13}(t;x) & \ldots \\
       \lhat_{21}(t;x) & \square & \lhat_{23}(t;x) & \ldots \\
       \vdots & \square & \vdots & \ddots \end{array} \right) \\
  p(t;x) &= p(0) \prod_{s\le t} e^{A(s;z)}
\end{align*}
where $\lhat_{jk}$ is the predicted hazard from the model for a $j$ to $k$
transition.  The diagonal elements are such that row sums are zero.

The mstate package uses the first order Taylor series  $\exp(A) \approx I + A$,
which is parallel to the formula in the Aalen-Johansen.
We do not, for a couple of reasons.
\begin{itemize}
  \item If the chosen $x$ corresponds to a particularly high relative hazard,
    then the diagonal element can be negative, and $A$ is no longer a valid
    transition matrix.
  \item Consistency with the single state Cox model's Breslow estimate.
    (This approximation has never been proposed for the simple Cox model.)
\end{itemize}

For two matrices $A$ and $B$, $\exp(A)\exp(B) != \exp(A+B)$ unless $AB=BA$,
a condition that does not occur with our transtion matrices.
Per section \ref{matexp}, the computation of $exp(A)$ will normally be very
fast, so this does not add any undue time burden.
    
\subsection{Leverage}
The derivative of $lambda_{jk}(t)$ with respect to observation $i$ will have
two components: one essentially the same as the usual hazard, and another
involving the leverage wrt $\beta$.
Let $Y_{ij}(t)$ be the indicator that subject $i$ is at risk and in state $j$
at time $t$ (at risk for transtition to somewhere else),
and $N_{ijk}(t)$ be the cumulative number of $jk$ events for observation $i$.
In the code, an observation (row of data) is only ever in one state, and has
at most one transition at the end, so $dN =N$.

\begin{align*}   
\frac{\partial \lhat_{jk}(t))}{\partial w_m} &=
  &= \left[\frac{N_{mjk}(t)}{\sum_i w_i Y_{ij}(t) r_{ijk}t)} -  \lhat_{jk}(t;z)
                  \frac{Y_{mj}(t)}{\sum_i w_i Y_{ij}(t) r_{ijk}(t)}\right] -
       \lhat_{jk}(t;z) D_{m(jk)} (\xbar(t)-z)_{(jk)} \\
  &= A - B
\end{align*} 
In the above, part A is of exactly the same form as developed in equation
\eqref{e1} and following, with $w_ir_{ijk}$ replacing $w_i$, so it can be
accumulated efficiently using the same approach.
In a MSH the risk score $r_{ijk}$ for a particular transtion $jk$ will often use
only a subset of the coefficient vector, indexed by the \code{cmat} component of
the fit.  For part B above we have abused notation slightly with $(jk)$ standing
for the subset of columns of $D$, $\xbar$ and $z$ that contrbute to
$r_{ijk}$.  
If there are shared coefficients then some columns of $D$ will affect multiple
transitions.  
Since the $\lhat$ term is a scalar we have freedom to move it to the middle
for final term.
A more interesting aspect of term B is that there is no time window: the
contribution to $\hat\beta$ appears at all time points.  
This makes the bookkeeping for accumulation of this term somewhat easier.
