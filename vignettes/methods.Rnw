...
and working $Y$ and strata vectors.  It proceeds column by column through
...
This distinction is inferred at the end of the fit, by identifying 
...
absolute risks.
If any are identified, a phcoef matrix is included that duplicates relevant
rows of cmap, and contains relevant value from the $X$ matrix.
By definition, any time-fixed covariate is not eligible, and (IMHO) it
...
strata. All are 0 in the 1:5 group, are (1,0,0) for all rows in the second,
(0,1,0) for all in the third and (0,0,1) for the last.  The code is a pair
of nested loops over row of cmap and col of smap.

The shared vignette now does this 2 other ways, leading to similar fits
<<nfit2>>=
...
In the above model $X\gamma$ has two coefficients for the 3 bilirubin
groups and one for edema: 10 hazards with an additive structure. To
create the multiplier the survfit routine needs to the $X$ values from
phcoef, and the coefficients from the coefficient vector.
