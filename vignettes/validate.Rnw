...
One of the more common usages in the software
realm is to use it as a synonym for repeatability, i.e. 
...
working out correct answers is hard, tedious work.
This note contains a set of examples, often very simple, that evolved over
multiple decades of my work with survival analysis. 
They have proven very useful in debugging the methods, not least
because all the intermediate steps of each calculation are transparent.
The tests continue to be a resource for defence of the survival package.
...
My reply is that the survival package passes all of the tests below and  
SAS passes most of them.

It is my hope that they can be a resource for devolopers on multiple platforms.
...
is zero at this point, and gives the partial likelihood and hessian
...
Newton--Raphson  iterates are given below.
The data set for the coxph call has one missing value added as a further
check of the routine.
<<>>=
imat <- matrix(0, nrow=6, ncol=4,
               dimnames=list(paste("iter", 0:5),
                             c("beta", "loglik", "U", "H")))
# Exact Newton-Raphson
beta <- 0
for (i in 1:6) {
    imat[i,] <- breslow1(beta)
    beta <- beta + imat[i,"U"]/imat[i,"H"]
}
imat
...
temp <- matrix(0, nrow=5, ncol=4,
               dimnames=list(1:5, c("iter", "loglik", "beta", "H")))
for (i in 0:5) {
    tfit <- coxph(Surv(time, status) ~ x, test1, 
                  ties= "breslow", iter.max=i)
    temp[i,] <- c(tfit$iter, tfit$loglik[2], coef(tfit),
                  1/vcov(tfit))
}
@ 
The \code{coxph} routine declares convergence after 4 iterations.

The martingale residuals are a simple function of the cumulative hazard
which was listed in table \ref{tab:val1},
$M_i = \delta_i - r\lhat(t_i)$.
\begin{center}
\begin{tabular}{c|lrr}
Subject &\multicolumn{1}{c}{$\Lambda_0$} & $\Mhat(0)$ & $\Mhat(\bhat)$ \\ 
	\hline
1& $1/(3r+3)$ & $1-1/6$ & .728714 \\
2& $1/(3r+3)$ & $0-1/6$ & --.271286 \\
...
That is, if the \Co{ties=efron} option is chosen the formulas for
...
their utility.
%{\splus} uses the calculations of this section by default.
...
function, rapidly goes to zero as $\beta$ grows.
   ...
   Both SAS and {\splus} use the Nelson--Aalen estimate of hazard after
...
One consequence of this is that $l$ can be positive for weights $<1$, which
...
When $\beta=0$, the three unique values for $\xbar$ a
at $t=1$, 2, and 4
...
observations).
This is the approach taken by SAS \co{phreg} using the \co{freq}
...
e/(r^2+ 11r+7) &  0.03272832 & e^2/(r^2+ 11r+7)^2 & \section{Test data 4}
