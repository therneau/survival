...
legend(80, .8, c("female", "male"), col=1:2, lwd=2, bty='n')
par(oldpar)
@ 
...
the second scenario in figure \ref{sfig1}.
...
\begin{enumerate}
  \item The event variable was created as a \emph{factor}; 
    ...
    for those whose follow-up terminated without reaching an endpoint.
...
\end{enumerate}
The multi-event curves can be subscripted as though they were a matrix.
The first subscript indexes the groupings found on the right hand side of
the formula, sex in this case, and the second subscript the endpoints.
...
legend(0, 1, c("Males, PCM, incorrect curve", "Males, PCM, competing risk"),
...
legend(48, .04, c("female", "male"), lty=1, col=1:2, lwd=2, bty='n') 
@ 
...
plot(mfit2[2,], lty=c(2,1),
     ...
     xlab="Years post diagnosis", ylab="Prevalence")
lines(mfit4[2,3], mark.time=FALSE, col=2, lty=2, lwd=2,
      conf.int=FALSE)

legend(180, .5, c("male:death w/o pcm", "male: ever pcm", 
