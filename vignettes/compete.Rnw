...
\section{Multi-state prevalence curves}
\subsection{Aalen-Johansen estimate}
A starting point for analysis is to compute and plot 
`prevalence' or `occupancy' estimates.
This is a vector function $p(t)$ that estimates the fraction
of subjects in each state at time $t$.
We compute this using the Aalen-Johansen estimate via the 
\code{survfit} function.

Mathematically the estimate is simple.
For each unique time that an event occurs form the
transition matrix $T(t)$ with elements
 $t_{ij}(t) =$ the fraction of subjects in state $i$ at time $t$
who transition to state $j$ at that time.
...
\begin{equation*}
  T(t) = \left( \begin{array}{cc} 
    [n(t)- d(t)]/n(t)  & n(t)/d(t) \\\\ 0 & 1 \end{array}
  \right)
\end{equation*}
...
recovers the well known cumulative incidence estimate;
...
weight $dp(t)/dw_i$. The $V(t) = D'D$ is the estimated variance-covariance
matrix of the estimates at time $t$.

The $p(t)$ vector estimates the fraction of subjects in each state at
time $t$.  It obeys the obvious constraint that the sum of states at any time
...
(In epidemiology the prevalence is normally defined as the proportion 
among those
who are not dead; here we do not treat the death state any differently
than the others.)
...
1- Pr(dead) interchangeably, but that habit needs to be left behind for
multi-state models, for them the values $1-p_k(t)$ 
...
in the diagram) is rarely included in the final plot and the remaining curves 
rise from 0.
...
I have come to the opinion that the analysis of multi-state data has four
key steps.
\begin{enumerate}
  \item  Draw the figure.
  \item Think through the arrows.
  \item Build a correct data set.
  \item Fit the data. Create prevalence curves and other summaries.
\end{enumerate}

These are it turns out also in order of importance. 
As seen in figure 2 above and the examples, there will often be multiple
possible constructions for a data set, each of which is illuminating.
Step 2 has a number of important subcategories:
\begin{enumerate}
  \item  Draw the figure.
  \item Think through the arrows.
    \begin{enumerate}
      \item Ask again, is this the right or best figure?
      \item Which covariates should be attached to each arrow? 
        Sometimes a covariate is important for one transition but not for
        another.
      \item For which transitions should one or more of the  covariate be 
        constrained to have the same coefficient? 
        Sometimes there will be a bilogic rationale for this.  For other 
        studies an equivalence is forced just because we have too many 
        unknowns and need to thin the herd --- the same reason models
      ...
      \item Which, if any, of the transitions should share the same baseline 
       ...
       \item What is an approriate correlation structure for the random effect,
         ...
         I don't currently have a method to fit it, but I've sometimes
         thought that there would be an effect $b_i$ for one transtion and 
         $k b_i$ for another: the same group(s) are high or low but the effect
         is attenuated for some transition types.
  ...
  \item Fit the data.
    \end{enumerate}
Step 3 will often
be the one that demands the most time and attention to detail.
...
We will illustrate using the competing risks form of the MGUS data.
<<cfit1>>=
mtemp <- mgus2
