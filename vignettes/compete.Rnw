...
``AalenJohansen'', but I use this often and didn't want to type that 
long a name.)
...
endpoint.  For subjects diagnosed at age 80 the Fine-Gray
models predict that more than 
...
abline(0,0, col=3)
legend(12, .9, c("Female, 60", "Male, 60","Female: 80", "Male, 80"),
       col=c(1,2,1,2), lty=c(1,1,2,2), lwd=2, bty='n')
@ 
...
This text will make the most sense if it
...
\code{prev} component with the cumulative sum, and then plot the result
<<figT4, fig=TRUE>>=
temp <- ajfit
...
we have used the order found above in the non-parametric curves \code{fit2}.
<<figT5, fig=TRUE>>=
...
The last plot in the section contains the non-parametric fits.
<<figT9, fig=TRUE>>=
...
round(cbind('1->2'= coef(efit1.2) , '1->3'= coef(efit1.3), 
            '2->3' =coef(efit2.3)), 3)
@ 
These match the coefficients matches the left column of table T3,
...
We will use the fit from column 2 of tabl
