...
the lower left is an example of competing risks, and
 the lower right panel is an illness-death model.}
 \label{mfig1}
\end{figure}
...
models for PCM and for death. Per \cite{Geskus11}, the robust
variance estimate from the coxph call should closely match the variance
formula in Fine and Gray, model coefficients will be identical.
