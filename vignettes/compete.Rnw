...
simple. 
The primary idea is to turn the multi-state problem into a set of
two-state ones.  
For the MGUS data these are (entry/PCM) $\longrightarrow$ death and
(entry/death) $\longrightarrow$ PCM.  
...
second applies the usual \code{coxph} or \code{survfit} programs to
those data sets.
...
pcmdat <- finegray(Surv(etime, event) ~ sex, data=mgus2,
                   endpoint="pcm")
pcmdat[1:4,]
pfit <- survfit(Surv(fgstart, fgstop, fgstatus) ~ sex, data=pcmdat, weight=fgwt)
...
@ 
The model states that males have a lower rate of \emph{observed} PCM, 
...
that the usual Cox model with a single categorical predictor is equivalent
to the log-rank test.
An overlaid plot of the non-parametric Aalen-Johansen estimates for the
PCM state and that of the Fine-Gray estimate show that proportional hazards
is not unreasonable for this question.

The mathematics behind the Fine-Gray estimate starts with
the functions $F_k(t) = P_{1k}(t)$, which can
...
understanding
a covariate --- assuming of course that the assumptions of
...
<<fg3>>=
pdata2 <- finegray(Surv(etime, event) ~ ., mgus2, na.action=na.pass)
ddata2 <- finegray(Surv(etime, event) ~ ., mgus2, na.action=na.pass,
                   endpoint= "death")
...
fgfit2b
@ 
The Fine-Gray show an effect of both variables on the subdistribution
...
\begin{itemize}
  ...
  \item From the per-transition models one can work forward and compute
    the $P(t)$, the occupancy probabilities for each state over time;
    both are useful.
...
model, but I think it is actually a minor complaint, but the others are
substantial.
