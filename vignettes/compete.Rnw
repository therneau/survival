  ...
  \item Think through the rates (arrows).
    \begin{enumerate}
      ...
      \item For which transitions should one or more of the covariates be 
        ...
        unknowns and cannot accomodate them all.
      ...
      \item Which, if any, of the transitions should share the same baseline 
          ...
          be different.
       \item Shoud there be random effects, and if so what is an appropriate
...
obtain $\lambda_{12}$, the rate of transtion to the PCM state for our
four subjects, and $\lambda_{13}$ = the rate of transition to the ``death
before PCM state''.  These are placed into a matrixx and 
...
(Very few subjects aquire PCM more than 15 years after a MGUS diagnosis at
...
multi-state survival curves, though there is not good reason to use this
rather round about approach instead of 
the simpler \code{survfit(Surv(etime, event) ~ sex)}.
<<pfit2>>=
...
@ 
The two new curves are almost identical to the prior estmates, 
...
models for plasma cell malinancy and for death.  
<<fg2, fig=TRUE>>=
...
Average prediction at any age is nearly the same, but 
...
linearity of the continuous variables age and msplike by using
...
\begin{itemize}
  ...
  \item The attempt to capture a complex process in a single number is
    a search for a simplicity that will often not exist.
    ...
    --- the mixture of transition rates to each endpoint --- which is an even
    stronger assumption. 
  \item From the per-transition models one can work forward and compute
    ...
    both the hazard ratio and $P$ are useful summaries of the data.
    There are no tools to work backwards from a Fine-Gray fit to the
    individual hazards.
