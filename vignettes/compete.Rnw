...
The $i,j$ off diagonal element of $\Lamba$ is the the cumulative
...
exp(\Sexpr{round(coef(fgfit)[1], 3)} = \Sexpr{round(exp(coef(fgfit)[1]),1)}
...
and the boxes (prevalence) of figure \ref{fig1} can be examined post fit.
...
<<fine2>>=
fval <- log(1-prev)
round(fval['progression',, 'female'] / fval['progression',, 'male'], 2)
@ 
