...
model, but it is actually a minor complaint.  The first issue is
...
with a simple 0/1 status variable encoding a PCM outcome.
The second $n$ observations are the data set we would have used for the 
`death before PCM' fits, with status encoding the death endpoint.
