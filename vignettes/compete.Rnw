...
When the \code{survfit} call does not contain an \code{istate} directive
then all of the subjects are assumed to begin in an unnamed ``entry''
state, which has a blank label on the printout.
By default this state is not plotted -- point 3 above --
...
Technically, the default is to not plot any unnamed state, unless it is
the only state in the object.
One can easily add this line to the plot if desired, e.g.,
\code{lines(mfit2[,3], col=4, lty=1:2)};
the subscripting operation has removed the first two states.
...
The diagonal elements of $T$ are filled in last and are chosen to make each row
sum to 1.  
...
problem into a set of two-state ones.  
...
second fits weighted \code{coxph} or \code{survfit} model to the data.
...
pcmdat <- finegray(Surv(etime, event) ~ sex, data=mgus2,
                   endpoint="pcm")
pcmdat[1:4,]
pfit <- survfit(Surv(fgstart, fgstop, fgstatus) ~ sex,
                data=pcmdat, weight=fgwt)

deathdat <- finegray(Surv(etime, event) ~ sex, data=mgus2,
                     endpoint="death")
dfit <- survfit(Surv(fgstart, fgstop, fgstatus) ~ sex, 
                  data=deathdat, weight=fgwt)
oldpar <- par(mfrow=c(1,2))
plot(mfit2, col=c(1,2,1,2), lty=c(2,2,1,1),
     lwd=2,  xscale=12,
     xlab="Years post diagnosis", ylab="Probability in State")
text(240, .4, "CR survival")
plot(dfit, fun='event', col=1:2, lwd=2,  xscale=12,
     xlab="Years post diagnosis", ylab="Probability in State")
lines(pfit, fun='event', lwd=2, col=1:2, lty=2) 
text(240, .4, "Survival, FG data")
par(oldpar)
@ 
The two curves are almost identical, and in fact would be identical if we
...
plot(mfit2[,1], col=1:2,
     lwd=2,  xscale=12,
     xlab="Years post diagnosis", ylab="Fraction with PCM")
tfit <- survfit(fgfit1, newdata=data.frame(sex=c("F", "M")))
lines(tfit, fun="event", lty=2, lwd=2, col=1:2)
...
the functions $F_k(t) = P_{1k}(t)$, where $P$ is the probablility in state
function esimated by the AJ estimate.  This can
...
all hold..
In a multi-state model, however, proportional hazards cannot hold for 
both the per-transition and Fine-Gray models formulations;
if it is true for one it will not be true for the other.

Now consider a multivariate fit on age, sex, and serum m-spike.
<<fg3>>=
pdata2 <- finegray(Surv(etime, event) ~ ., mgus2)
ddata2 <- finegray(Surv(etime, event) ~ ., mgus2,
                   endpoint= "death")
fgfit2a <- coxph(Surv(fgstart, fgstop, fgstatus) ~ age + sex + mspike,
                 data=pdata2, weight=fgwt)
fgfit2a

fgfit2b <-  coxph(Surv(fgstart, fgstop, fgstatus) ~ age + sex + mspike,
                 data=ddata2, weight=fgwt)
fgfit2b
@ 
...
risk of death before PCM, while a higher serum m-spike works in the opposite
direction.
The Cox models showed no effect of sex on the instantaneous hazard and none
for serum m-spike on the death rate.
However, as
as shown in the last section the Cox models do predict a lifetime risk
for females.
...
We can check the proportional hazards assumption using the usual tools.
<<fgph>>=
ph1 <- cox.zph(fgfit2a, transform="identity")
ph1
plot(ph1[1])

cox.zph(cfit1, transform='identity')
@ 
We find that the FG model badly violates proportional hazards, whereas
the per-transition fit was reasonable.
A serious violation in PH taints all coefficients in a fit.
We can try a fit on age scale.

<<fgage>>=
endage <- mgus2$age + etime/12
cfit3 <- coxph(Surv(age, endage, event=="pcm") ~ sex + mspike, mgus2)

pdata3 <- finegray(Surv(age, endage, event) ~., mgus2, id=id)
fgfit3 <- coxph(Surv(fgstart, fgstop, fgstatus) ~ sex + mspike, pdata3,
                weight=fgwt)
@ 
...
``females have a 1.2 fold higher risk, after adjustment for age and serum
m-spike''.
This simplicity is not without price, however, and the author is personally
not a fan of the Fine-Gray approach.
There are four issues.
\begin{itemize}
  \item The approach is viable only for competing risks and not for
    any other multi-state model.
  \item Proportional hazards is often violated in the FG setup, and to a
    much larger extent than for per-transition models using the same
    data.
  \item From the per-transition models one can work forward and compute
    ...
    both the hazard ratio and $P$ are useful.
    There are no tools to work backwards from a Fine-Gray summary to the
    individual hazards.
  \item The overall summary is too simplistic in many cases.
  \item The risk sets are odd.
\end{itemize}
The last of these is perhaps the most frequently listed issue with the Fine-Gray
model, but I think it is actually a minor complaint.  The others are
substantial.
...
par(mfrow=c(1,2))
tdata <- expand.grid(mspike=c(.5, 1.5), age=c(6,8), sex=c("F", "M"))
fsurv1 <- survfit(fgfit2a, newdata=tdata)  # time to progression curves
fsurv2 <- survfit(fgfit2b, newdata=tdata)  # time to death curves
plot(fsurv1[,1], xscale=12, col=1, lty=1:2, lwd=2,
     xlab="Years", ylab="FG predicted")
...
A deeper analysis is called for, but will have to be left for Cindy.
...
It is not always easy to reason correctly from observed
