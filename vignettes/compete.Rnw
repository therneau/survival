...
\begin{enumerate}
  ...
  \item The \code{mfit2} object contains curves for all the states, but 
    the entry state is not plotted by default.
...
censoring) and then any states found in the \code{istate} variable,
...
state with a default name of '' (the empty string).
By default any state with this name is not plotted -- point 3 above --
...
\code{lines(mfit2[,3], col=4, lty=1:2)}, which is the ordering
of the states in the printout.
(One can use e.g. \code{mfit2[, 'pcm']} to select a state as
well, but subscripting using an empty string does not work.)
...
\begin{itemize}
  \item The attempt to capture a complex process as a single value will
    in many (perhaps most) cases be
    grasping for a simplicity that does not exist.
    ...
    --- the mixture of transition rates to each endpoint --- which is a
...
model, but I think it is actually a minor complaint.  The first issue is
substantial, and checking the model assumptions in a Fine-Gray fit is
mandatory.

\section{Other software}
\subsection{mstate}
For risk models that have shared coefficients or baseline hazards, one
will fit the set of Cox models for the rates `all at once' on a combined
...
are the data set we would use for time to PCM, a 0/1 status variable
encoding PCM, and the
second $n$ observations are the data set we would have used for the 
death without PCM fits, with status encoding the death endpoint.
A last variable \code{group} would be 'pcm' for the first $n$ and
'death' for the second.  Then fit a model
...
effects for the two endpoints.

When there are multiple states and constraints then creating the necessary
data set can be a challenge, one that is capably addressed by
the \code{mstate} package.  
A second advantage of the stacked data set approach is that all the Cox
model fits are now in one object, which allows for calculation of proper
confidence intervals for the state probabilities $P(t)$.
...
\begin{itemize}
  ...
  \item Survival data arises when we observe the transitions directly, or
    know their timing;  death for example.
\end{itemize}
