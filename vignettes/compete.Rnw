...
Let $A(t)$ be a matrix of cumulative incidence functions, whose $ij$
element is the cumulative incidence of transitions from state $i$ to
state $j$.
$$
...
multi-state models, for them the values $1-P$ 
...
with which subject via the \code{id} argument of \code{survfit} --- both
for valid estimates and valid standard errors.
...
For the multi-state model we need to be more explicit: a sojourn time of 0
...
\begin{align}
  ...
  \log(\Lambda(t))  &= \log(\Lambda_0(t)) e^{X\beta} \label{cumhaz}\\
  S(t)       &= \exp(-\Lambda(t))  \label{surv}
  \end{align}
There is a single linear predictor which governs
both for the rate (the arrow in figure \ref{sfig1}) and the prevalence 
value of the left hand box, which is the survival curve.
...
Risk rises by \Sexpr{round(exp(coef(cfit2)[1]),1)} per decade of age and
males have \Sexpr{round(exp(coef(cfit2)[2]),1)} times a great a hazard
as females
...
@ 
These individual survival curves are not actually of interest, since 
...
values at specified times, but turns out to to also be an easy way to pluck off 
values.)
<<mprev2>>=
...
#  Since they all come from the same pair of Cox models, the time value
...
this model assumes that each covariate's effect on $\log(1-F)$ is a constant,
...
females have a \Sexpr{round(exp(fgfit$coef[1]),2)} fold higher risk.    %$
...
<<compare>>=
cox.f <- log(1- progmat)    #log(1-P) for the 4 time points
...
are displayed below for subjects with the higher
mspike value of 1.5.  
