...
\section{Reproducing the output in ``Tutorial in biostatistics: Competing risks and multi-state models''}
...
The following text will make the most sense if it
...
% Since survival is a required package, we want it to not depend on any
% packages which are not also baseline or required.  So we print out one 
% thing for readers of the vignette, which is what we expect them to do,
% but actually do another, which is to load a local copy of the data sets.
%  The lines below were originally an Sweave block with eval=FALSE, but
% this gives a false positive CRAN check wrt requirement of mstate.
%
\begin{verbatim}
