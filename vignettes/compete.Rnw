...
These outcomes are in to order of the levels of \code{event},
...
newdata <- tmerge(newdata, mgus2, id, pcm = event(ptime, pstat))
...
One choice would be to move their malignancy date back: though
undetected, the disease had to have been present for some amount of time.
But how far back, and how would it affect the plot?
Below we will simply consider them a death on that day, effectively
not passing through the PCM state.
(The last tmerge line above creates a variable \code{enum} which
simply counts rows for each person.)
...
plot(mfit2[2,], lty=c(2,1),
     xscale=30.5, mark.time=FALSE, lwd=2, 
     xlab="Years post diagnosis", ylab="Prevalence")
lines(mfit4[2,3], mark.time=FALSE, xscale=30.5, col=2, lty=2, lwd=2,
      conf=FALSE)

legend(4, .5, c("male:death w/o pcm", "male: ever pcm", 
...
and the interquartile range is close to 1g.
Meantime the effect of age on progression rate is much less pronounced,
with a coefficient 1/4 that for mortality, and the effect of sex
is neglible.
...
The average rate of progression is about 2.7\% per year, as shown below,
while the average post diagnosis lifetime is about 18 months longer for 
females.  The competing risks plot below shows
<<mpyears>>=
pfit1 <- pyears(Surv(ptime, pstat) ~ sex, mtemp, scale=30.5)
round(100* pfit1$event/pfit1$pyears, 1)  # PCM rate

temp <- summary(mfit1, rmean="common")
