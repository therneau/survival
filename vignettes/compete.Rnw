...
pcmdat <- finegray(Surv(etime, event) ~ sex, data=mgus2,
                   endpoint="pcm")
pcmdat[1:4,]
pfit <- survfit(Surv(fgtime1, fgtime2, fgstatus) ~ sex, data=pcmdat, weight=fgwt)

deathdat <- finegray(Surv(etime, event) ~ sex, data=mgus2,
                     endpoint="death")
dfit <- survfit(Surv(fgtime1, fgtime2, fgstatus) ~ sex, 
                  data=deathdat, weight=fgwt)
par(mfrow=c(1,2))
plot(mfit2, col=c(1,2,1,2), lty=c(2,2,1,1),
     lwd=2,  xscale=12,
     xlab="Years post diagnosis", ylab="Probability in State")
text(240, .4, "Competing risks survival")
plot(dfit, fun='event', col=1:2, lwd=2,  xscale=12,
     xlab="Years post diagnosis", ylab="Probability in State")
lines(pfit, fun='event', lwd=2, col=1:2, lty=2) 
text(240, .4, "Ordinary survival, FG data")
@ 
...
model.  
<<fg2>>=
fgfit1 <- coxph(Surv(fgstart, etime, event) ~ sex, data=pcmdat,
               weight= fgwt)
summary(fgfit1)
...
by a factor of \Sexpr{round(exp(coef(fgsex)), 2)}, and that
...
We found earlier that sex has an effect on death rates but not on
PCM, and conversely that the size of the serum m-spike has an effect
on PCM but not death rates.  
In the non-parametric curves both variables had an effect on the lifetime
probabilities of ``death without PCM'' and ``PCM before death''.
<<fg3>>=
pdata2 <- finegray(Surv(etime, event) ~ ., mgus2, na.action=na.pass)
ddata2 <- finegray(Surv(etime, event) ~ ., mgus2, na.action=na.pass,
                   endpoint= "death")
fgfit2a <- coxph(Surv(fgtime1, fgtime2, fgstatus) ~ age + sex + mspike,
                 data=pdata2, weight=fgwt)
fgfit2a

fgfit2b <-  coxph(Surv(fgtime1, fgtime2, fgstatus) ~ age + sex + mspike,
