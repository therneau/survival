...
upper right of figure \ref{mfig1} exactly recovers the CI estimate:
it is also a special case of the Aalen-Johansen.
...
\begin{enumerate}
  ...
  \item Each subject's initial state is specified by the \code{istate}
    ...
    are assumed to all start from an entry state named '' (the empty
    string), as displayed in the printout above.
\end{enumerate}

The printout shows that a male subject will spend, on average,
8.7 of the first 20 years post diagnosis in the entry state, 1.1
...
The result of a multi-state survfit is a matrix of probabilities with
...
By default an unnamed state is not plotted -- point 3 above --
...
The mean time in state for PCM is higher for mfit2 and mfit3, since in
the former someone who converts to PCM at 5 years, say, will spend the
next 15 in the PCM state.
...
I have often found the three curve display below useful in the case
...
Risk rises \Sexpr{round(exp(10*coef(cfit2)[1]),1)} fold per decade of age and
...
\end{equation*}
The standard survival curve calculations for a Cox model can be used to
...
The cumulative hazard portion of the result is what is used to
build an Aalen-Johansen estimate.
(Calling \code{survfit} on a set of \code{survfit} objects is, I admit, a
bit confusing.  
It perhaps would have been better to name the second routine
``AalenJohansen'', but I didn't want to type that long a name.)

Sex had nearly no effect on the hazard of PCM, i.e., on any given day the
risk of conversion for a male is essentially the same as for a female of the
same age.  
We see above that the fitted Cox models predict a higher lifetime risk 
...
age 80 for the obvious reason: very few of them are predicted to still 
be alive.)
...
The resulting survfit object is easy to plot or print using standard calls.
...
In the upper right panel of figure 1, draw a circle around all of the
...
models for plasma cell malignancy and for death.  
<<fg2, fig=TRUE>>=
...
the functions $F_k(t) = P_{1k}(t)$, where $P$ is the probability in state
...
$T^*= I(\mbox{etype}=k)T + I(\mbox{etype}\ne k)\infty$.
...
fgfit2b
@ 
The Fine-Gray fits show an effect of both variables on the subdistribution
...
direction.
The Cox models showed no effect of sex on the instantaneous hazard and none
...
Now compute predicted survival curves for the model, and show it
...
All are obvious and important next steps.
...
endpoint.  For subjects diagnosed at age 80 it predicts that more than 
100\% will eventually either progress or die without progression.
...
and serum m-spike''.
This simplicity is not without a price, however, and the author is personally
not a fan of the Fine-Gray approach.
...
are the data set we would use for a time to PCM analysis, 
with a simple 0/1 status variable encoding the PCM outcome.
The second $n$ observations are the data set we would have used for the 
`death before PCM' fits, with status encoding the death-before-PCM endpoint.
A last variable \code{group} is added which is 'pcm' for the first $n$ 
observations and
...
\section{Tutorial in biostatistics}
...
This next section shows how to recreate the graphs and tables from
this paper, it parallels a similar document that is a vignette
in the \code{mstate} package.  This section will make the most if it
...
The fact that they cross is used to emphasise the inconsistency of the
...
Figure T4 is the stacked plot, of the competing risks outcome.
We can modify the ajfit object by replacing each row of the
\code{prev} component with the cumulative sum.
<<figT4, fig=TRUE>>=
temp <- ajfit
temp$prev <- t(apply(temp$prev, 1, cumsum))  # apply transposes (why?)
...
the second is now the different in CCR-5 effect between the two endpoints.
Which form one prefers is a matter of taste.
In the tutorial, the stacked data set also has indicator variables for
the types.  We can repeat that instead of using the interaction to
...
with obtaining the simple predictions for the two levels of
CCR-5 and for the two endpoints = four curves.
...
we have used the order found in \code{fit2} above.
<<figT5, fig=TRUE>>=
...
Now proceed to Fine-Gray fits for AIDS and SI appearance.
<<figT8, fig=TRUE>>=
...
newdata <- data.frame(ccr5=c("WM", "WW"))
fgsurv1 <- survfit(fgfit1, newdata)
plot(fgsurv1, col=1:2, lwd=2, fun='event', xmax=13, 
     xlab="Years from HIV infection", ylab="Probability")
fgsurv2 <- survfit(fgfit2, newdata)
lines(fgsurv2, col=1:2, lwd=2, lty=2, fun='event', xmax=13)
legend(0, .4, c("AIDS, WM", "AIDS, WW", "SI, WM", "SI, WW"),
       col=1:2, lty=c(1,1,2,2), bty='n', lwd=2, cex=1.2)
@ 

The predicted curves based on the Fine-Gray model.
<<figT9, fig=TRUE>>=
fit3 <- survfit(Surv(time, event) ~ ccr5, adata)
oldpar <- par(mfrow=c(1,2))
plot(fit3[,1], col=1:2, lty=c(1,1,2,2), lwd=2, xmax=13,
     ylim=c(0, .5),
     xlab="Years from HIV infection", ylab="Probability")
title("AIDS")
plot(fit3[,2], col=1:2, lty=c(1,1,2,2), lwd=2, xmax=13,
     ylim=c(0, .5), 
     xlab="Years from HIV infection", ylab="Probability")
title("SI appearance")     
par(oldpar)
...
Next we create the analysis data set \code{edata}.
...
platelet recovery.  
We also shorten one of the factor label so as to better fit
...
We then generate the multistate $P(t)$ curves, a 
...
edata <- tmerge(temp, ebmt3, id, 
                ...
                enum  = cumtdc(prtime))
print(edata[15:20,-(3:5)], digits=2)
...
Subjects can go from baseline to platelet recovery (temp1),
baseline to death/relapse (temp2) or from PR to the death/relapse state
(temp3).
<<data2>>=
...
The first fit, found in the left colums of table T3, has separate strata
and covariate effects for each of the three transitions.
We can fit the model in multiple ways: first as 3 separate fits
(efit1.1, 1.2, 1.3), then using the stacked data set and a strata by
covariate interaction (efit1b) and then rewriting the model to get a
slightly different variable coding (efit1c).
In the first two fits below everyone has \code{tstart=0} and so 
the variable can be left out of the model statement.
<<efit1>>=
...
matrix(c(efit1.2$loglik + efit1.3$loglik + efit2.3$loglik, 
       ...
       dimnames=list(c("Initial LL", "Final LL"), 
                     c("fit a", "fit b", "fit c")))
@ 
The first set of coefficients matches the left column of table T3,
...
The last fit gives the same coefficients as the separate fits but in a different
order.  
We can see from the log-likelihoods that the fits are equivalent.
...
Again there are multiple ways to do this.  Since this leaves the
...
(In table T3 the first 6 coeffiencts in columns 2 and 3 are left blank since
...
Rather than using the plot of figure T14 along a ``they look parallel''
...
We chose to plot on the $\sqrt{t}$ scale to emphasise the first year, where
more events occur.  There is little formal evidence for non-proportional
hazards, with a p-value of 0.27, though the confidence limit are very wide.
<<zph, fig=TRUE>>=
...
The coefficients in table S4 use a ``clock reset'' approach, where time
is measured from the entry to the state. 
These fits use the same data set, but with \code{Surv(tstop-tstart, status)}
rather than \code{Surv(tstart, tstop, status)} as the left hand side
of the equation.
...
In short, a model man not include a time dependent covariate whose value (at
any given event time) is dependent on the future.
In this case we are okay.  Because all the observations are on a common
time scale, the value of \code{start} does not become ``known'' for subject
...
table and the others follow as in the above code for table T3.
...
We now predict the future state of a patient, using as as our reference
set two subjects who are $\le 20$ years old, gender matched, AML, with and
without T-cell depletion. 
We will use the fit from column 2 of table T3, which has proportional hazards 
...
re-do the above calculation as a four state model.  
In order to make it easy to do the stacked plot that they show, the states
matrix is created in a particular order.
...
themselves enamoured of the stacked form, at least partly because the
impression it gives can depend on the order of the states.
We split it as two figures as the version with both TCD and no TCD together
had too many crossing lines.  Figure T15 corresponds to the left hand panel.
<<figT4, fig=TRUE>>=
tcurve <- ecurve2
tcurve$prev <- t(apply(tcurve$prev, 1, cumsum))
oldpar <- par(mfrow=c(1,2))
plot(tcurve[1,1:3], col=1, xscale=365.25, ylim=c(0,1), 
      xlab="Years since transplant", ylab="Predicted probabilities")
text(rep(6*365, 4), c(.2, .5, .7, .9), cex=.7, 
     c("Alive in remission, PR", "Relapse or death after PR",
       "Relapse or death without PR", "Alive in remission, no PR"))
title("No TCD")
plot(tcurve[2,1:3], col=1, xscale=365.25, ylim=c(0,1), 
      xlab="Years since transplant", ylab="Predicted probabilities")
text(rep(6*365, 4), c(.3, .65, .82, .95), cex=.7, 
     c("Alive in remission, PR", "Relapse or death after PR",
       "Relapse or death without PR", "Alive in remission, no PR"))
title("TCD")
par(oldpar)
...
\code{efit2b}.  This will be neccesary for more models with shared
coefficients across some strata, for instance, since they can
only be fit using the stacked data form
At the moment the survfit routine gets confused by these more 
complex models, but we are working on rectifying this.

\section{Other software}
When there are thee states or so with multiple transtions and contraints, then
the `by hand' approach used above for creating a stacked data sets, labeling
coefficients, and producing mult-state curves all become a challenge.
(It is still fairly easy to do, just not easy to ensure it has
been done \emph{correctly}.)
The \code{mstate} package starts with a definition of the matrix of possible
transitions and uses that to drive tools that build and analyse the stacked
data set in a more automated fashion. 
We highly recommend it for more complex models.
A second advantage mstate is that all the Cox
...
The msm package addresses panel data, while survival, mstate, and a host of
others are devoted to the second.
