...
In simple studies we have gotten used to the idea of using P(dead) and
...
the default behavior was chosen.
...
Matrix package, the \code{mstate} package uses a different computational
path that avoids matrix exponentials.
For illustration we will compute the 20 year probabilities of PCM from the
...
states: initial diagnosis, PCM and death, but only the $1\rightarrow 2$
...
chaz1 <- summary(surv1, times=60*1:4)$cumhaz 
chaz2 <- summary(surv2, times=60*1:4)$cumhaz 
prev <- matrix(0, 4, 8) #progression curves
for (i in 1:4) {
    for (j in 1:8) 
        prev[i,j] <- pfun(chaz1[i,j], chaz2[i,j])[1, 3]
}
prev <- array(prev, dim=c(4,4,2))
dimnames(prev) <- list(c(5,10,15,20),
                       c("50:0.5", "50:1.5", "80:0.5", "80:1.5"),
                       c("female", "male"))
round(100*prev[4,,], 1)  #males and females at 20 years
@ 
The above table shows that females
have a higher risk of \emph{observed} progression, even
though their hazard at any given moment is nearly identical to males.
The difference at 20 years is on the order of our ``back of the envelope''
...
Proving that $P_{1k}$ as computed by a matrix exponential is equivalent 
to $CI(t)$ is well beyond the scope of this vignette.
(The label ``CI''  is one of the more unfortunate ones in the survival lexicon,
...
compute appropriate confidence intervals.  
...
is an analog to the survival probability in the two state model: it is the
probability that you will \emph{not} be in state $k$ by time $t$.  
Unlike actual survival curves $F(\infty)$ is normally not zero.
They refer to $\log(F)$ as a \emph{subdistribution} function and assume that
\begin{equation}
...
this model assumes that each covariate's effect on $\log(F)$ is a constant,
independent of other variables.  
Both assumptions are wonderfully simplifying with respect to understanding
a covariate --- assuming of course that the particular assumption is correct.
...
exp(\Sexpr{round(fgfit$coef[1], 3)} = \Sexpr{round(exp(fgfit$coef[1]),2)}
...
model is that we can't go backwards: the matrix exponential $\exp(A)$ is
defined for an transition matrix $A$, but $\log(B)$ exists
for only select matrices $B$.
...
<<compare>>=
fine.f <- log(1- prev)    #log(1-P) for the 4 time points
round(fine.f[,,1] / fine.f[,,2], 2)
@ 
...
follow-up times.  The overall average, however, is similar.
...
It is often very useful, but is one available option among many.
