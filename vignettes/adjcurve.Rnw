...
The first, sometimes known as \emph{marginal} analysis, first modifies
the data so that it is balanced with respect to the confounding variables,
by forcing the distribution of each subgroup to match that of the target.
...
ajusted for age and sex.  The reference population will describe and age and
...
The set $c$ might be empty, producing single overall curve, but this is
the uncommon case.
...
\begin{center}
  \begin{tabular}{rccc}
    ...
    & $\longrightarrow$ & averge the predictions for each $c$
   \end{tabular}
\end{center}
...
immunoglobulins; PC comprise a small portion ($<1$\%) of the 
...
A subset of the Dispensieri study is available is available in the survival
...
subjects by a dilegent searcher, and so only a subset of the study has been
made available as a measure to strengthen anonyminity.
...
We have chosen in this analysis analysis to exclude 
...
and how much is simply an artifact of age?
 (The cutpoints are arbitrary, but we have chosen to mimic the original study
...
reference group.
\begin{enumerate}
  \item Emprical.  The overall distribution of confounders $x$ in the
    data set as a whole.  For this study we would use the observed
    age/sex distribution, ignoring FLC group.  This is also may be
  ...
  \item Internal reference.  A particular subset of the data is
    ...
    it.  
\end{enumerate}
Method 2 is common in epidemiology, using a reference popultion based
...
such that the its disctribution matches
...
As important is that though it returns an unbiased comparison, the result
...
sample itself as
our target distribution $\pi$, i.e., the emprical distribtion.
...
case, as is diplayed in figure \ref{flc3a}.
...
of the weigths for the females.  The table was to have a miniumn weight
...
The robust score test from \texttt{coxph} corresponds to a logrank test 
