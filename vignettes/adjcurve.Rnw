...
presence of confounders is less well known, and is the topic
of this note.

\begin{figure}[hb] \centering
  \begin{tabular}{rccc}
    Marginal: & balance data on $c$& $\longrightarrow$ &  
    survival curves by $x$ \\
    Conditional: & survival curves by $x$ and $c$ &  $\longrightarrow$ & 
        average curves over $c$ to balance
  \end{tabular}
  \caption{Two approaches}
  \label{figtwo}
\end{figure}
    
Assme that we have an effect of interes $x$ and a set of possible confounding
variables $c$. There are two main approaches to adjustment.
The first approach, sometimes known as \emph{marginal} analysis,
modifies the data so that
the confounders $c$ are balanced across the factor of interest $x$, we can then
proceed with simple analyses of survival verus $x$ using the 
...
examinining the conditional survivals given $x$ and $c$.
From these conditional survivals average curves are created that balance
on the confounder.
As shown below, these differ essentially in the \emph{order}
in which the two necessary operatations are done, balancing and survival
curve creation.
\begin{center}
  \begin{tabular}{rccc}
    Marginal: & balance data on $c$ & $\Longrightarrow$ & 
             form survival curves by $x$ \\
    Conditional: & form survival curves by $x$ and $c$& $\Longrightarrow$ &
        average curves over $c$ to balance
  \end{tabular}
\end{center}


Many analyses use combinations of these, of course, balancing on
...
and her collleagues at the Mayo Clinic \cite{Dispenzieri12}.
...
the circulating antigen will be the product of a single malignant clone.
...
In 2010 Dr. Dispensizeri assayed a subfraction of the immunogloblin,
...
Our data set is an age and sex stratified sample of this second data set
and contains 7874 of the subjects from the original study.
It is included in the survival package as the data frame \texttt{flchain}.
...
those in the 70--89th pecentile, and the remainder.
...
<<flc1, echo=FALSE, fig=TRUE, include=FALSE>>=
group3 <- factor(1+ 1*(flchain$flc.grp >7) + 1*(flchain$flc.grp >9),
                      levels=1:3, 
                      labels=c("FLC < 3.38", "3.38 - 4.71", "FLC > 4.71"))
flchain$group3 <- group3
sfit1 <- survfit(Surv(futime, death) ~ group3, flchain)
...
temp <- with(flchain, table(group3, age2, sex))
...
for (i in 1:dd[1]) {
    for (j in 1:dd[2]) {
        for (k in 1:dd[3]) {
            indx <- which(as.numeric(group3)==i &
                          as.numeric(age2) ==j &
                          as.numeric(flchain$sex) ==k)
            select <- c(select, sample(indx, size[j,k]))
        }
    }
}

data2 <- flchain[select,]
sfit2 <- survfit(Surv(futime, death) ~ group3, data2)
plot(sfit2, mark.time=F, col=c(1,2,4), lty=1, lwd=2,
     xscale=365.25, xlab="Years from Sample", 
     ylab="Survival")
lines(sfit1,  mark.time=F, col=c(1,2,4), lty=2, lwd=1,
      xscale=365.25)
legend(2,.4, levels(flchain$group3), lty=1, col=c(1,2,4),
               bty='n', lwd=2)
@ 
...
FLC group & 50--54& 55--59& 60--64 & 65--69 & 70--74 & 75--79 & 80--89& 90+ \\
   \hline
<<echo=F, results=tex>>=
age2 <- cut(flchain$age, c(49,54, 59,64, 69,74,79, 89, 110))
levels(age2) <- paste(c(50,55,60,65,70,75,80,90),
                      c(54,59,64,69,74,79,89,109), sep='-')
flchain$age2 <- age2

tab1 <- with(flchain, table(group3, age2, sex))
...
\end{table}
 ...
 One way to balance the data set is to select a subset of subjects
such that the subset is balanced.
...
tab3 <- with(flchain, table(age2, group3))
tab3 <- round(100*scale(tab3, center=F, scale=colSums(tab3)))
tab4 <- with(data2, table(age2, group3))
...
and is applied in the situation
...
interpretation. 
Interpretation of the curves for the FLC data is much less clear;
we have gained an unbiased comparison, but for a very peculiar
population of subjects.

<<echo=FALSE>>=
# I can't seem to put this all into an Sexpr
z1 <- with(flchain,table(age, sex, group3))
z2<- apply(z1, 1:2, min)
ztemp <-  3*sum(z2)
z1b <- with(flchain, table(age>64, sex, group3))
...
homogeneous within age group,
yet large enough so as to limit the number of subjects excluded.
For instance, if we had divided the
...
larger, however, 
%with \Sexpr{ztemp2} in each group rather than
%\Sexpr{nrow(data2)/3}. 
due to less stringent balancing criteria.
On the other hand, if we had done a very fine division, say by age in weeks,
the resulting matched data set will be reduced to almost zero;
...
create acceptably homogeneous groups, but this is not common.
...
<<>>=
survdiff(Surv(futime, death) ~ group3, data=data2)
@ 
...
New weights are the values $\pi/p$ = pi.us/tab100.
<<>>=
refpop <- uspop2[as.character(50:100),c("female", "male"), "2000"]
pi.us  <- refpop/sum(refpop)
age100 <- factor(ifelse(flchain$age >100, 100, flchain$age), levels=50:100)
tab100 <- with(flchain, table(age100, sex, group3))/ nrow(flchain)
...
tab2 <- with(flchain, table(age2, sex, group3))/ nrow(flchain)
us.wt <- rep(pi.us, 3)/ tab2
range(us.wt)
index <- with(flchain, cbind(as.numeric(age2), as.numeric(sex), 
                           as.numeric(group3)))
flchain$uswt <- us.wt[index]                      %$
sfit3a <-survfit(Surv(futime, death) ~ group3, data=flchain, weight=uswt) 
@ 
\begin{figure}[tb]
  \myfig{flc3a}
...
based on these two reference populations nearly overlap.

<<flc3a, echo=T, fig=TRUE, include=FALSE >>=
tab1 <- with(flchain, table(age2, sex))/ nrow(flchain)
matplot(1:8, cbind(pi.us, tab1), pch="fmfm", col=c(2,2,1,1),
        xlab="Age group", ylab="Fraction of population",
        xaxt='n')
axis(1, 1:8, levels(flchain$age2))

tab2 <- with(flchain, table(age2, sex, group3))/nrow(flchain)
tab3 <- with(flchain, table(group3)) / nrow(flchain)

rwt <- rep(tab1,3)/tab2 
round(rwt[,1,], 1) #show female data
flchain$rwt <- rwt[index]  # add per subject weights to the data set
sfit3 <- survfit(Surv(futime, death) ~ group3, data=flchain, weight=rwt)
@ 
...
The score test from \texttt{coxph} is equivalent to the log-rank test that
is often used to label curves.
<<>>=
id <- 1:nrow(flchain)
cfit <- coxph(Surv(futime, death) ~ group3 + cluster(id), data=flchain, 
              weight=rwt)
summary(cfit)$robscore

require(survey)
sdes <- svydesign(id = ~0, weights=~rwt, data=flchain)
dfit <- svykm(Surv(futime, death) ~ group3, design=sdes)
...
#                 data=flchain, weight=rwt)
@ 
Note: including the \texttt{cluster} term in the coxph call causes it to
treat the weights as resampling values and thus to use the proper 
...
regression model, leading to \emph{inverse probabililty weighting}
as an alternate label for this approach.
We can reproduce the weights calculated just above with three
logistic regression models.
<<echo=T>>=
options(na.action="na.exclude")
gg <- as.numeric(flchain$group3)
lfit1 <- glm(I(gg==1) ~ factor(age2) * sex, data=flchain,
            family="binomial")
lfit2 <- glm(I(gg==2) ~ factor(age2) * sex, data=flchain,
            family="binomial")
lfit3 <- glm(I(gg==3) ~ factor(age2) * sex, data=flchain,
            family="binomial")
temp <- ifelse(gg==1, predict(lfit1, type='response'),
               ifelse(gg==2, predict(lfit2, type='response'),
                      predict(lfit3, type='response')))
all.equal(1/temp, flchain$rwt)
@ 
...
lfit1b <-glm(I(gg==1) ~ age + sex, data=flchain,
            family="binomial")
lfit2b <- glm(I(gg==2) ~ age +sex, data=flchain,
            family="binomial")
lfit3b <- glm(I(gg==3) ~ age + sex, data=flchain,
            family="binomial")
...
tdata <- data.frame(flchain, lwt=twt)

#grouped plot for the females
temp <- tdata[tdata$sex=='F',]
temp$gg <- as.numeric(temp$group3)
...
for each of the groups is \Sexpr{nrow(flchain)}, 
...
<<>>=
# compute new weights
wtscale <- table(flchain$group3)/ tapply(flchain$rwt, flchain$group3, sum)
wt2 <- c(flchain$rwt * wtscale[flchain$group3])
c("rescaled cv"= sd(wt2)/mean(wt2), "rwt cv"=sd(flchain$rwt)/mean(flchain$rwt))

cfit2a <- coxph(Surv(futime, death) ~ group3 + cluster(id),
                data=flchain, weight= rwt)
cfit2b <- coxph(Surv(futime, death) ~ group3 + cluster(id),
                data=flchain, weight=wt2) 
...
for each subject that accounts for flc group, and then take a weighted
...
constant in each strata with each age/sex stratum.  Homogeneity of
...
<<echo=TRUE>>=
allfit <- survfit(Surv(futime/365.25, death) ~ group3 + 
                               age2 + sex, flchain)
...
this was originally designed to allow printout of curves at selected
...
pi <- with(flchain, table(age2, sex))/nrow(flchain)  #overall dist
for (i in 1:3) {
...
<<>>=
survdiff(Surv(futime, death) ~ group3 + strata(age2, sex), flchain)
@ 
...
cfit4a <- coxph(Surv(futime, death) ~ age + sex + strata(group3),
               data=flchain)
...
since that will impose a constraint of proportional hazards across the
3 FLC groups.
...
We can address the second problem by doing a proper average.
The Cox model fit above can produce the predicted curves for any
age/sex combination. 
The most straighforward approach is to retrieve the predicted individual 
curves for all
\Sexpr{nrow(flchain)} subjects in the data set, assuming each of the three
...
\Sexpr{nrow(flchain)} curves.  However there are only 98
unique age/sex pairs in the data and it is sufficient to obtain survival
curves for this subset and take a weighted average.
We will make use of the survexp function, which is designed to create
population average curves.
Start by creating a data set which has one row for each age/sex combination
along with its count.
The replicate it into 3 copies, assigning one copy to each of the three
FLC strata.  
The predicted curve for the first copy corresponds to a population that
has 

<<flc6, echo=TRUE, fig=TRUE, include=FALSE>>=
temp <- with(flchain, table(age, sex))
uage <- as.numeric(dimnames(temp)[[1]])
tdata <- data.frame(age=uage[row(temp)],
                    sex=c("F","M")[col(temp)],
                    count= c(temp))
tdata3 <- tdata[rep(1:nrow(tdata), 3),]  #three copies
tdata3$group3 <- factor(rep(1:3, each=nrow(tdata)), 
                       labels=levels(flchain$group3))
sfit4a <- survexp(~group3, data=tdata3, weight = count, 
                    ratetable=cfit4a) 

                    
uage <- sort(unique(flchain$age))  #age 98 has none, nuisance
tdata <- data.frame(age=rep(uage, 2),
                    sex=factor(rep(levels(flchain$sex), each=length(uage))))
sfit4a <- survfit(cfit4a, newdata=tdata, se.fit=FALSE)
tab4a <- with(flchain, table(age, sex))
sfit4a$surv <- sfit4a$surv %*% c(tab4a/sum(tab4a))
...
survfit function has a matrix of survival curves \texttt{sfit4a\$surv} 
which is a matrix of 104 columns, one for each row in the tdata data
frame.  
The matrix multiplication replaces it with a single column containing
the weighted average of these, which also happens to be a legal survfit
object and thus is easy to plot.  
A better approach (for anyone but the author of the package) is to use
the survexp function, which is designed for just such a case.
...
\begin{figure}
  ...
  \caption{Left panel: comparison of Cox model based adjustment (solid) with the
     curves based on a marginal adjustment.  
...
Suspicion should first fall on the use of a simple linear model for
age and sex. 
Start by fitting a slightly more refined model that allows
...
cfit4b <- coxph(Surv(futime, death) ~ age*sex + strata(group3),
                flchain)
sfit4b <- survfit(cfit4b, tdata, se=F)

sfit4b$surv <- sfit4b$surv %*% c(tab4a/sum(tab4a))
plot(sfit4b, mark.time=FALSE, fun='cumhaz', xscale=365.25,
     xlab="Years from sample", ylab="Cumulative Hazard")
lines(sfit3, mark.time=FALSE, fun='cumhaz', xscale=365.25, lty=2)

temp <- median(flchain$sample.yr)                                %$
mrate <- survexp.mn[as.character(uage),, as.character(temp)]
crate <- predict(cfit4b, newdata=tdata, reference='sample', type='lp')
crate <- matrix(crate, ncol=2)[,2:1] # mrate has males then females
...
matplot(mrate, crate,  pch='mf')
abline(0, 1)
@ 
The resulting curves are shown in the left panel of
figure \ref{flc6b} and reveal that 
...
Since this would be a very large number of curves to plot ---
age by sex by FLC = 48 --- an overlay of the observed and expected
...
obs <- with(flchain, tapply(death, list(age2, sex, group3), sum))
pred<- with(flchain, tapply(predict(cfit4b, type='expected'),
              list(age2, sex, group3), sum))
...
round(excess, 1)
@ 
The excess risks, defines as the observed/expected number of deaths,
...
in figure \ref{flc6b} was observed to lie.
...
<<>>=
cfit5a <- coxph(Surv(futime, death) ~ group3:age +sex + 
                strata(group3), flchain) 
cfit5b <- coxph(Surv(futime, death) ~ group3:(age +sex) + 
                strata(group3), flchain)
cfit5c <- coxph(Surv(futime, death) ~ group3:(age *sex) + 
                strata(group3), flchain)

options(show.signif.stars=FALSE) #remove bad option
...
or separate age coefficients per sex does not add importantly
beyond that 
\footnote{There are certain TV shows that make one dumber just by
  ...
  statisticians.  
  We have purposefully turned off the show.signif.stars option.}.
The age coefficients decrease with FLC group, as predicted by
the prior pattern of residuals.
...
pred5a <- with(flchain, tapply(predict(cfit5a, type='expected'),
              list(age2, sex, group3), sum))
...
sfit5a <- survfit(cfit5a, newdata=data.frame(tdata, group3="FLC < 3.38"), 
                 se=FALSE)
sfit5b <- survfit(cfit5a, newdata=data.frame(tdata, group3="3.38 - 4.71"),
                  se=FALSE)
sfit5c <- survfit(cfit5a, newdata=data.frame(tdata, group3="FLC > 4.71"),
                  se=FALSE)
plot(sfit3, mark.time=FALSE, fun='cumhaz', xscale=365.25, lty=2,
     xlab="Years from sample", ylab="Cumulative Hazard")
@ 
