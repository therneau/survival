...
Assume that we have an effect of interest $x$ and a set of possible confounding
...
proceed with simple analysis of survival versus $x$ using the 
...
examining the conditional survivals given $x$ and $c$.
...
In 2010 Dr. Dispenzieri assayed a sub fraction of the immunoglobulins,
...
Therefore, all the analyses below have excluded subjects with less than 7 days of
follow-up.
...
and the result from the re weighted sample is called the \emph{direct adjustment}
method. 
...
(In the example below the sykm function is only run if the survey package
...
This simplifies using the derived weights with the original data set.
...
symbols do not over plot one another, but in this case the simple linear
