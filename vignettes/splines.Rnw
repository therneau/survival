...
%\VignetteIndexEntry{Splines and interactions}
...
\title{Spline terms with interactions in a Cox model}
\author{Terry Therneau}

\begin{document}
\maketitle
This is a topic that comes up just often enough in my work that I end up
re-discovering how to do it correctly about once a year.
A note showing how may be useful to others.
...
your breath).
Thus we need to create our own dummy variables to handle the interaction.
...
In this case, we will decide to use 65 year old males as our reference, with
...
\begin{enumerate}
  ...
  \item We had decided to center the female curve at age 65, risk =1.  The
    ...
    the curves should be at that point.
  \item Get the predicted curve and confidence bands for the females
    ...
    for a 65 year old female.  Do the same for males, also adding in
    the curve separation.
...
For questions of parallism and shape, as here, it seemed best to
ignore it;
