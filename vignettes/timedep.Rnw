...
In the above simple data set this means that we cannot assume a creatinine
of 1.1 on day 30, for instance;  on day 30 the later value of 1.5 has
not yet been seen.
...
=======
It is a subtle function and still undergoing growing pains;
we illustrate it below with a set of
>>>>>>> other
...
<<<<<<< local
    the tstop and (optional) tstart arguments, or implicitly via
    first adding a terminal event as will be done in an example below.
    This first data set has \Sexpr{nrow(cgd0)} rows, the same number as
=======
...
<<<<<<< local
 \item Additions with a missing time value are ignored.
=======
...
<<<<<<< local
    (These would be ignored.) An event at
=======
    An event at
>>>>>>> other
...
\begin{enumerate}
  \item The coefficients in table 6.1 of the definitive analysis found in
    ...
    in precisely the same way, since they include interactions.
    (Table 5.2 in the original edition).
    For age this is (age in days)/ 365.25 - 40 
    years, and for year of enrollment it is the number of years since the
<<<<<<< local
    start of the study: (entry date - 1967/10/1)/365.25.  We'll ignore
    this for now.
=======
...
<<<<<<< local
    time interval for the \code{coxph}.  It suffices to have them die on day
    0.5.
=======
...
between 0 and 1, it will not affect the results.
=======
...
>>>>>>> other
<<stanford>>=
tdata <- with(jasa, data.frame(subject = 1:nrow(jasa),
<<<<<<< local
                               futime= pmax(.5, tx.date - accept.dt),
                               txtime= ifelse(tx.date== accept.dt,
=======
...
>>>>>>> other
                               fustat = fustat))
<<<<<<< local
sdata <- tmerge(jasa[,c("age", "surgery")], tdata, id=subject,
=======
sdata <- tmerge(jasa[,c("age", "surgery", "accept.dt")], tdata, id=subject,
>>>>>>> other
...
# model 6 of the table
coxph(Surv(tstart, tstop, death) ~ age*transplant + surgery + year, 
...
functions
<<<<<<< local
may have a second argument, which will be used as the value
=======
...
(or \code{cumtdc}) call that has two arguments.
=======
>>>>>>> other
...
the other 2 are listed as occurring on the leading edge of the follow-up
...
coef(coxph(Surv(tstart, tstop, death==2) ~ log(bili) + log(protime), pbc2)))
@ 
Again, we start the build with a baseline data set that has a subset of
the variables.  
...
set it is coded 682 from 1492 until last follow up.
One can change this default by adding 
\code{options=list(nadrop=FALSE)} to the second call above, in which case 
the alkaline phosphotase value over the
inteval (2453, 2503] will be missing.
Any \code{tdc} calls with a missing time are still ignored, independent
of the nadrop value, since we would not know where to insert them.
...
which the asities value was missing.
...
follow-up was continued for all subjects, both those that did and did not
...
the clerical staff responsible for this aspect of the study were reassigned.  
Automatic updates
from a state death index contiued to accumulate, however.  A Kaplan-Meier
...
The dropna option has been discussed above; it affects tdc() and
cumtdc() directives within a single tmerge call.
The delay option also affects only a single call, and causes any tdc
or cumtdc action in that call to be delayed by a fixed amount.
...
missing values into the result.
...
of death but rather is simply a marker for a death that is
already in progress.
A simple example would the the time dependent covariate ``have called in
...
understanding why is an critical step.  
...
baseline data.
When, as in this case, the baseline and timeline data sets have a diffrent
observation count, the baseline data set much contain a copy of the
id variable (here \code{repid}) and is constrained to have only a
single line for each id value.  
...
to be accurate, even if that diagnosis occured during a prior period when
the subject was not a resident. 
For events outside of the timeline, we have no way to know who the appropriate
comparison group is, and so ignore those events.  
(Formally, the risk set would the set of
...
conditions'' section of the medical record.)
county.)
