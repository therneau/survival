...
This vignette covers 3 somewhat different but interrelated concepts:
\begin{itemize}
  \item An introduction to time dependent covariates, along with some
    of the most common mistakes.
  \item Tricks and tools for creating time-dependent covariates, or rather the
...
The practical reason for this is based on the underlying way in which the Cox
...
event.  Each subject's risk score determines how likely they are to win,
e.g., how many ``tickets'' they have in the drawing.
...
(in the risk set or not) and the
...
For dose it would cumulative dose to date.
...
not finish the treatment plan, imputing the date of an adverse event at midway
between observation times, etc. 
Using future data will very often generate large bias in the results, which can
be positive, negative, or sometimes barely visible.
It is nearly impossible to predict a priori either the direction or size of
the bias.
Using such a covariate is similar to running a red light in your car: 
...
\subsection{The timemerge function}
A useful function for building data sets is \code{timemerge}, which is
part of the survival library.  For our first example we will show how the 
\code{cgd} data set is created that allows for analysis of a recurrent
event.
...
Below are the first 10 observations, see the help page for \texttt{cgdraw}
...
Subject 1 had 2 infections during the study, subject 2 had 7, and subject
3 had none.
\small
\begin{verbatim}
...
The data set above is included as \code{cgdraw} in the survival
...
\begin{itemize}
  \item Each row of the resulting data set represents a time interval
    ...
    over that interval.
  \item The event variable for each row is 1 if that time interval ends
...
The tmerge routine starts with a basic data set that contains each subject's
time interval and baseline covariates, and then sequentially adds time
dependent information by weaving it in.  We first make a data set
\code{tdata} that has all of the event times in it.
<<cgd1>>=
tdata <- with(cgdraw, data.frame(id2=rep(id, 7),
                                 etime = c(etime1, etime2, etime3, etime4,
                                           etime5, etime6, etime7)))
tdata <- na.omit(tdata)  #toss missing values

# Remove etime variables from the starting data set
indx <- match(paste("etime", 1:7, sep=''), names(cgdraw))
cgd1 <- tmerge(cgdraw[, -indx], tname =c(id="id", end="futime", start="time1"))

# Add the event times
cgd1 <- with(tdata, tmerge(cgd1, id=id2, status= event(etime), 
                           enum=cumcount(etime)))
