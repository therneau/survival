...
at that time. 
So computationally, we only need to know covariate values at that point in
time.
The model had a theoretical foundation in martigale theory,
...
actions depend only on the past. The decision of whether to play and the
size of a bet can depend in any way on
...
resulting random walk, including the \emph{gambler's ruin}, i.e., that 
no pattern of betting can erase the house advantage.
...
a better curve.  A Cox model will demonstrate a strong ``significant'' effect.
...
If response is instead coded as a time-dependent covariate which starts at
0 for all subjects and is recoded to 1 only when the response occurs, then the
problem disappears.
...
Below are the first 10 observations, see the help page for \texttt{cgd}
