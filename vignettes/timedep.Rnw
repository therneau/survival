...
in Bonadonna et al. \cite{Bonadonna81}.
...
their treatment. 
This approach leads to a severe bias
since early deaths do not finish all their cycles of chemotherapy and hence
by definition get a lower dose.  E
It is early death which predicts a low total dose, and not vice-versa.
...
vfit4 <-  coxph(Surv(time, status) ~ trt + prior + karno + tt(karno),
                data=veteran,
                tt = function(x, t, ...) x* ns(t, knots=c(100, 200),
                                                Boundary.knots = c(5,400)))
vfit4
# look at the spline term
smat <- cbind(1, ns(1:600,  knots=c(100, 200), Boundary.knots = c(5,400)))
...
Unfortunately, follow-on functions such as \code{termplot}, which would have
made the plot much simpler, or \code{cox.zph} to test for PH do not work
with this specialized fit.  We will come back to this example
