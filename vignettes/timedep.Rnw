...
\begin{itemize}
  \item The first call sets the time \emph{range} for each subject to
    ...
    as will be done in an example below.
...
is moderately common: if the first
...
the starting vector is either a vector of zeros, or NA for a
for a \code{tdc} or \code{cumtdc} call that has two arguments.
...
trailing edge of their interval, which is expected since last follow-up was
...
and in the pbcseq data set there are several observations with a 
missing alkiline phosphotase value.
A consequence of this behavior is that the pbc2 data set effectively
uses ``last value carried forward'' values for alkphos, replacing those
missing values. 
One can change the default by adding 
\code{options=list(nadrop=FALSE)} to the second call above. 
Subject 6 for instance has a total follow-up of 2503, and alk.phos values
of 682 and NA on days 1492 and 2453, respectively.
With \code{nadrop=FALSE} their alkiline phosphotase value over the
inteval (2453, 2503] will be missing and with the default setting it will
be 682 over that interval.
...
and \Sexpr{atemp[1, 'within']} within the subjects' follow-up
interval.  This causes a new break point to be added at each laboratory date,
and for subsequent additions these \Sexpr{atemp[1, 'within']} times lie on
a boundary of two intervals.  
Another \Sexpr{atemp[1, 'late']} non-missing alkiline phosphotase
...
unique break points in the follow-up.  
...
to extend the last follow-up date for these \Sexpr{atemp[2,'late']} subjects
...
obtained the extension, nor would their death have been recorded in the
original data set.
We would need to create and record an updated followup time, and deaths,
for of all subjects.  
