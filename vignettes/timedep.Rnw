...
Each addition is fitted into the original data in the same way that one
would slide a new card into an existing deck of cards. 
In the above example a new time dependent covariate ``newvar'' will be
created based on (time, value) pairs found in \code{data2}.

As a first illustration consider the \code{jasa} data set, which contains
data from the Stanford Heart Transplant study.
The data is laid out in the form found in the JASA methods paper which
introduced the data set to statistics, with
one line per subject that contains the baseline covariates along with
...
Our starting data set will simply contain the baseline variables of age and
prior bypass surgery.
The second data set contains the times to transplant and death.
There are a couple of wrinkles in setting up the second data set.
\begin{itemize}
  \item One subject died on the day of enrollment, which leads to an interval
    of length 0, something that the \code{coxph} function will reject.  As
    a workaround, have them die on day 0.5.
  \item One subject dies on the day of transplant, leading again to a zero
    length interval. Move their transplant date ahead by 1/2 a day.
\end{itemize}
<<jasa>>=
temp <- with(jasa, data.frame(id=1:nrow(jasa),
                              txtime = ifelse(tx.date < fu.date,
                                              tx.date - accept.dt,
                                              (tx.date - accept.dt) - .5),
                              futime = ifelse(fu.date > accept.dt,
                                              fu.date - accept.dt, .5),
                              fustat = fustat))
newdat <- tmerge(jasa[,c('age', 'surgery')], temp, id,
                 death= event(futime, fustat), transplant= tdc(txtime))
newdat[1:6,]
coxph(Surv(tstart, tstop, death) ~ transplant, newdat)                      
@ 

The tmerge call above adds two new variables to the baseline data: a
time dependent event \code{death} and a time dependent covariate 
\code{transplant}, along with the time variables \code{tstart} and \code{tstop}
which are always added.
For subject 4 the translant event happens between enrollment and death,
and causes the baseline row for the subject to be split into two.
Subject 3 was transplanted on the day of enrollment, so addition of the
transplant covariate did not require generation of a new observation.
...
These lines show a canonical way to use tmerge: each call adds one
more bit of information to the data set.
\begin{itemize}
  \item The first call sets the time \emph{range} for each subject to
    ...
    adding a terminal event as was done in the jasa example above.
    This first data set has \Sexpr{nrow(cgd0)} rows.
  \item Each additional call then adds either an endpoint or a covariate,
    ...
    Events happen at the ends of intervals which time-dependent covariates 
    happen at the start of an interval.  
 \item Additions with a missing time value are skipped.
  \item The result of \code{tmerge} is a data frame with a few extra
    ...
    intervals, one at 14 is at the leading edge of an interva, one at
...
The \code{tmerge} function processes arguemts sequentially, and the 
above example can be rewritten as below.  There is no computational
advantage of one versus the other.
<<cgd1b>>= 
