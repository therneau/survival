...
\begin{enumerate}
  ...
  \item The treatment coefficients in table 6.1 of the 
    ...
    (Until I figured this out I would get occassional ``why is coxph giving
...
at transplant.  
(At the time of the PBC study liver transplantion was still in its infancy and
...
inteval (2453, 2503] will become missing.
...
understanding why this occured is an critical step.  
...
\begin{itemize}
  ...
  \item timeline: one or more rows for each subject containing 
    ...
    uncommon.
  \item outcome: one row for the first occurence of each outcome of interest.  
    The outcomes were 20 comorbid conditions as defined by NIH.
\end{itemize}
