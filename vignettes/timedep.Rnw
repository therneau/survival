...
think a new variable is being created de novo but the name already is
...
attr(pbc2, "tcount")
coxph(Surv(time, status==2) ~ log(bili) + log(protime), pbc)$coef
coxph(Surv(tstart, tstop, status==2) ~ log(bili) + log(protime), pbc2)$coef
@ 
The coeffcients of bilirubin and probrombin time are somewhat larger in
...
interval.  This causes a new breakpoint to be added at each laboratory date,
...
would a  better analysis have first used this data to extend the last follow-up
date for these 138 subjects?
Not necessarily.
...
``if they had had an event over that interval, we would have seen it.''
Say someone ended their initial follow-up time at 3000 days and then had a
lab test at 3300 days (subjects returned about once a year).
If we only extend the time of those who had a test, then saying that this
subject was at risk during the interval (3000, 3300) is false: if they
had died in that interval, they would not have gotton the extension and this
analysis would not have noticed.  
...
will forge ahead.
(Most who fit the log(t) form have not bothered to even look at a plot.)
...
@ 
This error has been made often enough the the \code{coxph} routine has been
...
covariate,
rather it creates a time-static variable based on the single value
of ``time'' for each subject in the data set; no differently that if we had
...
\code{time} predict long survival well,  because long survival leads to 
large values for \code{time}; the resulting coefficient is very large.
...
based on \code{time} will be fixed,  and a programming statement within
phreg that 
...
or rows in the intermediate data set is
...
At this moment the $x=TRUE$ argment is needed to get proper residuals and
