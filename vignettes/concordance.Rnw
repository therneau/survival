...
\begin{align}
  ...
  (D+1)/2  &= \frac{c+ t_x/2}{c + d + t_x} \label{dC}
\end{align}
Notice that all 4 range from -1 to 1, similar to the correlation coefficient 
$R$.
...
in computation of $G$.  Consider the aml data set as an
example, the first few lines of the relevant survival curve are shown below.

<<amlexample>>=
afit <- survfit(Surv(time, status) ~1, aml)
summary(afit, times=afit$time[1:6], censor=TRUE)
@ 

The first censor is at time 13, when there is both a death and a censor.
For the event at time 13, however, no one has yet been censored, no
``censoring correct'' should be done at this point.
...
As a consequence the Schemper and Uno weigths are also identical.
...
legend(1, 2200, c("n(t)", "nS(t-)", "nS(t-)/G(t-)", "n(t)/G^2(t-)"), lwd=2,
...
The title of the second makes the purpose clear:``What do we mean by validating
a prognostic model?''
