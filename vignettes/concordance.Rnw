...
\end{align}
Notice that all 4 range from -1 to 1, similar to the correlation coefficient $R$,
the former when $x= -y$ and the latter when $x=y$ (without ties).  
A transformation $(z+1)/2$ changes the scale to 0-1, which is the natural
scale for a probability such as $C$.
...
Why Somers' $D$ rather than one of the other three? \begin{itemize}
...
$X \beta$ and the response $y$, by definition, but this is not necessarily 
true for ${\rm rank}(X \beta)$ and $y$.
...
Order the data by increasing $y$ values, then
\begin{align}
  c-d &= \frac{1}{2} \sum_{i=1}^n \sum_{j=1}^n {\rm sign}(y_i-y_j) 
         {\rm sign}(x_i-x_j) \label{cd1} \\
      &= \sum_{i=n}^1 \left[ \sum_{j>i,\, y_j != y_i} {\rm sign}(x_i - x_j) \right]
          \label{cd2} \\
\end{align}
...
\begin{enumerate}
  \item Create a balanced binary tree for all $n$ $x_i$ values.  This can be
    ...
    unique $x$ value.  Each node contains the value, and counts for the
  ...
  \item Go through the data from largest $y$ to smallest $y$.
    \begin{enumerate}
      \item For this new $x_i$, count the number of $x_j$ values in the 
        tree that are smaller or larger than this element, and increment
        the overall counts of concordant, discordant, and tied.  This can be done
        in $\log_2(n)$ steps (proceed down from the top).
      \item Add this observation to the tree.
        Each addition will update the count for the node, then
...
(The fact that strata names are not retained as labels is a deficiency.)
...
as
\begin{align}
 c-d &= \sum_{i=1}^n \left[ \sum_{j>i,\, t_j != t_i} {\rm sign}(x_i- x_j) \right]
          \nonumber \\
     &=  \sum_{i=1}^n \left[ \sum_{t_j \ge t_i} {\rm sign}(x_i - x_j) \right]
            \label{cd4} \\
     &=   2 \sum_i \delta_i n(t_i) \left[ r_i(t_i) - \overline r \right] \label{cscore}
\end{align}

In equation \eqref{cd4} the inner term is a sum over all subjects in the
risk set at time $t_i$, a familiar concept in survival models.
Recognizing that if $y_i$ is censored, all other observations with
$y_j \ge y_i$ are incomparable leads to equation \eqref{cscore},
...
There is a deep literature with respect to the "best" weight for survival tests,
...
\item Schemper et al
    ...
    model.
    The same argument may apply more strongly for the concordance, since our 
...
\item  Uno et al \cite{Uno11} recommend the use of $n/G^2$ as a weight based
    ...
    when there is censoring, and they therfore label it as biased.
    Note that per the Peto and Peto argument, $S/G$ and $n/G^2$ should be very
    similar.
\end{itemize}
...
concordance values; the $S/G$ and $n/G^2$ weights differ only in the 6th
decimal place. 
...
temp <- c(n= coef(ncord1), S=coef(ncord2), "S/G"= coef(ncord3), 
          "n/G2"= coef(ncord4))
round(temp,6)
@

The \code{concordance} function provides a \code{ranks=TRUE} argument which
can be used to further explore this.  If set, the output will include a
...
The relative ranks are exactly comparable to Schoenfeld residuals,
and their weighted sum will equal $c -d$. 
We can plot them over time and apply a smooth.
The graph below, for the veteran dataset, shows a precipitous drop to 0.
...
The first question that should be considered, in any model assessment, is to
...
Altman and Royston, ``What do we mean by validating a prognostic model?''
\cite{Altman00}
Korn and Simon point out the importance of restricting the range of comparison.
Though a risk model can be used for long-range prediction, in actual patient
practice this will very often not be the case; the model should be
...
restrict attention to a 4 year horizon.
(A 10 year Framingham heart risk might still be useful in \emph{convincing} a
subject to stop smoking.)

We sometimes receive pushback on this, arguing that one should use all the
data.  We disagree, and think that the target of the validation is critical.
...
Another reason for using an upper limit is that $1/G$ weights can become
...
lower limit of 10 is chosen.  The approach used for non-censored data 
is to treat 5 and 9 as tied values, but this does not extend to the censored
...
$$
They then order the $\eta$ values from a fitted model, and take the average
over all $n(n-1)/2$ ordered pairs.
The authors argue that this is an estimate that is indepenent of censoring, and
therefore preferable to Harrell's C.  This estimate is provided by the 
\code{royston} function.

The biggest problem with this estimate is that it gives an estimate of 
concordance under an assumption that \emph{the model is exactly correct}. 
Our goal, rather, is to assess how \emph{well} the model performs, 
for our needs, knowing that it will be imperfect.  
The G and H formula answers a question that we never asked, over a time range 
...
\begin{enumerate}
  \item An important issue
    ...
    The default action of the routine is to use the safe choice of $n(t)$.
  \item Consider setting a time ($y$) restriction using the 
    option, based on careful thought about the proper
    range of interest.  This often has a larger practical effect then the
...
A time weight of $S/G$ may be more stable than $n/G^2$ in the tail, when $G$
becomes small, but we have yet to discover a data set where results were
meaningfully different

We most definitely disgree with Uno's unstated assumption 
...
is therefore biased.  The target will never be attainable, and we would argue
...
the in rank residuals shown above will eventually tend to 0.
...
However, when the concordance is larger than 1/2 this estimator systematically
