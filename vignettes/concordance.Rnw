...
The concordance function can be used with various model types 
including logistic and linear regression.

<<examples1>>=
# logistic regression using Fisher's iris data
fit1 <- glm(Species=="versicolor" ~ ., binomial, data=iris) 
...
Primarily the focus of this vignette will be on survival models.
...
matrix for all three concordance values can be obtained using
\code{vcov(ctest)};
...
The concordance can be visualized using its contribution at each
event time as shown below.
The $y$ value on the graph is a rank of each subject's risk score
as compared to the risk scores of all those subjects with longer survival,
$r$ = fraction with lower scores - fraction with higher scores.
In a highly predictive Cox model the subject with the highest risks will
die soonest.
A log transformation on the x-axis was used to 
visually spread out the points. 
Somers' $d$ is the weighted average of these ranks and the 
concordance is $c= (d+1)/2$. 
After about 50 days, the baseline data has little predictive ability and 
the average ranks are close to zero.  
...
legend(300, 100, legend=c('n(t)','S(t)/G(t)'), lty=1:2, 
...
where the rank is defined such that $0 \le r \le 1$.
It turns out that 
...
An alternative that remains unbiased is the infinitesimal jackknife variance
\begin{align*}
...
this it is straughtforward to derive the influence of each subject on 
the concordance, or on any other
of the other possible association measures \eqref{tau.a} -- \eqref{somer}
such as $\tau_a$.
The IJ variance is printed by default but the PH variant is also returned;
the earlier \code{survConcordance} function only computed the PH variant.
...
This suggests that the most statisticially \emph{efficient} estimate of the
...
model, which is one side effect of this contrast in efficiency.
...
$U_a - U_b$.  
(If subject $i$ raises $c_a$ by .03 and raised $c_b$ by .01, then he/she
raises the difference between them by .02.)
It is not even necessary that the models be nested.
However, it is crucial that they be computed on the exact same set of
observations.
Here is a comparisone of concordance values for two previous models.
...
The lung dataset has several variables with missing data and shows
...
fit6b <- coxph(Surv(time, status) ~ age + ph.ecog, lung2)
...
risk scores.  Let $0 < r_i(t) < 1$ be the time dependent rank $2 (r_i(t) -1/2)$;
...
it will yield the log-rank statistic. 
...
to the concordance as well.  We point out four in particular.
...
\item If proportional hazards does not hold, Schemper 
    \cite{Schemper09} argues for
    a weight of $S(t)/G(t)$ in the Cox model.  The basic argument is that
    the quantity estimated by our statistic should be stable over time,
    e.g., if the data were allowed to mature for another year, and the 
    concordance recalculated, the estimate should not have a systematic
    change.
...
\item  A similar argument to Schemper's is found in Umo \cite{Umo09},
    who recommend the use
   of $n/G^2$ as a weight.  The Peto and Peto argument might suggest $S/G$ as
   an equivalent but more stable choice.
\item Korn and Simon \cite{Korn90} for point out the importance of
...
and then compute a 6 year model based on survival from that point, in order
to predict my outcome one year from today. 
\end{itemize}

So, which weight should we use?
First note that for non-survival data $n$, $S$ and $S/G$ are identical.
For survival data,
one issue that has not been sorted out is how to extend the above
...
Since this usage is quite frequent, and also because it is not possible
for the \code{coxph} routine to reliably tell the difference between
...
Beyond this the four primary choices are to
\begin{enumerate}
  \item Set a time ($y$) restriction using the \code{ymin} or \code{ymax}
  ...
  \item  Safety.  If using the usual Gehan-Wilcoxon weights of $n(t)$,
    the Peto-Wilcoxon variant $S(t)$ would appear advantageous.
...
the model in a more non-parametric way, that a log-rank type of focus on
...
Umo's assertion that the ordinary concordance is
...
the points in the $r(t)$ plot above will eventually go to 0.
...
might not get there.''
Of course, 
if proportional hazards approximately holds over the time window of
interest, as in the
lung data above, then any weighted average over the range will be close to
identical.

The \code{timewt} option allows you to modify weights for the concordance.  
The options are: \code{n, S, S/G, n/G, n/G2, I}.
...
Concordance values are all very similar.

<<tmwt, fig=TRUE>>=
colonfit <- coxph(Surv(time, status) ~ rx + nodes + extent, colon,
...
legend(1, 3000, c("n(t)", "S(t)", "S(t)/G(t)", "n(t)/G^2(t)"), lwd=2,
       col=1:4, lty=1:4, bty='n')
@

A second example where we would expect weights to have a larger
influence consider the \code{mgus2}
data set, a rare study in that it has a median follow-up time of over 15 years.
...
as predictive.
The R data set has time in months, convert this to years for plotting
purposes.
<<mgus1, fig=TRUE>>=
...
while there is 35 years of maximum follow-up.
Now calculate weighted concordance.
...
par(mfrow=c(1,2))
rfit <- lm(rank ~ ns(time,3), c6a$ranks)
...
Surprisingly, the four weightings are still yield almost identical
concordance values.  
...
them before 9; the larger weights simply play a much smaller role in the
sum.
...
200, say, was last known alive on that day because they were contacted then. 
...
larger weight.  
(Both the Umo and Schemper papers ignore this detail.)
...
The upshot of this is that the weight changes to 40 rather than 37,
while the rank becomes smaller by a factor of 37/40.
The Cox model score statistic is a weighted sum of the ranks, and is still
equal to the numerator of the concordance statistic.
