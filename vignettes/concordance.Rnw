...
The \code{concordance} function available in the survival package can be used to estimate concordance with various types of models  
...
\item  Umo et al \cite{Umo09} recommend the use of $n/G^2$ as a weight based
...
For non-survival data $n$, $S$ and $S/G$ weights are identical.
