...
summary.
As an aside,
we can use a Taylor series approximation of the linear predictor calculation
to approximate the response result.
The result is an underestimate, as expected, since $E(e^X) > e*{E(x)}$.
<<glm2>>=
# post-process the link
round(exp(y1$estimate$mppv),3)
round(exp(y1$estimate$mppv)* y1$estimate$std, 3)
...
the average.  Continuous and categorical age adjustment is similar in
...
it is the correct answer to a rather odd question: this is a
population that no one will ever encounter.

We can also look at the age effect after adjusting for sex.  For the
continuous model the age values for the MPPV need to be added using
...
This is actually not a surprise as both the US and Minnesota death rate 
curves for males and females are nearly parallel after age 50. 
...
population, and likewise for a mean log hazard ratio.
A MPPV based on these estimates is thus hard to interpret.
...
The longest death time is for the FLC data set is at
\Sexpr{round(with(flchain, max(futime[death==1])/365.25),1)} years; as
...
(But it is not a 1 number summary with a simple p value, and so spurned by
journals ;-).
<<surv3, fig=TRUE>>=
...
For other than the linear case the variance is obtained by a direct
computation.
Assume that $\hat\beta ~ sim N(\beta, V)$, and draw 
\code{nsim} independent samples $b$ from $N(\hat\beta, V)$.
The MPPV values are computed for each, and an empirical
