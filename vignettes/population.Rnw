...
yates(fit2, "Mask")
@ 
Because of the near perfect balance some of the means have not moved at
all, however.
...
ones from the coefficients alone,
and the standard R \code{predict} prints a warning message that the results
are unreliable.
A formal definition of estimability is that given prediction can be written 
as a 
...
In figure 1 for instance it corresponds to a population with the same number
...
\begin{enumerate}
  ...
  \item Create the $p$ by $p$ dependency matrix $D$ from the $n$ by $p$
    ...
    \begin{itemize}
      ...
      \item In R \code{D = coef(lm(X ~ X -1))}
    \end{itemize}
...
but much more challenging to prove this equivalence as a general case 
(shorthand for ``this author can't seem to do it'').
