...
y2 <- yates(fit1, "Opening", population = "data")  #default population

y1
y2
@ 
The estimates under a factorial population (\code{y1}) is the Yates' weighted
...
A test for the Mask effect gives the same result for the two populations.
This is because the other variables form a balanced set, so the `empirical'
(equivalent to 'data') and 'Yates' (equivalent to 'factorial') populations
are identical.  

<<solder3>>=
y3 <- yates(fit1, "Mask", population="empirical")
y4 <- yates(fit1, "Mask", population="Yates")
all.equal(y3, y4)
@ 
...
fit2 <-  lm(skips ~ Opening + Solder * Mask + PadType + Panel,
           data=solder)
y5 <- yates(fit2, cm("Mask", test="pairwise"))
