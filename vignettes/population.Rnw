...
\begin{itemize}
  \item Define a mixture population: factorial for categorical variables;
    ...
    all others.
  \item Assume a data set which had 2 categorical variables x1 and 
    x2 with 3 and 4 levels, respectively
    4 other continuous variables, and 47 observations.
  ...
  \item The three least squares means for variable x1 can be obtained by
    ...
    all 564 rows, then compute the mean predicted value, and likewise for the
  ...
  \item There many ways to shorten and/or speed up this calculation of course,
    but that is not material to the discussion.
\end{itemize}
...
The exact algorithm is described indirectly in SASxxx
\begin{enumerate}
  \item Create design matrix $X$ of the form used by glm from left to right:
    ...
    \begin{itemize}
      ...
      \item R's \code{model.matrix} function returns far fewer columns, since
        ...
        Having all the columns is critical to steps 2 and 3, however.
  ...
  \item Partially orthagonalize the transpose of $D$ from right to left
    \begin{itemize}
      ...
      \item An x1*x2*x3 term where x3 is continuous would not 
...
and two contrast matrices B and C correspoding to the effects will
satisfy $C(X'X)^{-1}D' =0$.
For an unbalanced design make the contrasts orthagonal to the variance
matrix of a balanced one.
Starting with any
...
design and the connection to a population average is lost.
However, it leads to a simpler R algorithm that agrees with SAS type III for
a set of test cases. 
If the data contains continuous variables then $Z'Z$ uses only the categorical
variable columns of $X$.
