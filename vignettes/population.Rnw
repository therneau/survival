...
what compuational algorithm should be used,
...
Searle et. al. \cite{Searle} use the term population marginal mean (PMM),
...
50 to 75th percentile, 75 to 90th percentil and above the 90th percentile.
...
This last leads to one of the most well known identities in ANOVA: for a 
...
\end{quote}
This is similar to the original rationale as Yates \cite{Yate34}.
Type I and II never coincide for continuous variables and so none of this
argument applies to them.

The $L'$ matrix for the sequential sums of squares is orthagonal with respect
...
unrelated terms, and is orthagonal to $(Z'Z)^-$, where $Z$ is the design matrix
for an orthagonal subset. 
If there are no missing cells in the design, i.e., no combinations of factors
which is present in the model statement but not in the data, then 
\code{Z = unique(X)} is a direct way to create the subset, with $L$ the
...
orthagonal to all $L$s of effects that contain the effect in question.''
The outline of an algorithm to create such an $L$ for design that
may or may not have missing cells is found in the examples of a second
SAS technical report \cite{SAS2013}.  This is used by the \code{yates} function.
\begin{enumerate}
  \item Create a design matrix $X$ in standard form, i.e., intercept, then
    ...
    with categorical variables coded in the same was as the SAS GLM procedure.
    \begin{itemize}
  ...
  \item Create the $p$ by $p$ dependency matrix $D=(X'X)^-(X'X)$ from the 
     ...
     \begin{itemize}
        \item $D$ will be upper triangular. If row $i$ of $X$ can be written
         ...
         will contain those coefficients, otherwise it will be a copy of
         the identity.
  ...
  \item Intialize $L' = D$, and then partially orthogonalize $L$.
     \begin{itemize}
       \item If terms $i$ and $j$, $i<j$, are such that term $j$ includes
         term $i$, then make $L_i$ orthagonal to $L_j$.  That is, replace
         $L_i$ with the residuals from a regression of $L_i$ on $L_j$.
         (Orthagonalize rows of $L'$ = columns of $L$.)
...
zeros, whose terms were orthagonal with respect to $(Z'Z)^-$, 
...
orthagonal with respect to the identity matrix $I$. 
Why does this work? 
