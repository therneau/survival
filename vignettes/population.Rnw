...
Now insert a short mathematical interlude. 
...
Further, let $(k)$ refer to those rows/colums of $X'X$ that correspond to
a particular effect in the model.  
In a standard R linear model \code{y \textasciitilde a*b} where \code{a}
and \code{b} were factors with 2 and 3 levels, respectively,
$(k)=\{1\}$ for the intercept, and $\{2\}$, $\{3,4\}$ and $\{7,8\}$ for the
\code{a}, \code{b} and interaction terms, respectively.
Then \begin{enumerate}
  \item The sums of squares for the components the test $L'\beta=0$, i.e.,
    ...
    type I sums of squares for the model, i.e., equivalent to the 
...
This drives the SAS algorithm: find an upper triangular contrast matrix $C$ 
with a diagonal of 1 such that $C(Z'Z)^-C'$ has off diagonal blocks of zero.
There are many such matrices all of which give the same SS if $Z'Z$ is full
rank.
The kicker is that SAS GLM will produce a ``type III'' SS for terms that
are \emph{not} estimable, i.e., the case of missing cells, where $Z'Z$ 
is not full rank.  To reproduce their result one has to reproduce exactly
their $C$ matrix.
  ...
  \item Create a design matrix $X$ of the form used by the GLM procedure,
      ...
      \item Create the $p$ by $p$ dependency matrix $C=(X'X)^-(X'X)$ from the 
        ...
        \item If column $k$ of $X$ can be written as a linear combination of
          prior columns, then $C_{1k}$ to $C_{k-1,k}$ contains that combination,
          and $C_{j,k}=0$ for $j \ge k$.
        \item Otherwise set $C_{kk}=1$ and $C_{jk}=0$ for $j ne k$.
      ...
      \item Partially orthogonalize the rows of $C$.
        \begin{itemize}
          \item For any row of $C_i$ that corresponds to a categorical term,
            ...
            right in the matrix, make this row orthogonal to any rows of 
            those containing terms.
          \item The term x1 is contained in x1*x2 for instance.
          \item That is, premultiply $C_{ij}$ by $I- C_{jj}(C_{jj}C_{jj}')^-C_{jj}',
            where $i$ is the set of rows in question and $j$ rows corresponding
            to the including term.
         \end{itemize}
\end{enumerate}
