...
The default survival prediction takes longer recommended for a CRAN
vignette: there are \Sexpr{nrow(flchain)} subjects in the study
4 flc groups, which leads to just over 30 thousand predicted survival curves
when using the default \code{population='data'}.  To compute the variance
this is repeated \code{nsim} times.  We cut the latter back to 50
for feasability in this particular case.
...
<<surv3>>=
plot(flkm, xscale=365.25, fun="event", col=1:4, lty=2, lwd=2,
...
linear model (all predictors categorical, 
all combinations appear the same number
of times) the tests for all the effects are orthagonal.  
...
$C$ and $D$ be contrast matrices for any two of the 3 terms above, i.e., 
that matrix such that a test for $C\beta=0$ is equivalent the the SS for x1.
Orthagonality implies that the covariance between the two effects
is zero, i.e. $CVD' =0$.

Some statisticians see this orthagonality and an interesting aside and
others (not this author) view it as a central aspect of the estimation,
...
\begin{itemize}
  ...
  \item Assume an $X$ matrix in the standard column order of intercept, then
    main effects, then 2 way interactions, etc.  One way to create orthagonal
    contrasts is
    \begin{enumerate}
...
\begin{enumerate}
  ...
  \item Create the $p$ by $p$ dependency matrix $D$ from the $n$ by $p$
    ...
    \end{itemize}
  \item Partially orthagonalize the transpose of $D$:
    \begin{itemize}
      \item For any column of $D'$ that corresponds to a categorical term,
        and is contained in some categorical term further to the right in
        the matrix, make this column orthagonal to those.
...
MPPV approach is challenging but doable, as is a proof that algorithms 1 and 2
...
the keep-unique-rows operation does not produce a population matrix, and
...
\begin{enumerate}
  ...
  \item For any given term $k$ do a second fit the removes those columns
    of $X$ corresponding to term $k$, but retaining all others, including those
    labeled as a higher order ``interaction'' involving said term.
\end{enumerate}
...
\begin{align*}
  ...
  \sum_{ij} \gamma_{ij} &=0
\end{align*}
then the `reference cell' for Solder will be the sum of Opening effects,
and the NSTT will correspond to an MPV using the factorial population.
...
But like many good ideas in statistics it can not be applied properly without
some thought. 
Sometimes the answer is not completely clear;
...
describe in Yates \cite{Yates34} or Goodnight \cite{Goodnight78} that 
relate to designed experiments, but in observational or clinical data
it will too often be the answer to a question that nobody asked.
Say that the pathology variable in a study had proportions of 
10, 20, 30 and 40\% for stages A--D, and that these proportions also hold
in the disease population at large.  Of what interest is the
result for a popluation that was 25\% class A, a population that no
one will ever see?
\end{document}
