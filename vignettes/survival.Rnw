...
<<states, echo=FALSE, figure=TRUE>>=
par(mfrow=c(1,2))
s1 <- c("Alive", "Dead")
smat1 <- matrix(c(0,0,1,0), 2,2, dimnames=list(s1, s1))
statefig(c(1,1), smat1)

s2 <- c("Entry", "Malignancy", "Death")
smat2 <- matrix(c(0,0,0,1,0,0,1,0,0), 3, 3, dimnames=list(s2,s2))
statefig(c(1,2), smat2)
par(mfrow=c(1,1))
@   
  
From a mathematical point of view survival data is often represented as
...
The left panel is simple survival with two states of alive and dead,
and the right shows a simple competing risks setup.
In this approach interest normally centers on the rates of transitions
...
When there more than one arrow the rate approach is often particularly useful.
...
<<cfit1>>=
mdata1 <- subset(mdata, enum==1)
cfit <- coxph(Surv(tstop, event) ~ I(age/10) + sex + mspike, 
              id = id, mdata1)
