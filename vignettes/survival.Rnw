...
The \code{tmerge} call above used the cumevent verb for events, and the first
line below checks that no subject had diabetes, for instance, coded more than
...
in which case the cumulative count of all events (ecount) would not be
...
ndata$cstate <- with(ndata, factor(diabetes + htn + lipid, 0:3, 
                                   c("0mc", "1mc", "2mc", "3mc")))
temp <- with(ndata, ifelse(death, 4, ecount))
...
are a \Sexpr{temp3$events[5,5]} subjects who transition through all 5 
...
current state endures across intervals: it is base in \code{tdc} variables
while the outcome is based on \code{event} outcome.
...
setup like 
this, to ensure that the data set which has been built is what was desired.
...
The standard printout is arranged by transition.
...
The names given to the coefficients in a multi-state model are a compromise,
...
transition that this coefficient corresponds to.
(First in the sense of standard R matrices, i.e., reading the elements in 
column order.)

\subsection{Timeline data}
The \code{survfit} and \code{coxph} routines also accept data in what we refer
to as a ``timeline'' form.  
In a timeline data contains a case identifier and a timeline variable, with a
...
analysis.  The \code{Surv2} operation on the left-hand side tells the
...
Here is a reprise of the NAFLD data set.
<<timeline1>>=
tdata <- data.frame(nafld1[,1:7],
                    days= 0,  death=0, iage=nafld1$age)
tdata <- merge(tdata, with(nafld1, data.frame(id=id, days=futime, death=status)),
               all=TRUE)
...
tdata <- merge(tdata, with(subset(nafld3, event=="diabetes"),
                           data.frame(id=id, days=pmax(0,days), diabetes=1)),
                   all=TRUE, by=c("id", "days"))
tdata <- merge(tdata, with(subset(nafld3, event=="htn"),
                           data.frame(id=id, days=pmax(0,days), htn=1)),
                     all=TRUE, by=c("id", "days"))
tdata <- merge(tdata, with(subset(nafld3, event=="dyslipidemia"),
                           data.frame(id=id, days= pmax(0, days), dyslipid=1)),
                       all=TRUE, by=c("id", "days"))
tdata <- merge(tdata, with(subset(nafld3, event=="nafld"),
                           data.frame(id=id, days= pmax(0,days), nafld=1)),
                     all=TRUE, by=c("id", "days"))
#
# For cumulative events within subject we use a helper function
cumevent <- function(id, time, status, istate, firstonly=FALSE) {
...
temp1 <- rowSums(tdata[,c('diabetes', 'htn', 'dyslipid')], na.rm=TRUE)
temp2 <- with(tdata, cumevent(id, days, pmax(temp1, 4*death, na.rm=TRUE)))
state <- factor(pmin(temp2$status, 4), -1:4,
                 c("censor", paste0(0:3, "mc"), "death"))
tdata <- merge(tdata, data.frame(id=temp2$id, days=temp2$time, state=state),
               all=TRUE)
check2 <- survcheck(Surv2(days, state) ~ 1, id=id, tdata)
check2$transitions
nfit2 <- coxph(list(Surv2(days, state) ~ nafld + male,
                    "0mc":state("1mc", "2mc", "3mc") ~ nafld+ male / common,
                     2:3 + 2:4   ~ nafld + male / common,
                     0:"death" ~ male / common),  
               data=tdata, id=id)
