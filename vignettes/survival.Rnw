...
alcohol consumption or certain medications, and absence of other liver
disease \cite{Puri12}.
...
As a final protection, only a 90\% random sample of the data was retained.
As a consequence analyses results will not exactly match the
original paper.

Start by building an analysis data set using the first and third of these.
...
are a few subjects who transition through all 5 possible states
(4 transitions).
...
current state endures across intervals.
If a portion of a subject's time
were divided into multiple sub-intervals, time dependent covariates and current state
...
A call to the \code{survcheck} routine is almost mandatory for a complex setup like 
...
The right hand side acts like \code{update.formula} in that it can also
remove of variables found in the default formula.
...
By default the summary command has rows for each coefficient, not arranged by state.
If a coefficient maps to multiple transitions it is labeled with the first.
The printout can be modified by adding an \code{expand=TRUE} option
