...
The last example chooses to plot only a subset of the curves, something that is
...
independently, while probabilities cannot.
...
But how important are the ties, actually?
...
\begin{itemize}
  ...
  \item The differences are very small. Which monthly coarsening,
...
If you see a 1 Euro coin on the sidewalk would you pick it up -- even though
it not make any real change in your income?
One downside is that no other package did this, leading to a very common
complaint/question that R ``gives different results''.
A second is that it leads to more downstream work as discussed in 
following sections.
...
This estimate was explored by Fleming and Harrington \cite{Flemingxxx}.
...
same.
An alternate estimate of the survival is $S(t) = \exp(-\Lamda(t))$.
...
\begin{equation}
  \Lhat(t) = \int_0^t \frac{\sum_i dN_i(t)}{\sum_i Y_i(t) r_i \lab{haz:breslow}
\end{equation}
...
(Nelson-Aalen), the hazard ratios for any given transtions (Cox model)
