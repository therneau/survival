...
from an unnamed initial state to a repair, (0) $\rightarrow 1, or from one
...
2. At time 1 form the 4 by 4 transtion matrix $T_1$
\begin{align*}
...
1 transtions to state a.  
There is no one in the other 3 states, for rows use a default
``stay in the same state'' row.
(Since no one ever leaves states a--c, the bottom three rows of $T$ will 
...
the name ``(0)''.  The parenthesis are an echo of ``(Intercept)'' in a
linear model and show a label that was not provided by the data.
The \code{istate} argument can instead be used to designate the subject's state 
...
\begin{enumerate}
  ...
  \item A set of state labels that distinguished death after SCT from death 
    before SCT.
\end{enumerate}
