...
A function \code{survfit23} is used by the print and plot methods to ``align''
all of the elements of the curves to a common starting x and y value, 
usually 0 for x=time and 1 for y= survival. This also improves consistency.
