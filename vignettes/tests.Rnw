...
legend(60, .02, c("Empirical reference", "LS reference"),
       lty=1, col=1:2, bty='n')
@ 
If we try to use population reference adjustment directly on
ages a problem immediately arises: in the US reference 
...
equal to the reference population.  We will treat age as a categorical
predictor in the next section.
...
\emph{not safe type three}(NSTT) was chosen, \emph{not SAS} and \emph{nonsense}
are sensible alternate mnemonics.

One of these three (NSTT) is completely unreliable, but is included because
it corresponds to a common R-help answer to the question of ``how to
compute type 3'' through use of the \code{drop1} function. 
In order to get the correct answer using this method the original model
fit must have been done using a``sums'' constraint on the coefficients, which is
popular in theory textbooks but almost never used in practice.
...
Historical knowledge of the underlying linear model algorithms is a useful
...
\begin{enumerate}
  \item Many take the sequential ANOVA table as primary, i.e., a set of
    ...
    Since other population weightings than the LS one do not fit nicely into
...
A modern summary with focus on the clinical trials arena is chapter xx of
the textbook by  Senn \cite{Sennxx}.
...
fit <- lm(flc ~ sex * age2, data=flchain)
drop1(fit, .~.)
@ 
...
drop1(fit, .~.)
@ 
Each diffent choice of contrasts, other than contr.sum, gives a different
answer, all of which are nonsensical.
...
The key step is \code{xtemp <- unique(model.matrix(fit))}.  We then average
all rows of \code{xtemp} where sex = female, and all those where sex = male;
the difference of these is the Yates contrast vector for sex.
This approach is closely related to the SAS type IV calculation.
(It may actually be completely identical, but I have not proven this.)
...
For our flc example this will have colums for intercept (1), sex (2), age group
