...
There are three central ideas as it turns out: populations, compuatation,
...
\emph{which} population''.  The SAS type 3 compuations corresponds to a very 
...
average.  There are two primary ways to set up this compuation.  In a linear
...
\begin{itemize}
  ...
  \item The agorithm used to compute $C$ by the SAS glm procedure is 
...
Four possible choices for a target popluation of ages are given below.
\begin{enumerate}
  \item Empricial: the age distribution of the sample at hand, also called
    the data distribution. In our sample this would be the age distribution
    of all \Sexpr{nrow(fdata)} subjects, ignoring sex.
