...
\begin{enumerate}
  ...
  \item Population adjusted.  An average difference between the curves,
    ...
    \begin{enumerate}
      ...
      \item Empirical population.  The overall population structure of the data.
...
aged 95 years, and our sample has 0 95 year old males; no reweighting
can fix this.
...
An increasingly popular approach for the empirical reference population
is to use a smoothed age effect, via the
use of inverse probability weights based on logistic regression,
...
\subsection{Computing the Yates contrast}
...
Three algorithms are provided in the \code{yates} function, which has been
added to the survival package.

The averaging method.  The key idea of this approach is to reprise
equation \eqref{ycont}.
The key step is \code{xref <- unique(model.matrix(fit))}.  We then average
all rows of \code{xref} where sex = female, and all those where sex = male;
...
The SAS type III method uses a dependency matrix $D$.  Start by writing the
$X$ matrix for the problem using all of the parameters in equation \eqref{std}.
...
identically zero and can be discarded.  
...
right of it. 
(Inspection shows that colums of any effect are already orthagonal to
everything except interactions that include said effect, so the regression
need not include all columns.)
...
Note that the computation requires estimates of the cell means, whether
...
\begin{align}
  ...
  \theta_{ij} &=  \mu + \alpha_i + \beta_j + \gamma_{ij} \label{a4} \\
\end{equation}
...
which is often found in textbooks (but nowhere else
...
The car package (Companion to Applied Regression text) package uses
this procedure.  
Here is another commonly cited R code for ``type III'' computation based 
on the same process.
...
(I suspect it could be approached using the Rao-Blackwell theorem, but
...
There is thus no Yates contrast possible corresponding to the hypothetical
population of interest.  
For that matter, adjustment to any fixed population is no longer possible,
such as the US 2000 reference, unless groups are pooled so as to remove the
zero, and even then the estimate could be problematic due to extreme
weights.
...
satisfactory explanation so have nothing to offer on the subject.
I would be very reluctant to use such an estimate.
...
\begin{itemize}
  \item The NSTT algorithm is a disaster in waiting, 
    so little more needs to be said about situations where its behavior
    may be even worse.
  \item When fitting the original model, there will be one or more NA
    ...
    set to NA will depend on the order in which variables were listed in
    the model statement, which in turn will change the result.
