...
\begin{itemize}
  \item SAS type 3 is a particular algorithm for computing a
    ...
    \begin{itemize} 
      \item The utility of the Yates approach for selected designs is
        ...
        method.  Senn \cite{Sennxx} provides hope for change in one area.
       \item My own view is that for balanced or near balanced experiments 
         the Yates' hypothesis is very sensible, but for most biological data 
  ...
  \item A good portion of this note is focused on linear models and 
    ...
    case.
  \item  If one is going to use the Yates approach it should at least be
    computed correctly.
    The SAS type 3 computation for linear models is sophisticated and
    reliable; that used in phreg is neither.  I would describe this 
    recent addition as a black eye for the SAS package, seriously
    undermining their reputation for reliable compuatation.
    The calculated phreg results depend on which
    particular coding is chosen for the factors, the default coding
    leads to an unexpected result, and labeling that output as ``type 3''
    is deceptive.  Documentation of the
    actual procedure is completely lacking.
    (This is true for SAS version 9.3; I will update this note if 
    things change.)
\end{itemize}    
...
Thus I am naturally suspicious whenever a linear model idea is grafted
...
by looking at the weights for the sex coefficient.
...
The emprical reference and least squares reference are nearly
...
not possible to reweight the sample so as to exactly match the
...
dimnames(emat) <- list(c("Unadjusted", "MVUE: continuous age", 
                         "MVUE: categorical age", "Emprical (data) reference",
...
\begin{table} \centering
  \begin{tabular}{rlrrrrr}
    &&50--59& 60--69 & 70--79 & 80--89 & 90+ \\ \hline
<<echo=FALSE, results=tex>>=
tname <- c("Unadjusted", "Min var", "Emprical", "Yates")
for (i in 1:2) {
    for (j in 1:4) {
        cat("&",tname[j], " & ",
...
Equation \eqref{std} is overdetermined and all computer
programs add constraints in order to guarrantee a unique solution.
...
the results in table \ref{allest}.
<<treatment>>=
...
Appendix \ref{sect:coding} describes three algorithms for the compuation.
These are implimented in the \code{yates} function, which has been
added to the survival package.  
...
in equaion \eqref{ycont} is zero and the simple 
...
SAS is a useful
and almost necessary adjuct, as many of the steps in the document are side
...
\begin{enumerate}
  \item Many take the sequential ANOVA table as primary, i.e., a set of
    ...
    Since population weightings other than the LS one do not fit nicely into
    the nested framework they are excluded.
...
\section{Cox models}
\subsection{LRT tests and contrasts}
...
cfit3 <- coxph(Surv(futime, death) ~ sex * age2, flchain)
#
summary(cfit1)$coef
...
If we follow the same logic and stratify on age groups, the
resulting estimated
sex effect for this data set is very similar to the additive model
fit.
<<>>=
cfit4 <- coxph(Surv(futime, death) ~ sex + strata(age2), flchain)
summary(cfit4)$coef
@ 
...
nstt.age <- diag(9)[2:5,]
ysex <- yates.sex[1,-1] - yates.sex[2,-1]  #test for sex, drop intercept
#
contrast(ysex, cfit3)$ss
...
cfit3b <- coxph(Surv(futime, death) ~ xmat3[,-1], flchain)
anova(cfit3b, cfit3)
@ 
This agrees with the LR ``type 3'' test of the phreg printout.

\subsection{Comparing estimates}
Since we have come all this way, one may as well look at the 
Cox model estimates a little more deeply.
...
frequently.
<<>>=
with(flchain, table(sex, age2, death))[,,2] #table of deaths per cell
@ 
...
Note that any population renorming requires estimates of the cell means, 
whether
they were explicit parameters or not, i.e., the model fit must include
interaction terms.

Here are three common algorithms for this post-hoc computation.
...
data1 <- data.frame(y = rep(1:6, length=20),
                    x = factor(letters[rep(1:3, length=20)]),
                    z = factor(LETTERS[rep(1:4, length=10)]))
data1$x[19] <- 'c'
data1 <- data1[order(data1$x, data1$z),]
row.names(data1) <- NULL
#table showing the number in each cell
with(data1, table(x,z))
     
# data2 -- single missing cell
indx <- with(data1, x=='a' & z=='D')
data2 <- data1[!indx,]

#data3 -- missing the diagonal
data3 <- data1[as.numeric(data1$x) != as.numeric(data1$z),]
@
...
drop1(fit3, .~.)
@ 
Each choice of contrasts, other than contr.sum, gives a different
answer, both of which are nonsensical.
...
# transpose, to fit on the page better
effects <- t(rowsum(ux, indx)/4)
contr <- effects[,-1] - effects[,1]
contr

yates(fit, method="ATT")$x
@ 
The data set ux has 12 rows, one for each of the 12 unique x*z combinations.
By definition the data set represents a uniform weighting of the 12 cells.
...
Taking the average of the first four rows gives an estimate of the
mean response when factor x = 'a' and etc for b and c,
any set of two differences between these three rows is a Yates contrast
matrix.

Define the order of a term as 0 for the intercep1, 1 for main effects,
2 for 2 way interactins, etc.
Notice that for the \code{x} term, any other variables of the same or lower
order cancel out in the contrast.  This is a side effect of the fact that
we are computing over a balanced subset.
...
For data1 we will have colums for the intercept (1), x (3), z (4), and the
xz interaction (12) = 20 columns.
Now define the lower triangular dependency matrix $D$ such that
\begin{itemize}
...
columns of $D$ with the resdiuals from a regression on all columns to the
right of it.
Simple inspection shows that the columns of $D$ corresponding to any given
effect will already be orthagonal to other effects in $D$ \emph{except}
those for interactions that contain it; so the regression could use only
a selection of columns to the right.  
...
(I suspect it can be approached using the Rao-Blackwell theorem.)  
...
I would be very reluctant to use such an estimate, however.
...
\begin{itemize}
  ...
  \item For the STT method, our statement that certain other columns in $D$ 
    will be orthagonal to the chosen effect is no longer true.  To match
    SAS, the orthagonalization step above should include only those
