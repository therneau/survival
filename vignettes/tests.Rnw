...
\begin{itemize}
  ...
  \item  If one is going to use the Yates approach it should at least be
    ...
    undermining their reputation for reliable computation.
...
These are implemented in the \code{yates} function, which has been
...
Senn \ref{Senn00} points out the strong parallels beween tests for main effects
...
cbind(contr.sum= y1$ss, contr.SAS =yates(fit2, method="NSTT")$ss,
                  contr.treatment =yates(fit3, method="NSTT")$ss)
@ 
