# residuals for a survfit object, from a coxph fit
residuals.survfitcoxms <- function(object, times, type="pstate", 
                                    collapse=FALSE, weighted=collapse, 
                              data.frame= FALSE, extra=FALSE, ...) {
    stop("residuals for survival curves from a multistate PH model are not yet a
vailable")
}

residuals.survfitcox <- function(object, times, type= "pstate",
                              collapse=FALSE, weighted=collapse, 
                              data.frame= FALSE, extra=FALSE, ...){
    if (!inherits(object, "survfitcox"))
        stop("argument must be a survfit object based on a coxph model")
    ...
    survfitms <- FALSE  # still to be finished
    ...
    Call <- object$call
    coxfit <- eval(Call$formula)  # this contains the name of the coxph fit
                                  # (we assume it is still available)
    ...
    n <- nrow(Y)
    strata <- cdata$strata
    if (is.null(strata)) strata <- integer(n) #everyone in strata 0
    nstrat <- length(unique(strata))

    id <- cdata$id  # may be NULL
    cluster <- cdata$cluster  #ditto
    ...
    if (is.null(object$weight)) casewt <- cdata$weights
    if (is.null(casewt)) {
