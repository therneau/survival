...
The transitions matrix shows that \Scode{ajfit$transitions[1,1]} subjects
transitioned from this initial state to AIDS, \Scode{ajfit$transitions[1,2]}
transitioned to SI, and \Scode{ajfit$transitions[1,3]} were censored.
...
pstack <- function(fit, top=FALSE, ...) {
    ...
    if (is.matrix(temp$pstate))  # usual case
        ...
        temp <- aperm(apply(temp$pstate, 1:2, cumsum), c(2,3,1))
...
This author much prefers the unstacked version, which shows more
...
(It also gives more opportunity to mess it up, and for this reason the
...
common1b <- coxph(list( Surv(time, event) ~ ccr5, 1:1 ~ ccr5/common),
                  data=adata, id=patnr)
@ 
...
stratum.
The partial likelihood for the Cox model has a term at each event time, each
term is a ratio that compares the risk score of the event to the sum of
risk scores for all subjects who were at risk for the event. 
...
one labeled with \code{cause} = AIDS and the other as SI.
This is not a proper risk set, since the data set has been set up so that
one of those clones is ``allowed to'' have an AIDS event and the other to have
an SI event; when an AIDS event occurs not all of them are actually at risk.
...
that the partial likelihood (PL) values satisfy an identity
PL(fit 2) = PL(fit 1) - d log(2), where $d$ is the total number of events.
...
estimates.
One can further show that if \code{cause} is added to the model as a second
...
These identities do not assure us that this \emph{psuedo risk set} 
...
One can not directly fit a version of \code{test2} using the joint model,
...
i.e. what set of covariate values. 
<<aidscurve>>=
...
The resulting fit replicates coefficient in the ``proportional hazards''
columns of table T3. 

<<efit2>>=
efit2 <- coxph(list(Surv(tstart, tstop, event) ~ dissub + age + drmatch + tcd,
                    1:state("RelDeath") ~ 1 / common,
                    "PR":"RelDeath" ~ priorpr),
                    id=id, data=edata, ties='breslow')
...
These are the same models as before but with 
...
efit4 <- coxph(list(Surv(tstart, tstop, event4) ~ dissub + age + drmatch + tcd,
                    "(s0)":state("RelDeath2", "RelDeath1") ~ 1/ common,
                    "PR":"RelDeath2" ~ priorpr), 
                    id=id, data=edata, ties='breslow')
@ 
re-package tabove calculation as a four state model which has two
