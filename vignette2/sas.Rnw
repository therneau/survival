...
numbers don't exactly agree, if one run used Efron and the other Breslow.
...
time for computing the terms which should be added to the sums,
...
and we will refer the run time for an ideal program as $O(n) + O(d)$.
Since $n >d$, often by a large margin, the first term normally dominates 
the second, and, more importantly,
real programs and algorithms will not match the ideal, since there are
...
Essenially no time is spent on the ``who in each risk set'' task. 
...
(Maintaining/updating a list of values is a well known problem in computer
...
# Adding xb last makes if remain a matrix within the dataframe.
...
reflecting 24 years of bi-annual follow up in the NHS.  
...
shown in the next graph.  Evaluation required 4 iterations of the 
\code{coxph} function. 
...
Said algorithm is expected to have $O(2n)$ run time since it touchs
...
singular that round off errors intervene.
...
leading to a consequence 730 fold increase in the compute time for \code{coxph}.
...
\end{verbatim}
where \code{dummy} is a dummy variable set to a constant value (often 1.0).
...
from \code{fit1} on the original time scale and \code{fit3a} and \code{fit3b}
on the coarsened and expanded data.
...
\Sexpr{round(stat2[6,3])} seconds for to fit 1/12 of the data in the expanded
...
over the 50 year age range of the NHS, a coarsening the time scale to months
is a small relative change to any subject.
A closer look reveals that the fit4a and fit4b differ by less than 1\%, and
...
Why is the (age1, age2) fit so much longer than an identical model stratified
...
the $O(nd)$ compuation for the original data set.  
...
be no surprise that replacing the risk set with a smaller subsample has no
measurable effect on the estimates; whether that were to be done with a
random subsample, stratification on entry date (1/300) or stratificatin on the
...
I have not been able to replicate any case where phreg is significanly
faster than coxph, and in that sense the report this behaviour from the
...
automatically print a quantity the type 3 test --- you couldn't turn it
off in fact.  
...
reference.  
This is very different than the type 3 narrative of that it is a
global test.
