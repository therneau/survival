...
One simple way to thin this down is to match subjects into sets based on
their age at first response, in months; the variable \code{agem} in our
data set above.  This gives groups of size 350--450.  What does it do to
the compute time?
...
hardware at the time we expect no more than a 2x distortion.)
...
A standard Cox model program can be used to fit this.
...
indeed using this approach, but had also coarsened the time scale to use time 
in months. 
...
If we make the further decision to use those with the same enrollment 
age as the ``selected subset'', no new data set
needs to be created at all.

<<matched>>=
dummy <- rep(1, nrow(nhs))
dmonth <- floor(nhs$age2 * 12/365.25)   # endpoint in months
system.time(assign("mfit", coxph(Surv(dummy, death) ~ x1 + x2 + xb +
                                     strata(dmonth, agem), nhs)))
@ 

The months with no deaths at all add nothing to the loglikihood and could
be removed from the data set, but there are very few of them.
Create an expanded data set using these parameters:
time scale in months, separate strata based on age at enrollment,
and expanded data within strata based on death month.
Since subjects
come in about every 2 years using months will expand the data set by 
approximately
24 fold. To keep the vignette from taking too long to run we'll only use
the first 1/10 of the NHS identifiers.
...
for (i in 1:3) {
    ...
    time1 <- system.time(coxph(Surv(month1, month2, death) ~ x1 + x2 + xb,
                         ...
                         gcFirst= TRUE)
    time2 <- system.time(coxph(Surv(month2, death) ~ x1 + x2 + xb + 
...
Because of tied times the new data set is a little less than 2.4 (24/10) fold 
...
exactly this same trick, i.e., it builds the risk sets for each event directly,
marks each risk set a unique stratum, and adds a dummy time value.
...
the entire nhs cohort (40 million rows), which is 
less than 1/5 of the counting process form.
Because SAS jobs at our institution have to be run on a separate server
due to licensing issues we can't directly compare the SAS and R run
times of 18 and 35 seconds for the 1/10 expansion (though the underlying
hardware at the time should be withing $\pm$ 30\%),
the important take home point is that both have linear growth.
