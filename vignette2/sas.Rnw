...
11, 86, and 268 seconds for 1/10, 3/10, and 5/10 of the subjects,
...
indeed using
an expanded data set, but had also coarsened the time scale to use time 
in months. 
Per the prior section on the Efron estimate, for a data set on this time 
scale the use of months vs. days will hardly change the final Cox model 
estimates, but it does lead 
to a much smaller intermediate data set.
...
The flawed NSTT algorithm is, I believe, the source of a common critique that
...
For example, the first two fits below code reproduce the phreg results
that use 'A' and 'D' as the reference levels.
The \code{yates} function produces Yates' population contrast;
it's result is identical whether fit1 or fit2 is used as the input.
The final phreg fit that used a sum constraint,
referred to as ``effects'' coding in SAS, agrees with the Yates' result.
