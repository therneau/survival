...
times of 18 and 35 seconds for the 1/10 expansion; 
the important take home point is that both 
have linear growth.

\begin{verbatim}
proc phreg data= nhs;
   where (id <= 12170);
   model (age1 age2) * death(0) = x1 x2 x3 x4;
proc phreg data=nhs2;
   where (id < 8113);
   model (age1 age2) * death(0) = x1 x2 x3 x4;
proc phreg data=nhs2;
   model age2*death(0) = x1 x2 x3 x4;
   strata age2;
\end{verbatim} 
