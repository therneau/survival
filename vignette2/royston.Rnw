...
I expect this latter has caused many to assume that the \code{(\_t, \_d)}
pair contain RFS, while examination shows that they encode 10 year relapse.
...
at 2000 to exclude large outliers, however, I match.
...
The Cox model was fit using fractional polynomials.  If we use the
