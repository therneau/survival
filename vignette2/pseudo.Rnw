...
the \emph{uncensored} data.  These pseudo values are not
...
Since the Kaplan-Meier gives an unbiased estimate of $S$,
...
The sojourn time for all states must sum to $\tau$,
...
the resulting object acts as the data for subsequent fits.

Pseudo-values' validity relies on an assumption of non-informative censoring.
When censoring is uninformative within strata, but not overall, a solution is
to base the pseudo values on the per-stratum survival curves.
...
from different patient populations, with different overall survival.
If those institutions had different median follow-up times as
well as differential survival, this would introduce informative censoring.
(Differential follow-up of quite common in multi-center studies; some will join
late due to administrative delays.)
...
It is interesting that correct variance is somewhat smaller than the 
simple glm result, in this case.
...
truncation is covariate dependent (personal communication, PK Andersen).  
