...
Of course, due to censoring our data is incomplete.

Pseudo-values are based on a simple idea.  Suppose the data are imcomplete but
we have an If we have an estimator 
$\hat\theta = E(f(Y)$ of the quantity, e.g., $f$ = survival probability at
...
but a critic might call the IJ approach ``pseudo-pseudo'' values and claim that
...
\begin{equation*}
 \mu = \int S(t) dt
\end{equation}
This extends to the multi-state case, the expected time in state will be
...
were 8.1, the phrase would be an expected liftime of 8.1 out of the next 10.
Other labels the sojourn time, or for a multi-state model, 
...
10 years, the last observed event time, or the point at which one of the
curves has 0 subjects. 
...
RMST and its standard error directly from the \code{survfit}. 
...
This section needs some input from Thomas L.  

\section{Survival and probability in state}
\subsection{AML data}
As a first case for this endpoint look at a simple survival data set, 
for a multi-state model this
extends to the more general probability in state vector $p(t)$.
Psuedo-values are obtained from the \code{survfit} object.  
Start with the simple AML data set, and get the pseudo values for 12 and 24 month
survival.
This data set records the time to relapse for patients with acute myelogenous
leukemia.
...
The \code{data.frame} argument causes the value to be returned in long form
as a data.frame.
<<test2>>=
...
The above code contains a lot of ideas. The Cox model estimates that the
subjects on the no maintainance arm have a hazard rate of about 2.5 fold higher
than the maintainance arm, $p=.09$.
The linear model estimates the probability of no recurrence at 12 or 24 months
...
Of potentially more consequence, the pseudo-value has two observations for
...
Most users will prefer a model for the risk of death, i.e., for psuedo values
based on the probability of death $1- S(t)$.  
It is easy to show that these are simply $1- p(S)$ where $p(S)$ are the 
psuedovalues for $S$.  This was saved as the variable \code{pdeath}.
...
Psuedo values based on the multi-state Aalen-Johansen estimate allow a
