...
each unique time.
We use then calculate the restricted mean survival time (RMST), which is the
...
rmst <- function(fit, tmax) {
    ...
    if (missing(tmax)) tmax <- max(fit$time)
...
Most importantly, the check did not reveal any reversions from a greater
...
to avoid this.
The xet of subjects at risk at age 40 (at risk for further events) 
defines the initial
