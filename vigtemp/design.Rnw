...
The reason is that S has allowed me to turn ideas into software.  
Not just programs that can let me write a paper, but software that 
I can count on for practical
analysis, imbedded into a system with rich tools for data manipulation and
presentation.
I am a biostatistician by trade, in an institution where the primary motivation is
...
so I need to be able to translate a new idea into software quickly.
  ...
  \item Flexibility.  I often mix and match approaches from prior functions, or use
their results in new ways that were unforseen at the time of writing. 
...
components of a model's output. 
  ...
  \item Reliability.  One of my colleages said this best during the interview of
    ...
    data:
    ``An A- is no longer good enough: results have to be correct''.

\item Algorithmic excellence and code simplicity.
This is more personal.  I would like to create the best.
\end{enumerate}
...
Below, in that it also documents several technical aspects of package
creation that I have not found written about in a cohesive way elsewhere.
...
\begin{itemize}
  \item Modeling
    \begin{description}
      ...
      \item[aareg] Aalen's additive hazard model
    \end{description}
 ...
 time.
  ...
  \item Ancillary
    \begin{description}
      \item[Surv] a survival time
      \item[strata] marks a variable as a stratum indicator
      \item[cluster] marks a variable as a cluster indicator
    \end{description}
\end{itemize}
...
R provides very good facilities for this which are easy to use; this
...
S and we certainly want to use it.  
...
numeric operators on Surv objects are very simple:
...
object is part of a data frame, and a subset of the data frame
is chosen.
...
This can make dataframe methods faster and was a useful addition at the
time I wrote it.  Whether this speed delta is still true I don't know, but
rather doubt it.
The print method turns the object into a character form and then calls
the ususal print function with \code{quote=FALSE}.
The most complicated method method for Surv objects
...
were compactness,
to make a survival object recognizable as one whenever it printed, 
...
In many of the core routines the result is stored in a variable ``f''.
...
The code then has to do extra work to pull a cluster variable out of the
...
There are two ways to create a method.  The first one is simple but a bit
...
if so the routine returns \code{m}, otherwise it goes forward with the
usual computation.  Note that \code{lm} already has a method argument which is
used to decide the compuational approach.  
The \code{model.frame.lm} method then chooses between two actions: if its
...
This is actually a very nice idea, since any changes to the original routine
...
One problem is data dependent transformations, \code{ns(x, df=3)} for instance
will construct a natural spline basis whose knots depend on selected quantiles
...
also what knot points were used by \code{ns}.
...
and contrast informaion as an attribute of the $X$ matrix.
...
The model matrix code for coxph models can't use the generic function
above \code{model.matrix(Terms,...}.
\begin{enumerate}
  \item In a Cox model the baseline hazard plays the role of an intercept.
    ...
    So we first force the model to have an intercept, then toss the first
    column of X after it is built.
  \item Neither strata nor cluster variable should appear in the model.
\end{enumerate}
...
model.matrix.coxph <- function(object, data=NULL, 
    ...
    if (is.null(data) && !is.null(object[['x']])) 
        return(object[['x']]) #don't accidentally match "xlevels" using $
...
The model.matrix.lm function is much shorter.  It first checks for
the existence of a pre-existing \code{x} component.  If that is not
present, then it calls model.frame.lm followed by the generic
model.matrix function.
If called with a data argument the routine currently does not correctly
use it (a fix may appear shortly).  This has been a rare to non-existent
usage, however.
...
distraction.  The line \code{eval(temp, parent.frame())} looks like it
would be a problem when we have 
predict.coxph $\rightarrow$ model.frame.coxph $\rightarrow$ model.frame.
Say someone had a variable \code{x} in their model and it's not part of the
data frame.  Then the parent frame when the eval is done inside
model.frame.coxph will be the predict function.  If my predict.coxph
function happened to have a variable \code{x} we'd pick that up instead.
Right?
...
in the environment of the formula.  The second argument of eval is
completely ignored.
...
data argument none of these issues arise in the first place.
...
Here is the model.frame function.
<<model.frame.coxph>>=
...
@ 
In the above, the odds that someone will want to rebuild the original data
frame but with different weights, subset, or na.action is very slim.
Even with a new data set they are unlikely, but including them is not
much extra work.
...
and a collection of
more formal ones of which ``S4'' is the first and most common and
was introduced about 10 years after S3. 
...
and there is a commonly voiced sentiment that
one always use the new methods by preference; some members of R core are
in this camp, lending the argument weight.
(Although the large historical body of code based on S3 means 
that support for it will not go away.)
In counterpoint are arguments that S4 is a complexity disaster
suitable only for masochists.
My view differs from these; 
...
date or
date/time objects.  You want to specify that date+integer, 
date-integer, and
date-date create sensible results, 
but integer-date and date+date do not.
The S4 system does this with ease.
As another example, the
coxme function makes heavy use of a libary for 
Block Diagonal Sparse matrices.
The bdsmatrix library used S4 classes entirely, since it allowed me 
to define
the appropriate routine to use for combinations 
like \code{bdsmatrix * numeric}
or \code{matrix %*% bdsmatrix}. 

The model fit objects for survival and coxme are all of S3 form.
...
accomodate change.  
The additions to the original coxph object make up almost
...
etc with
each having more components.  I think I'd be up to 9 or 10 by now --- it is
very hard to guess what will be important next year, 
even in my area of particular
expertise.  To quote the sage
``Making predictions is difficult, especially
about the future.'' (Yogi Berra).
This is my greatest difficulty with S4 and other formal objects, to
get a complex class like a coxph or other model correct on the first try
requires near omniscience.

One further restriction on S4 objects is that all of their components 
must also be valid S4 objects.  
For a simple object like Surv this is no hindrance, since the
basic elements of the language are, for obvious reasons, 
all defined using the S4 approach. 
However, the terms structure is not S4 and this plays a key role in
most of my downstream calculations, loss of it would be a major blow.
...
\emph{can} be done as well being an an illstration of the 
difficulties this choice brings.
...
almost certainly use the S4 paradym.
...
I am a pragmatist, with both metric and English wrenches in my toolbox 
(well used),
both frequentist and Bayes methods in my analyses, 
and both S3 and S4 methods in my code.  


% -*- mode: noweb; noweb-default-code-mode: S-mode; -*-
\section{Output}
...
\begin{itemize}
      ...
      \item Martingale residuals and linear predictor (as in lm).  
        ...
        Times change and theory advances, however, and now I use them only
        occassionally.
       \item A vector of means, which was used to center the covariates 
        ...
        more stabile if
        you do it.  Also, it was traditional to report the basedline 
        survival for
        this particular covariate combination.
...
\begin{itemize}
  \item Robust variance.  If someone asks for a robust variance 
    ...
    rest of the survival functions
    the original variance is saved as \code{naive.var} and the 
    robust one takes its
    place in \code{var}.
  \item Information on any penalized terms that were used in the model.  
    This addition
    allowed for smoothing spline and simple frailty terms.
...
I would have chosen, but consistency for the users was deemed more important.
...
The survival package test suite currently has 74 test files. 
...
users from accidentally overwriting one of my routines (or perhaps purposefully
if it is FH), but gets in the way when debugging.
